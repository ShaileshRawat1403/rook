use crate::agents::{Agent, AgentConfig, RookPlatform};
...
impl AgentManager {
        ...
        AGENT_MANAGER
            .get_or_try_init(|| async {
                ...
                let max_sessions = config
                    .get_rook_max_active_agents()
                    .unwrap_or(DEFAULT_MAX_SESSION);
                let default_mode = config.get_rook_mode().unwrap_or_default();
    ...
    pub async fn get_or_create_agent(&self, session_id: String) -> Result<Arc<Agent>> {
        ...
        if let Ok(session) = self.session_manager.get_session(&session_id, false).await {
            mode = session.rook_mode;
            info!(rook_mode = %mode, session_id = %session_id, "Session loaded");
        }
        ...
        let config = AgentConfig::new(
            ...
            mode,
            Config::global()
                .get_rook_disable_session_naming()
                .unwrap_or(false),
            RookPlatform::RookDesktop,
        );
...
mod tests {
    ...
    #[test_case(RookMode::Approve ; "approve")]
    #[test_case(RookMode::Chat ; "chat")]
    #[test_case(RookMode::SmartApprove ; "smart_approve")]
    #[tokio::test]
    async fn test_agent_inherits_session_mode(mode: RookMode) {
        let temp_dir = TempDir::new().unwrap();
        let manager = create_test_manager(&temp_dir).await;
        ...
        assert_eq!(agent.rook_mode().await, mode);
    }
    ...
    async fn test_session_mode_isolation() {
        ...
        assert_eq!(a1.rook_mode().await, RookMode::Approve);
        assert_eq!(a2.rook_mode().await, RookMode::Auto);
    }
}

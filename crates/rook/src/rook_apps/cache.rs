...
use super::app::RookApp;

static CLOCK_HTML: &str = include_str!("../rook_apps/clock.html");
static CHAT_HTML: &str = include_str!("../rook_apps/chat.html");
...
impl McpAppCache {
    ...
    fn ensure_default_apps(&self) {
        for (uri, html) in [("apps://clock", CLOCK_HTML), ("apps://chat", CHAT_HTML)] {
            if self.get_app(APPS_EXTENSION_NAME, uri).is_none() {
                if let Ok(mut app) = RookApp::from_html(html) {
    ...
    pub fn list_apps(&self) -> Result<Vec<RookApp>, std::io::Error> {
        ...
        for entry in fs::read_dir(&self.cache_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                match fs::read_to_string(&path) {
                    Ok(content) => match serde_json::from_str::<RookApp>(&content) {
    ...
    pub fn store_app(&self, app: &RookApp) -> Result<(), std::io::Error> {
    ...
    pub fn get_app(&self, extension_name: &str, resource_uri: &str) -> Option<RookApp> {
        ...
        fs::read_to_string(&app_path)
            .ok()
            .and_then(|content| serde_json::from_str::<RookApp>(&content).ok())
    }
        ...
        for entry in fs::read_dir(&self.cache_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                if let Ok(content) = fs::read_to_string(&path) {
                    if let Ok(app) = serde_json::from_str::<RookApp>(&content) {

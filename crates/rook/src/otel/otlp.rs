...
/// Promotes rook config-file OTel settings to env vars before exporter build.
...
fn create_resource() -> Resource {
    let mut builder = Resource::builder_empty()
        .with_attributes([
            KeyValue::new("service.name", "rook"),
            KeyValue::new("service.version", env!("CARGO_PKG_VERSION")),
            KeyValue::new("service.namespace", "rook"),
        ])
...
fn create_otlp_tracing_layer() -> OtlpResult<OtlpTracingLayer> {
    ...
    let tracer = tracer_provider.tracer("rook");
...
fn create_otlp_metrics_filter() -> FilterFn<impl Fn(&Metadata<'_>) -> bool> {
    FilterFn::new(|metadata: &Metadata<'_>| {
        ...
        if metadata.level() == &Level::DEBUG {
            let target = metadata.target();
            if target.starts_with("rook::telemetry")
                || target.starts_with("rook::metrics")
...
mod tests {
    ...
    use rook_test_support::otel::clear_otel_env;
    ...
    #[test_case(
        ...
        Resource::builder_empty()
            .with_attributes([KeyValue::new("service.name", "rook"), KeyValue::new("service.version", env!("CARGO_PKG_VERSION")), KeyValue::new("service.namespace", "rook")])
            .with_detector(Box::new(TelemetryResourceDetector))
            .build();
        "no env vars uses rook defaults"
    )]
    #[test_case(
    ...
    #[test_case(&[("RUST_LOG", "rook=debug"), ("OTEL_LOG_LEVEL", "trace")], Level::TRACE; "directive RUST_LOG falls through to OTEL_LOG_LEVEL")]
    #[test_case(&[("RUST_LOG", "rook=debug")], Level::INFO; "directive RUST_LOG falls through to default")]

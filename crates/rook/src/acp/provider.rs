...
use crate::config::{ExtensionConfig, RookMode};
...
pub struct AcpProviderConfig {
    ...
    pub mode_mapping: HashMap<RookMode, String>,
...
pub struct AcpProvider {
    name: String,
    model: ModelConfig,
    rook_mode: Arc<Mutex<RookMode>>,
    mode_mapping: HashMap<RookMode, String>,
...
impl AcpProvider {
    ...
    pub(crate) async fn send_set_mode(&self, _rook_id: &str, mode_id: String) -> Result<()> {
...
impl Provider for AcpProvider {
    ...
    async fn update_mode(&self, session_id: &str, mode: RookMode) -> Result<(), ProviderError> {
        let mode_str = self
            .mode_mapping
            .get(&mode)
            .cloned()
        ...
        if let Ok(mut guard) = self.rook_mode.lock() {
            *guard = mode;
        }
        Ok(())
    }
    ...
    async fn stream(
        ...
        let rook_mode = *self
            .rook_mode
            .lock()
            .map_err(|_| ProviderError::RequestFailed("rook_mode lock poisoned".into()))?;

        let reject_all_tools = rook_mode == RookMode::Chat;
                ...
                match update {
                    ...
                    AcpUpdate::ToolCallStart { id, name, kind, raw_input } => {
                        if reject_all_tools {
                            ...
                            // call. rook.acp.kind preserves ACP's stable categorization for
                    ...
                    AcpUpdate::PermissionRequest { request, response_tx } => {
                        if let Some(decision) = permission_decision_from_mode(rook_mode) {
    ...
    async fn run(
        ...
        Client
                        ...
                        match &notification.update {
                            SessionUpdate::CurrentModeUpdate(update) => {
                                if let Some(mode) = resolve_mode(
                                    ...
                                    if let Ok(mut guard) = rook_mode.lock() {
                                        *guard = mode;
                                    }
                                }
                            }
                            ...
                            .and_then(|g| g.as_ref())
                        {
                            match notification.update {
                                ...
                                SessionUpdate::ToolCall(tool_call) => {
                                    ...
                                    // rook.external_dispatch marker keeps `name` off the
            ...
            .connect_with(transport, async move |cx: ConnectionTo<Agent>| {
                handle_requests(config, rook_mode, cx, rx, prompt_response_tx, init_tx).await
            })
            .await?;
            ...
            ClientRequest::NewSession { response_tx } => {
                ...
                let result = match session {
                    Ok(session) => {
                        ...
                        apply_session_mode(&config, &rook_mode, &cx, session).await
                    }
...
async fn apply_session_mode(
    ...
    let current_mode = rook_mode.lock().ok().map(|mode| *mode);
...
/// Convert ACP `ToolCallContent` blocks into the rmcp `Content` shape rook's
...
fn reverse_mode_mapping(
    mode_mapping: &HashMap<RookMode, String>,
) -> HashMap<String, Vec<RookMode>> {
    let mut reverse: HashMap<String, Vec<RookMode>> = HashMap::new();
...
fn resolve_mode(
    reverse_modes: &HashMap<String, Vec<RookMode>>,
    mode_id: &str,
    current: &Arc<Mutex<RookMode>>,
) -> Option<RookMode> {
...
fn permission_decision_from_mode(rook_mode: RookMode) -> Option<PermissionDecision> {
    match rook_mode {
        RookMode::Auto => Some(PermissionDecision::AllowOnce),
        RookMode::Chat => Some(PermissionDecision::RejectOnce),
        RookMode::Approve | RookMode::SmartApprove => None,
    }
}
...
mod tests {
    ...
    #[test_case(RookMode::Auto => Some(PermissionDecision::AllowOnce) ; "auto allows")]
    #[test_case(RookMode::Chat => Some(PermissionDecision::RejectOnce) ; "chat rejects")]
    #[test_case(RookMode::Approve => None ; "approve defers")]
    #[test_case(RookMode::SmartApprove => None ; "smart_approve defers")]
    fn test_permission_decision_from_mode(mode: RookMode) -> Option<PermissionDecision> {
        permission_decision_from_mode(mode)
    }

    #[test_case(
        HashMap::from([
            (RookMode::Auto, "yolo".to_string()),
            (RookMode::Approve, "default".to_string()),
            (RookMode::SmartApprove, "auto_edit".to_string()),
            (RookMode::Chat, "plan".to_string()),
        ]),
        HashMap::from([
            ("yolo".to_string(), vec![RookMode::Auto]),
            ("default".to_string(), vec![RookMode::Approve]),
            ("auto_edit".to_string(), vec![RookMode::SmartApprove]),
            ("plan".to_string(), vec![RookMode::Chat]),
        ])
    ...
    #[test_case(
        ...
        HashMap::from([
            ("bypassPermissions".to_string(), vec![RookMode::Auto]),
            ("default".to_string(), vec![RookMode::Approve]),
            ("acceptEdits".to_string(), vec![RookMode::SmartApprove]),
            ("plan".to_string(), vec![RookMode::Chat]),
        ])
    ...
    #[test_case(
        ...
        HashMap::from([
            ("full-access".to_string(), vec![RookMode::Auto]),
            ("read-only".to_string(), vec![RookMode::Approve, RookMode::Chat]),
            ("auto".to_string(), vec![RookMode::SmartApprove]),
        ])
        ; "codex duplicate read-only"
    )]
    fn test_reverse_mode_mapping(
        forward: HashMap<RookMode, String>,
        expected: HashMap<String, Vec<RookMode>>,
    ) {
    ...
    fn codex_reverse_modes() -> HashMap<String, Vec<RookMode>> {
        HashMap::from([
            ("full-access".to_string(), vec![RookMode::Auto]),
            (
                "read-only".to_string(),
                vec![RookMode::Approve, RookMode::Chat],
            ),
            ("auto".to_string(), vec![RookMode::SmartApprove]),
        ])
    }
    ...
    #[test_case(
        "full-access", RookMode::Auto, Some(RookMode::Auto)
        ; "unique mapping returns the only candidate"
    )]
    #[test_case(
        "read-only", RookMode::Approve, Some(RookMode::Approve)
        ; "duplicate prefers current when current is Approve"
    )]
    #[test_case(
        "read-only", RookMode::Chat, Some(RookMode::Chat)
        ; "duplicate prefers current when current is Chat"
    )]
    #[test_case(
        "read-only", RookMode::Auto, Some(RookMode::Approve)
        ; "duplicate falls back to first when current not in candidates"
    )]
    #[test_case(
        "unknown-id", RookMode::Auto, None
        ; "unknown mode id returns None"
    )]
    fn test_resolve_mode(mode_id: &str, current: RookMode, expected: Option<RookMode>) {
        ...
        if mode_id == "read-only" && expected == Some(RookMode::Approve) {
            assert!(result == Some(RookMode::Approve) || result == Some(RookMode::Chat));
        } else {
    ...
    fn tool_meta_pairs_external_dispatch_marker_with_acp_kind() {
        ...
        for (kind, expected) in cases {
            ...
            assert_eq!(
                tool_meta["rook.acp.kind"],
                serde_json::Value::String(expected.to_string()),
                "rook.acp.kind serialized wrong for kind={kind:?}"
            );
        }
    }
}

...
impl TelegramGateway {
    ...
    ///
    /// Files are stored under `<tmp>/rook_voice/voice_<uuid>.<ext>` so Rook
    ...
    fn save_voice_file(
        ...
        let dir = std::env::temp_dir().join("rook_voice");
    ...
    /// Build the text prompt that tells Rook about a voice message file.
    fn voice_prompt(
        path: &std::path::Path,
        duration: Option<i32>,
        mime_type: Option<&str>,
                    ...
                    match result {
                        Ok(updates) => {
                            for update in updates {
                                ...
                                // Rook to transcribe the file using CLI tools.
...
mod tests {
    ...
    fn voice_prompt_includes_path_and_duration() {
        let path = std::path::PathBuf::from("/tmp/rook_voice/voice_test.ogg");
        let prompt = TelegramGateway::voice_prompt(&path, Some(10), Some("audio/ogg"));
        assert!(prompt.contains("/tmp/rook_voice/voice_test.ogg"));
    ...
    fn voice_prompt_with_mp3_mime() {
        let path = std::path::PathBuf::from("/tmp/rook_voice/voice_test.mp3");

...
impl GatewayHandler {
                ...
                if let Some(gateway_type) = self.try_consume_code(message.text.trim()).await? {
                    ...
                    self.gateway
                        .send_message(
                            &message.user,
                            OutgoingMessage::Text {
                                body: "Welcome! Enter your pairing code to connect to rook.".into(),
                            },
                        )
                        .await?;
                }
    ...
    async fn complete_pairing(&self, user: &PlatformUser) -> anyhow::Result<()> {
        ...
        if let Ok(provider) = config.get_rook_provider() {
            update = update.provider_name(provider);
        }
        if let Ok(model_name) = config.get_rook_model() {
        ...
        self.gateway
            .send_message(
                user,
                OutgoingMessage::Text {
                    body: "Paired! You can now chat with rook.".into(),
                },
            )
            .await?;
    ...
    async fn sync_session_config(&self, session: &Session) -> anyhow::Result<bool> {
        ...
        let current_provider = config.get_rook_provider().ok();
        let current_model_name = config.get_rook_model().ok();
        let current_extensions = get_enabled_extensions();
        let current_mode = config.get_rook_mode().unwrap_or_default();
        ...
        let mode_changed = current_mode != session.rook_mode;
        ...
        if mode_changed {
            update = update.rook_mode(current_mode);
        }

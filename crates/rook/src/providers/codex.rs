...
use crate::config::{Config, ExtensionConfig, RookMode};
...
pub struct CodexProvider {
    ...
    #[serde(skip)]
    mode_by_session: tokio::sync::RwLock<HashMap<String, RookMode>>,
}
...
impl CodexProvider {
    ...
    /// Apply permission flags based on RookMode
    fn apply_permission_flags(cmd: &mut Command, rook_mode: RookMode) -> Result<(), ProviderError> {
        match rook_mode {
            RookMode::Auto => {
    ...
    async fn execute_command(
        ...
        if std::env::var("ROOK_CODEX_DEBUG").is_ok() {
        ...
        // This allows users to set ROOK_PROVIDER=codex without needing to specify a model
        ...
        Self::apply_permission_flags(&mut cmd, rook_mode)?;
...
impl Provider for CodexProvider {
    ...
    async fn stream(
        ...
        let lines = self
            .execute_command(system, messages, tools, rook_mode)
            .await?;
...
mod tests {
    ...
    use rook_test_support::TEST_IMAGE_B64;
    use std::collections::HashMap;
    ...
    #[test_case(RookMode::Auto, &["--yolo"] ; "auto_yolo")]
    #[test_case(RookMode::SmartApprove, &["--full-auto"] ; "smart_approve_full_auto")]
    #[test_case(RookMode::Approve, &[] as &[&str] ; "approve_no_flags")]
    #[test_case(RookMode::Chat, &["--sandbox", "read-only"] ; "chat_read_only")]
    fn test_apply_permission_flags(mode: RookMode, expected: &[&str]) {

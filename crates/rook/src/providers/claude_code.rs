...
pub struct ClaudeCodeProvider {
    ...
    #[serde(skip)]
    initial_mode: tokio::sync::Mutex<Option<RookMode>>,
}
...
impl ClaudeCodeProvider {
    ...
    fn build_stream_json_command(&self) -> Command {
        ...
        // Allow rook to run inside a Claude Code session.
    ...
    fn apply_permission_flags(cmd: &mut Command) -> Result<bool, ProviderError> {
        ...
        match rook_mode {
            ...
            RookMode::SmartApprove | RookMode::Approve => {
                cmd.arg("--permission-prompt-tool").arg("stdio");
                Ok(true)
            }
            RookMode::Chat => Ok(false),
        }
    }
...
impl Provider for ClaudeCodeProvider {
    ...
    async fn update_mode(&self, _session_id: &str, mode: RookMode) -> Result<(), ProviderError> {
...
mod tests {
    ...
    use chrono::Utc;
    use rook_test_support::session::TEST_SESSION_ID;
    use serde_json::json;
    use std::collections::HashMap;
    use std::fs;

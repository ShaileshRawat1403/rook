...
fn resolve_ollama_num_ctx(model_config: &ModelConfig) -> Option<usize> {
    ...
    let input_limit = match config.get_param::<usize>("ROOK_INPUT_LIMIT") {
        Ok(limit) if limit > 0 => Some(limit),
        Ok(_) => None,
        Err(crate::config::ConfigError::NotFound(_)) => None,
        Err(e) => {
            tracing::warn!("Invalid ROOK_INPUT_LIMIT value: {}", e);
            None
        }
    };
...
/// Configurable via OLLAMA_STREAM_TIMEOUT, ROOK_STREAM_TIMEOUT, or falls back
...
/// Priority: OLLAMA_STREAM_TIMEOUT > ROOK_STREAM_TIMEOUT > OLLAMA_TIMEOUT > default (120s).
...
fn resolve_ollama_chunk_timeout() -> u64 {
    ...
    if let Ok(val) = config.get_param::<u64>("ROOK_STREAM_TIMEOUT") {
        if val > 0 {
            return val;
        }
    }
...
mod tests {
    ...
    fn test_apply_ollama_options_uses_input_limit() {
        let _guard = env_lock::lock_env([("ROOK_INPUT_LIMIT", Some("8192"))]);
    ...
    fn test_apply_ollama_options_falls_back_to_context_limit() {
        let _guard = env_lock::lock_env([("ROOK_INPUT_LIMIT", None::<&str>)]);
        let model_config = ModelConfig::new("qwen3")
            .unwrap()
    ...
    #[test]
    fn test_resolve_ollama_chunk_timeout_uses_rook_stream_fallback() {
        let _guard = env_lock::lock_env([
            ("OLLAMA_STREAM_TIMEOUT", None::<&str>),
            ("ROOK_STREAM_TIMEOUT", Some("90")),
            ("OLLAMA_TIMEOUT", Some("300")),

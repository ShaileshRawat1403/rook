...
impl<'a> SessionUpdateBuilder<'a> {
    ...
    pub fn rook_mode(mut self, mode: RookMode) -> Self {
        self.rook_mode = Some(mode);
        self
    }
...
impl SessionManager {
    ...
    pub async fn create_session(
        ...
        self.storage
            .create_session(working_dir, name, session_type, rook_mode)
            .await
    }
...
impl SessionStorage {
    ...
    pub(crate) async fn pool(&self) -> Result<&Pool<Sqlite>> {
        self.initialized
            .get_or_try_init(|| async {
                ...
                .await
                .unwrap_or(false);
                let sessions_exists = sqlx::query_scalar::<_, bool>(
                    r#"SELECT EXISTS (SELECT name FROM sqlite_master WHERE type='table' AND name='sessions')"#,
                )
                .fetch_one(&self.pool)
                .await
                .unwrap_or(false);

                if schema_exists || sessions_exists {
    ...
    async fn create_schema(pool: &Pool<Sqlite>) -> Result<()> {
        ...
        sqlx::query(
            ...
            CREATE TABLE sessions (
                ...
                rook_mode TEXT NOT NULL DEFAULT 'auto',
                thread_id TEXT
            )
        "#,
        )
    ...
    async fn import_legacy_session(pool: &Pool<Sqlite>, session: &Session) -> Result<()> {
        ...
        INSERT INTO sessions (
            ...
            provider_name, model_config_json, rook_mode
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        "#,
        )
        .bind(&session.id)
        ...
        .bind(session.rook_mode.to_string())
        .execute(&mut *tx)
        .await?;
    ...
    async fn run_migrations(pool: &Pool<Sqlite>) -> Result<()> {
        ...
        let schema_table_exists = Self::table_exists(&mut tx, "schema_version").await?;
        let current_version = Self::get_schema_version(&mut tx).await?;

        if !schema_table_exists {
            info!("Initializing schema_version table for legacy session database...");
            Self::apply_migration(&mut tx, 1).await?;
            if current_version > 0 {
                Self::update_schema_version(&mut tx, current_version).await?;
            }
        }
    ...
    async fn get_schema_version(tx: &mut sqlx::Transaction<'_, Sqlite>) -> Result<i32> {
        let table_exists = Self::table_exists(tx, "schema_version").await?;

        if !table_exists {
            if !Self::table_exists(tx, "sessions").await? {
                return Ok(0);
            }

            if Self::column_exists(tx, "sessions", "rook_mode").await? {
                return Ok(8);
            }
            if Self::column_exists(tx, "sessions", "thread_id").await? {
                return Ok(10);
            }
            if Self::column_exists(tx, "messages", "message_id").await? {
                return Ok(7);
            }
            if Self::column_exists(tx, "sessions", "provider_name").await? {
                return Ok(6);
            }
            if Self::column_exists(tx, "sessions", "session_type").await? {
                return Ok(5);
            }
            if Self::column_exists(tx, "sessions", "name").await? {
                return Ok(4);
            }
            if Self::column_exists(tx, "messages", "metadata_json").await? {
                return Ok(3);
            }
            if Self::column_exists(tx, "sessions", "user_recipe_values_json").await? {
                return Ok(2);
            }
            return Ok(1);
        }
        ...
        if version >= 10 && !Self::column_exists(tx, "sessions", "rook_mode").await? {
            warn!("Database schema_version={version} but missing rook_mode column, adjusting to 7");
            return Ok(7);
        }
    ...
    async fn update_schema_version(
        ...
        sqlx::query("INSERT OR REPLACE INTO schema_version (version, applied_at) VALUES (?, CURRENT_TIMESTAMP)")
            .bind(version)
            .execute(&mut **tx)
            .await?;
        Ok(())
    ...
    async fn apply_migration(tx: &mut sqlx::Transaction<'_, Sqlite>, version: i32) -> Result<()> {
        match version {
            ...
            8 => {
                sqlx::query(
                    r#"
                    ALTER TABLE sessions ADD COLUMN rook_mode TEXT NOT NULL DEFAULT 'auto'
                "#,
                )
                .execute(&mut **tx)
                .await?;
    ...
    async fn create_session(
        ...
        let session = sqlx::query_as(
            r#"
                INSERT INTO sessions (id, name, user_set_name, session_type, working_dir, extension_data, rook_mode)
                VALUES (
                    ? || '_' || CAST(COALESCE((
    ...
    async fn get_session(&self, id: &str, include_messages: bool) -> Result<Session> {
        ...
        SELECT id, working_dir, name, description, user_set_name, session_type, created_at, updated_at, extension_data,
               ...
               accumulated_total_tokens, accumulated_input_tokens, accumulated_output_tokens,
               schedule_id, recipe_json, user_recipe_values_json,
               provider_name, model_config_json, rook_mode, thread_id
        FROM sessions
        WHERE id = ?
    "#,
        )
    ...
    async fn apply_update(&self, builder: SessionUpdateBuilder<'_>) -> Result<()> {
        ...
        add_update!(builder.rook_mode, "rook_mode");
        ...
        if let Some(rook_mode) = builder.rook_mode {
            q = q.bind(rook_mode.to_string());
        }
    ...
    async fn list_sessions_by_types(&self, types: Option<&[SessionType]>) -> Result<Vec<Session>> {
        ...
        let query = format!(
            ...
            SELECT s.id, s.working_dir, s.name, s.description, s.user_set_name, s.session_type, s.created_at, s.updated_at, s.extension_data,
                   ...
                   s.provider_name, s.model_config_json, s.rook_mode, s.thread_id,
    ...
    async fn copy_session(
        ...
        // Preserve provider, model config, and rook_mode from original session
        ...
        builder = builder.rook_mode(original_session.rook_mode);
...
mod tests {
    ...
    #[test_case(RookMode::Approve)]
    #[test_case(RookMode::SmartApprove)]
    #[test_case(RookMode::Chat)]
    #[tokio::test]
    async fn test_rook_mode_persists(mode: RookMode) {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        assert_eq!(reloaded.rook_mode, mode);
    }

    #[tokio::test]
    async fn test_rook_mode_update() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sm.update(&session.id)
            .rook_mode(RookMode::Approve)
            .apply()
            .await
            .unwrap();

        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.rook_mode, RookMode::Approve);
    }

    #[tokio::test]
    async fn test_rook_mode_malformed_defaults_to_auto() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sqlx::query("UPDATE sessions SET rook_mode = 'garbage' WHERE id = ?")
            .bind(&session.id)
            .execute(pool)
            .await
            .unwrap();
        ...
        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.rook_mode, RookMode::default());
    }
    ...
    #[tokio::test]
    async fn test_legacy_db_without_schema_version_migrates_rook_mode() {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join(SESSIONS_FOLDER).join(DB_NAME);
        ...
        sqlx::query(
            "INSERT INTO sessions (id, working_dir, extension_data, provider_name, model_config_json)
             VALUES (?, ?, ?, ?, ?)",
        )
        .bind("legacy_id")
        .bind("/tmp")
        ...
        let reloaded = sm.storage().get_session("legacy_id", false).await.unwrap();
        assert_eq!(reloaded.rook_mode, RookMode::Auto);
    }
}

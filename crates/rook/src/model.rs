...
fn get_predefined_models() -> Vec<PredefinedModel> {
    static PREDEFINED_MODELS: Lazy<Vec<PredefinedModel>> =
        Lazy::new(|| match std::env::var("ROOK_PREDEFINED_MODELS") {
            Ok(json_str) => serde_json::from_str(&json_str).unwrap_or_else(|e| {
                tracing::warn!("Failed to parse ROOK_PREDEFINED_MODELS: {}", e);
                Vec::new()
            }),
            Err(_) => Vec::new(),
        });
...
impl ModelConfig {
    ...
    fn new_base(model_name: String, context_env_var: Option<&str>) -> Result<Self, ConfigError> {
        ...
        // then fall back to ROOK_CONTEXT_LIMIT.  Using Config::global().get_param()
        // reads from both environment variables and config.yaml, so users can set
        // `ROOK_CONTEXT_LIMIT: 1000000` in config.yaml instead of exporting an
        // env var.  See #7839.
        let context_limit = if let Some(env_var) = context_env_var {
            ...
            match crate::config::Config::global().get_param::<usize>("ROOK_CONTEXT_LIMIT") {
                Ok(limit) => {
                    if limit == 0 {
                        return Err(ConfigError::InvalidRange(
                            "ROOK_CONTEXT_LIMIT".to_string(),
    ...
    fn parse_temperature() -> Result<Option<f32>, ConfigError> {
        if let Ok(val) = std::env::var("ROOK_TEMPERATURE") {
            let temp = val.parse::<f32>().map_err(|_| {
                ConfigError::InvalidValue(
                    "ROOK_TEMPERATURE".to_string(),
                    val.clone(),
    ...
    fn parse_max_tokens() -> Result<Option<i32>, ConfigError> {
        match crate::config::Config::global().get_param::<i32>("ROOK_MAX_TOKENS") {
            Ok(tokens) => {
                if tokens <= 0 {
                    return Err(ConfigError::InvalidRange(
                        "rook_max_tokens".to_string(),
                        "must be greater than 0".to_string(),
                    ));
                }
                Ok(Some(tokens))
    ...
    fn parse_toolshim() -> Result<bool, ConfigError> {
        if let Ok(val) = std::env::var("ROOK_TOOLSHIM") {
            match val.to_lowercase().as_str() {
                "1" | "true" | "yes" | "on" => Ok(true),
                "0" | "false" | "no" | "off" => Ok(false),
                _ => Err(ConfigError::InvalidValue(
                    "ROOK_TOOLSHIM".to_string(),
                    val,
    ...
    fn parse_toolshim_model() -> Result<Option<String>, ConfigError> {
        match std::env::var("ROOK_TOOLSHIM_OLLAMA_MODEL") {
            Ok(val) if val.trim().is_empty() => Err(ConfigError::InvalidValue(
                "ROOK_TOOLSHIM_OLLAMA_MODEL".to_string(),
                val,
    ...
    pub fn is_openai_reasoning_model(&self) -> bool {
        const DATABRICKS_MODEL_NAME_PREFIXES: &[&str] = &["rook-", "databricks-"];
...
mod tests {
    ...
    fn test_parse_max_tokens_valid() {
        let _guard = env_lock::lock_env([("ROOK_MAX_TOKENS", Some("4096"))]);
    ...
    fn test_parse_max_tokens_not_set() {
        let _guard = env_lock::lock_env([("ROOK_MAX_TOKENS", None::<&str>)]);
        let result = ModelConfig::parse_max_tokens().unwrap();
    ...
    fn test_parse_max_tokens_invalid_string() {
        let _guard = env_lock::lock_env([("ROOK_MAX_TOKENS", Some("not_a_number"))]);
    ...
    fn test_parse_max_tokens_zero() {
        let _guard = env_lock::lock_env([("ROOK_MAX_TOKENS", Some("0"))]);
        let result = ModelConfig::parse_max_tokens();
        assert!(result.is_err());
    ...
    fn test_parse_max_tokens_negative() {
        let _guard = env_lock::lock_env([("ROOK_MAX_TOKENS", Some("-100"))]);
        let result = ModelConfig::parse_max_tokens();
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), ConfigError::InvalidRange(..)));
    }
    ...
    fn test_model_config_with_max_tokens_env() {
        let _guard = env_lock::lock_env([
            ("ROOK_MAX_TOKENS", Some("8192")),
            ("ROOK_TEMPERATURE", None::<&str>),
            ("ROOK_CONTEXT_LIMIT", None::<&str>),
            ("ROOK_TOOLSHIM", None::<&str>),
            ("ROOK_TOOLSHIM_OLLAMA_MODEL", None::<&str>),
        ]);
    ...
    mod is_openai_reasoning_model {
        ...
        const ENV_LOCK_KEYS: [(&str, Option<&str>); 5] = [
            ("ROOK_MAX_TOKENS", None),
            ("ROOK_TEMPERATURE", None),
            ("ROOK_CONTEXT_LIMIT", None),
            ("ROOK_TOOLSHIM", None),
            ("ROOK_TOOLSHIM_OLLAMA_MODEL", None),
        ];
        ...
        #[test]
        fn rook_prefixed_reasoning_models() {
            let _guard = env_lock::lock_env(ENV_LOCK_KEYS);
            assert!(ModelConfig::new_or_fail("rook-o3-mini").is_openai_reasoning_model());
            assert!(ModelConfig::new_or_fail("rook-o4-mini").is_openai_reasoning_model());
            assert!(ModelConfig::new_or_fail("rook-gpt-5").is_openai_reasoning_model());
        }
        ...
        fn non_reasoning_models() {
            ...
            assert!(!ModelConfig::new_or_fail("rook-claude-sonnet-4").is_openai_reasoning_model());

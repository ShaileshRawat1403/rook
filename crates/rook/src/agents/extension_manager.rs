...
use crate::agents::mcp_client::{McpClient, McpClientTrait, RookMcpClientCapabilities};
...
fn resolve_timeout(timeout: Option<u64>) -> u64 {
    timeout.unwrap_or_else(|| {
        Config::global()
            .get_rook_default_extension_timeout()
...
/// Manages rook extensions / MCP clients and their interactions
...
const TOOL_EXTENSION_META_KEY: &str = "rook_extension";
...
async fn child_process_client(
    ...
    client_name: String,
    capabilities: RookMcpClientCapabilities,
...
const ROOK_USER_AGENT: reqwest::header::HeaderValue =
    reqwest::header::HeaderValue::from_static(concat!("rook/", env!("CARGO_PKG_VERSION")));
...
async fn create_streamable_http_client(
    ...
    default_headers.insert(reqwest::header::USER_AGENT, ROOK_USER_AGENT);
    ...
    if should_attempt_oauth_fallback(&client_res) {
        match oauth_flow(&uri.to_string(), &name.to_string()).await {
            Ok(auth_manager) => {
                ...
                auth_headers.insert(reqwest::header::USER_AGENT, ROOK_USER_AGENT);
...
async fn create_unix_socket_http_client(
    ...
    custom_headers.insert(
        ...
        ROOK_USER_AGENT
            ...
            .unwrap_or("rook")
            .parse()
            .unwrap_or_else(|_| HeaderValue::from_static("rook")),
    );
...
impl ExtensionManager {
    ...
    pub async fn add_extension(
        ...
        let effective_working_dir = working_dir
            .clone()
            .or_else(|| std::env::var("ROOK_WORKING_DIR").ok().map(PathBuf::from))
        ...
        let client: Box<dyn McpClientTrait> = match &config {
            ...
            ExtensionConfig::StreamableHttp {
                ...
                let capability = RookMcpClientCapabilities {
            ...
            | ExtensionConfig::Platform { ref name, .. } => {
                ...
                if let Some(def) = PLATFORM_EXTENSIONS.get(normalized_name.as_str()) {
                    ...
                    if let Some(container) = container {
                        ...
                        let capabilities = RookMcpClientCapabilities {
                            mcpui: self.capabilities.mcpui,
                        };

...
use crate::config::{get_enabled_extensions, Config, RookMode};
...
const COMPACTION_THINKING_TEXT: &str = "rook is compacting the conversation...";
...
#[derive(Clone, Debug)]
pub enum RookPlatform {
    RookDesktop,
    RookCli,
}

impl fmt::Display for RookPlatform {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            RookPlatform::RookCli => write!(f, "rook-cli"),
            RookPlatform::RookDesktop => write!(f, "rook-desktop"),
        }
    }
}
...
pub struct AgentConfig {
    ...
    pub rook_platform: RookPlatform,
}
...
pub struct Agent {
    ...
    pub(super) current_rook_mode: Mutex<RookMode>,
...
pub enum AgentEvent {
    ...
    RunStateChanged(super::RunState),
}
...
impl Agent {
    pub fn new() -> Self {
        ...
        Self::with_config(AgentConfig::new(
            ...
            None,
            config.get_rook_mode().unwrap_or_default(),
            config.get_rook_disable_session_naming().unwrap_or(false),
    ...
    pub fn with_config(config: AgentConfig) -> Self {
        ...
        let rook_platform = config.rook_platform.clone();
        let initial_mode = config.rook_mode;
        let capabilities = match config.rook_platform {
            RookPlatform::RookDesktop => ExtensionManagerCapabilities { mcpui: true },
            RookPlatform::RookCli => ExtensionManagerCapabilities { mcpui: false },
        };
        ...
        Self {
            ...
            config,
            current_rook_mode: Mutex::new(initial_mode),
            extension_manager: Arc::new(ExtensionManager::new(
                provider.clone(),
                session_manager,
                rook_platform.to_string(),
                capabilities,
            )),
    ...
    fn create_tool_inspection_manager(
        ...
        // Add adversary inspector (LLM-based review, enabled by ~/.config/rook/adversary.md)
    ...
    async fn prepare_reply_context(
        ...
        let rook_mode = *self.current_rook_mode.lock().await;

        if rook_mode == RookMode::SmartApprove {
            self.tool_inspection_manager.apply_tool_annotations(&tools);
        }

        let tool_call_cut_off = match Config::global().get_param::<usize>("ROOK_TOOL_CALL_CUTOFF") {
            Ok(v) => v,
            Err(_) => {
                ...
                let compaction_threshold = Config::global()
                    .get_param::<f64>("ROOK_AUTO_COMPACT_THRESHOLD")
    ...
    async fn reply_internal(
        ...
        let reply_stream_span = tracing::info_span!(target: "rook::agents::agent", "reply_stream", session.id = %session_config.id);
        let inner = Box::pin(async_stream::try_stream! {
            let mut turns_taken = 0u32;
            let max_turns = session_config.max_turns.unwrap_or_else(|| {
                Config::global()
                    .get_param::<u32>("ROOK_MAX_TURNS")
            ...
            loop {
                ...
                yield AgentEvent::RunStateChanged(crate::agents::RunState::Understanding);
                    ...
                    match next {
                        Ok((response, usage)) => {
                            ...
                            if let Some(response) = response {
                                ...
                                yield AgentEvent::RunStateChanged(crate::agents::RunState::Acting);
                                ...
                                for request in frontend_requests.iter().chain(remaining_requests.iter()) {
                                    ...
                                    } else if let Err(e) = &request.tool_call {
                                        error!(
            ...
            if !last_assistant_text.is_empty() {
                tracing::info!(target: "rook::agents::agent", trace_output = last_assistant_text.as_str());
            }
    ...
    pub async fn update_rook_mode(&self, mode: RookMode, session_id: &str) -> Result<()> {
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
        ...
        *self.current_rook_mode.lock().await = mode;
        self.config
            .session_manager
            .clone()
            .update(session_id)
            ...
            .apply()
            .await
            .context("Failed to persist rook_mode to session")
    }

    pub async fn rook_mode(&self) -> RookMode {
        *self.current_rook_mode.lock().await
    }
    ...
    pub async fn restore_provider_from_session(&self, session: &Session) -> Result<bool> {
        ...
        let provider_name = session
            .provider_name
            .clone()
            .or_else(|| config.get_rook_provider().ok())
        ...
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
                .update_mode(&session.id, session.rook_mode)
                .await
                .map_err(|e| anyhow!("Failed to propagate mode to provider: {}", e))?;
        }
        *self.current_rook_mode.lock().await = session.rook_mode;
        Ok(provider_changed)
    }
    ...
    pub async fn create_recipe(
        ...
        let settings = Settings {
            rook_provider: Some(provider_name.clone()),
            rook_model: Some(model_name.clone()),
...
mod tests {
    ...
    async fn test_add_final_output_tool() -> Result<()> {
        ...
        let system_prompt = prompt_manager
            .builder()
            .with_rook_mode(RookMode::default())
            .build();

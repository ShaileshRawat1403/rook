...
pub fn manage_schedule_tool() -> Tool {
    Tool::new(
        ...
        indoc! {r#"
            Manage rook's internal scheduled recipe execution.

            Actions:
            - "list": List all rook scheduled jobs
            - "create": Create a new rook scheduled job from a recipe file
            - "run_now": Execute a rook scheduled job immediately
            - "pause": Pause a rook scheduled job
            - "unpause": Resume a paused rook scheduled job
            - "delete": Remove a rook scheduled job
            - "kill": Terminate a currently running rook scheduled job
            - "inspect": Get details about a running rook scheduled job
            - "sessions": List execution history for a rook scheduled job

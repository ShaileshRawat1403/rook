...
fn discover_filesystem_sources(working_dir: &Path) -> Vec<Source> {
    ...
    let local_recipe_dirs: Vec<PathBuf> = vec![
        ...
        working_dir.join(".rook/recipes"),
        working_dir.join(".agents/recipes"),
    ];

    let global_recipe_dirs: Vec<PathBuf> = std::env::var("ROOK_RECIPE_PATH")
        .ok()
        .into_iter()
        .flat_map(|p| {
...
fn max_background_tasks() -> usize {
    Config::global()
        .get_param::<usize>("ROOK_MAX_BACKGROUND_TASKS")
        .unwrap_or(5)
}
...
impl SummonClient {
        ...
        let schema = serde_json::json!({
            ...
            "properties": {
                ...
                "max_turns": {
                    "type": "integer",
                    "minimum": 1,
                    "description": "Maximum turns for this delegate. Overrides recipe settings.max_turns and ROOK_SUBAGENT_MAX_TURNS."
                },
                "async": {
                    "type": "boolean",
                    "default": false,
    ...
    async fn handle_delegate(
        ...
        let agent_config = AgentConfig::new(
            ...
            crate::agents::RookPlatform::RookCli,
        );
    ...
    fn build_recipe_from_agent(
        ...
        let settings = model.map(|m| Settings {
            rook_model: Some(m),
            rook_provider: params.provider.clone(),
    ...
    async fn resolve_provider(
        ...
        let provider_name = params
            ...
            .or_else(|| {
                recipe
                    .settings
                    .as_ref()
                    .and_then(|s| s.rook_provider.clone())
            })
            .or_else(|| {
                Config::global()
                    .get_param::<String>("ROOK_SUBAGENT_PROVIDER")
                    .ok()
            })
        ...
        } else if let Some(model) = recipe.settings.as_ref().and_then(|s| s.rook_model.as_ref()) {
            model_config.model_name = model.clone();
        } else if let Ok(model) = Config::global().get_param::<String>("ROOK_SUBAGENT_MODEL") {
    ...
    fn resolve_max_turns(&self, session: &crate::session::Session) -> usize {
        session
            ...
            .or_else(|| {
                std::env::var("ROOK_SUBAGENT_MAX_TURNS")
                    .ok()
                    .and_then(|v| v.parse().ok())
            })
            .or_else(|| {
                Config::global()
                    .get_param::<usize>("ROOK_SUBAGENT_MAX_TURNS")
                    .ok()
            })
...
You review code."#;
    ...
    async fn test_discover_recipes_and_agents() {
        ...
        let recipes = temp_dir.path().join(".rook/recipes");
        ...
        let agents = temp_dir.path().join(".rook/agents");
    ...
    async fn test_recipe_deduplication_local_wins() {
        ...
        let local = temp_dir.path().join(".rook/recipes");
    ...
    fn test_resolve_max_turns_recipe_overrides_env_var() {
        ...
        std::env::set_var("ROOK_SUBAGENT_MAX_TURNS", "99");
        let result = client.resolve_max_turns(&session);
        std::env::remove_var("ROOK_SUBAGENT_MAX_TURNS");
    ...
    fn test_resolve_max_turns_falls_back_to_env_var() {
        ...
        std::env::set_var("ROOK_SUBAGENT_MAX_TURNS", "7");
        let result = client.resolve_max_turns(&session);
        std::env::remove_var("ROOK_SUBAGENT_MAX_TURNS");

        assert_eq!(
            result, 7,
            "should fall back to ROOK_SUBAGENT_MAX_TURNS env var"
        );
    }

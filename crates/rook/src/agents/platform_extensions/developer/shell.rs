...
/// Resolve the preferred Unix shell, respecting ROOK_SHELL.
///
/// Returns `(shell_path, is_user_configured)` — the boolean is true when
/// `ROOK_SHELL` was explicitly set, which matters in Flatpak mode: an
...
///
/// In Flatpak mode without an explicit ROOK_SHELL, we skip sandbox
...
///
/// If the user explicitly configured ROOK_SHELL, honour the full path
...
fn build_shell_command(
    ...
    #[cfg(windows)]
    let mut command = {
        let shell = std::env::var("ROOK_SHELL").unwrap_or_else(|_| "cmd".to_string());
    ...
    let mut command = {
        ...
        if is_flatpak() {
            ...
            // If ROOK_SHELL was explicitly set, honour the full path (likely a host

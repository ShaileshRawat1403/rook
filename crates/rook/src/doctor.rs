...
async fn ensure_working_provider(
    ...
    let provider_name = config.get_rook_provider().ok();
    let model_name = config.get_rook_model().ok();
    ...
    Ok(Some(Message::assistant().with_text(format!(
        "**Rook Doctor**\n\n{}\n\n\
         No working provider found. Run `rook configure` to set one up.",
        preamble,
    ))))
}
...
async fn save_and_set(
    ...
    let config = Config::global();
    config.set_rook_provider(provider.get_name()).ok();
    config
        .set_rook_model(&provider.get_model_config().model_name)
        .ok();
...
fn describe_error(e: &ProviderError) -> String {
    match e {
        ProviderError::Authentication(_) => {
            "Authentication failed — check your API key. Run `rook configure` to update it."
                .to_string()
        }

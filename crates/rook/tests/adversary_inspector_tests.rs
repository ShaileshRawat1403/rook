...
use rook::conversation::message::{Message, MessageContent, ToolRequest};
use rook::security::adversary_inspector::AdversaryInspector;
use rook::tool_inspection::ToolInspector;
use std::sync::Arc;
...
async fn test_adversary_enabled_default_tools() {
    ...
    assert!(matches!(
        results[0].action,
        rook::tool_inspection::InspectionAction::Allow
    ));

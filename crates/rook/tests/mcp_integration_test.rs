...
use rook::agents::extension_manager::{ExtensionManager, ExtensionManagerCapabilities};
use rook::agents::RookPlatform;
...
async fn test_replayed_session(
    ...
    let mode = if env::var("ROOK_RECORD_MCP").is_ok() {
        TestMode::Record
    } else {
    ...
    let session_manager = Arc::new(rook::session::SessionManager::new(
        temp_dir.path().to_path_buf(),
    ));
    let extension_manager = Arc::new(ExtensionManager::new(
        provider,
        session_manager,
        RookPlatform::RookDesktop.to_string(),

...
use rook::acp::ACP_CURRENT_MODEL;
use rook::agents::{Agent, AgentConfig, AgentEvent, PromptManager, RookPlatform, SessionConfig};
use rook::config::{ExtensionConfig, PermissionManager, RookMode};
...
use rook::providers::anthropic::ANTHROPIC_DEFAULT_MODEL;
use rook::providers::azure::AZURE_DEFAULT_MODEL;
use rook::providers::base::Provider;
#[cfg(feature = "aws-providers")]
use rook::providers::bedrock::BEDROCK_DEFAULT_MODEL;
use rook::providers::claude_code::CLAUDE_CODE_DEFAULT_MODEL;
use rook::providers::codex::CODEX_DEFAULT_MODEL;
use rook::providers::create_with_named_model;
use rook::providers::databricks::DATABRICKS_DEFAULT_MODEL;
use rook::providers::errors::ProviderError;
use rook::providers::google::GOOGLE_DEFAULT_MODEL;
use rook::providers::litellm::LITELLM_DEFAULT_MODEL;
use rook::providers::openai::OPEN_AI_DEFAULT_MODEL;
#[cfg(feature = "aws-providers")]
use rook::providers::sagemaker_tgi::SAGEMAKER_TGI_DEFAULT_MODEL;
use rook::providers::snowflake::SNOWFLAKE_DEFAULT_MODEL;
use rook::providers::xai::XAI_DEFAULT_MODEL;
use rook::session::{SessionManager, SessionType};
use rook_test_support::{
...
impl ProviderFixture {
    async fn setup(config: &ProviderTestConfig, mode: RookMode) -> Result<Self> {
        let mut env_vars: Vec<(&'static str, Option<&str>)> =
            vec![("ROOK_MODE", Some(<&str>::from(mode)))];
    ...
    async fn tool_roundtrip(
        &self,
        prompt: &str,
        model_config: Option<rook::model::ModelConfig>,
    ) -> Result<Message> {
        let tools = self
            .agent
            .extension_manager
        ...
        let ctx = rook::agents::ToolCallContext::new(
    ...
    async fn test_image_content_support(&self) -> Result<()> {
        let image_config = match &self.image_model {
            Some(model) => {
                Some(rook::model::ModelConfig::new(model)?.with_canonical_limits(&self.name))
            }
            None => None,
        };
        let response = self
        ...
        assert!(
            text.contains("hello rook") || text.contains("test image"),
            "{text}"
        );
    ...
    async fn test_model_switch(&self) -> Result<()> {
        ...
        let alt_config = rook::model::ModelConfig::new(alt)?.with_canonical_limits(&self.name);
    ...
    async fn test_mode_update(&self) -> Result<()> {
        ...
        self.agent
            .update_rook_mode(RookMode::Approve, &self.session_id)
            .await?;
...
async fn test_provider(config: ProviderTestConfig) -> Result<()> {
    ...
    let run_test = |mode: RookMode| ProviderFixture::setup(&config, mode);

    if run_test(RookMode::Auto).await.is_err() {
    ...
    let result: Result<()> = async {
        run_test(RookMode::Auto).await?.test_model_listing().await?;
        run_test(RookMode::Auto)
            .await?
            .test_basic_response()
            .await?;
        if config.test_mcp_tools {
            run_test(RookMode::Auto).await?.test_tool_usage().await?;
            run_test(RookMode::Auto)
                .await?
        ...
        if config.model_switch_name.is_some() {
            run_test(RookMode::Auto).await?.test_model_switch().await?;
        }
        ...
        if config.test_permissions {
            ...
            if config.test_smart_approve {
                run_test(RookMode::SmartApprove)
                    .await?
        ...
        if config.test_mode_update {
            run_test(RookMode::Auto).await?.test_mode_update().await?;
        }
        Ok(())
    }
    .await;

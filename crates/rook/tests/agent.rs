...
use rook::agents::{Agent, AgentEvent, RookPlatform};
use rook::config::extensions::{set_extension, ExtensionEntry};
...
mod tests {
    ...
    mod schedule_tool_tests {
        ...
        use rook::agents::platform_tools::PLATFORM_MANAGE_SCHEDULE_TOOL_NAME;
        use rook::agents::AgentConfig;
        use rook::config::permission::PermissionManager;
        use rook::config::RookMode;
        use rook::scheduler::{ScheduledJob, SchedulerError};
        use rook::scheduler_trait::SchedulerTrait;
        use rook::session::{Session, SessionManager};
        ...
        async fn test_schedule_management_tool_list() {
            ...
            assert!(tool
                ...
                .clone()
                .unwrap_or_default()
                .contains("Manage rook's internal scheduled recipe execution"));
        }
    ...
    mod retry_tests {
        use super::*;
        use rook::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use rook::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        ...
        use rmcp::object;
        use rook::agents::SessionConfig;
        use rook::config::RookMode;
        use rook::conversation::message::{Message, MessageContent};
        use rook::model::ModelConfig;
        use rook::providers::base::{
        ...
        impl ProviderDef for MockToolProvider {
            ...
            fn from_env(
                _model: ModelConfig,
                _extensions: Vec<rook::config::ExtensionConfig>,
            ...
            while let Some(response_result) = reply_stream.next().await {
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                        ...
                        {
                            if let rook::conversation::message::ActionRequiredData::ToolConfirmation { id, .. } = &action.data {
                                agent.handle_confirmation(
                                    id.clone(),
                                    rook::permission::PermissionConfirmation {
                    ...
                    Ok(AgentEvent::RunStateChanged(_)) => {}
    ...
    mod tool_pair_summarization_tests {
        ...
        async fn test_batch_summarization_preserves_all_summaries() -> Result<()> {
            ...
            Config::global()
                .set_param("ROOK_TOOL_CALL_CUTOFF", 2)
                .unwrap();
            ...
            Config::global().delete("ROOK_TOOL_CALL_CUTOFF").unwrap();
    ...
    mod extension_manager_tests {
        use super::*;
        use rook::agents::extension::ExtensionConfig;
        use rook::agents::platform_extensions::{
        ...
        async fn setup_agent_with_extension_manager() -> (Agent, String) {
            ...
            let todo_extension_entry = ExtensionEntry {
                ...
                config: ExtensionConfig::Platform {
                    name: "todo".to_string(),
                    description:
                        "Enable a todo list for rook so it can keep track of what it is doing"
                            .to_string(),
    ...
    mod streaming_persistence_tests {
        ...
        use rmcp::object;
        use rook::agents::{AgentConfig, SessionConfig};
        use rook::config::permission::PermissionManager;
        use rook::config::RookMode;
        use rook::conversation::message::Message;
        use rook::model::ModelConfig;

...
fn asset_name() -> &'static str {
    ...
    {
        "rook-aarch64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
    {
        "rook-x86_64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64"))]
    {
        "rook-x86_64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64"))]
    {
        "rook-aarch64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64"))]
    {
        "rook-x86_64-pc-windows-msvc.zip"
    }
}
...
async fn fetch_attestations(digest: &str, token: Option<&str>) -> Result<Vec<serde_json::Value>> {
    let url = format!(
        "https://api.github.com/repos/ShaileshRawat1403/rook/attestations/sha256:{digest}\
...
/// Update the rook binary to the latest release.
///
...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    ...
    #[cfg(not(feature = "disable-update"))]
    {
        ...
        let url = format!("https://github.com/ShaileshRawat1403/rook/releases/download/{tag}/{asset}");
        ...
        if provenance_verified {
            println!("rook updated successfully (verified with Sigstore SLSA provenance).");
        } else {
            println!("rook updated successfully.");
        }

        // --- Reconfigure if requested -------------------------------------------
        if reconfigure {
            println!("Running rook configure...");
            let status = Command::new(current_exe)
                .arg("configure")
                .status()
                .context("Failed to run rook configure")?;
            if !status.success() {
                eprintln!("Warning: rook configure exited with {status}");
            }
        }
...
///   1. A `rook-package/` subdirectory (Windows releases)
...
fn find_binary(extract_dir: &Path, binary_name: &str) -> Option<PathBuf> {
    // 1. Check rook-package subdir (matches download_cli.sh / download_cli.ps1)
    let package_dir = extract_dir.join("rook-package");
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if old_exe.exists() {
            fs::remove_file(&old_exe).with_context(|| {
                format!(
                    "Failed to remove old backup {}. Is another rook process running?",
                    old_exe.display()
                )
            })?;
        }
        ...
        fs::rename(current_exe, &old_exe).with_context(|| {
            format!(
                "Failed to rename running binary to {}. Try closing Rook Desktop if it's open.",
                old_exe.display()
            )
        })?;
...
mod tests {
    ...
    fn test_find_binary_in_package_subdir() {
        ...
        let pkg = tmp.path().join("rook-package");
    ...
    fn test_replace_binary_basic() {
        let tmp = tempdir().unwrap();
        let new_bin = tmp.path().join("new_rook");
        let current = tmp.path().join("current_rook");
    ...
    fn test_replace_binary_windows_rename_away() {
        let tmp = tempdir().unwrap();
        let current = tmp.path().join("rook.exe");
        let new_bin = tmp.path().join("new_rook.exe");
    ...
    fn test_extract_zip_with_package_dir() {
        ...
        // Create a zip in memory with rook-package/ structure
        let mut buf = Vec::new();
        {
            ...
            writer.add_directory("rook-package/", options).unwrap();
            writer.start_file("rook-package/rook.exe", options).unwrap();
            writer.write_all(b"fake rook binary").unwrap();
            writer
                .start_file("rook-package/libtest.dll", options)
                .unwrap();
        ...
        let binary = find_binary(tmp.path(), "rook.exe");
        assert!(binary.is_some());

        let content = fs::read_to_string(binary.unwrap()).unwrap();
        assert_eq!(content, "fake rook binary");

        // DLL should be in rook-package too
        assert!(tmp.path().join("rook-package/libtest.dll").exists());
    }
    ...
    fn test_validate_entry_path_accepts_safe_paths() {
        assert!(validate_entry_path(Path::new("rook")).is_ok());
        assert!(validate_entry_path(Path::new("rook-package/rook")).is_ok());
    ...
    fn test_extract_tar_bz2_safe_archive() {
        ...
        let mut builder_buf = Vec::new();
        {
            ...
            let data = b"rook binary content";
            let mut header = tar::Header::new_gnu();
            header.set_size(data.len() as u64);
            header.set_mode(0o755);
            header.set_cksum();
            builder
                .append_data(&mut header, "rook-package/rook", &data[..])
                .unwrap();
        ...
        let extracted = tmp.path().join("rook-package/rook");

use anyhow::Result;
use rook::execution::manager::AgentManager;
use rook::gateway::manager::GatewayManager;
use std::sync::Arc;
...
pub async fn handle_gateway_start(
    ...
    let mut config = rook::gateway::GatewayConfig {
        gateway_type,
        platform_config,
        max_sessions: 0,
    };

    let gw = rook::gateway::create_gateway(&mut config)?;

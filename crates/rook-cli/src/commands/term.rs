...
use rook::conversation::message::{Message, MessageContent, MessageMetadata};
...
alias @rook='{rook_bin} term run'
alias @r='{rook_bin} term run'

rook_preexec() {
    [[ "$1" =~ ^rook\ term ]] && return
    [[ "$1" =~ ^(@rook|@r)($|[[:space:]]) ]] && return
    ('{rook_bin}' term log "$1" &) 2>/dev/null
}

if [[ -z "$rook_preexec_installed" ]]; then
    rook_preexec_installed=1
    trap 'rook_preexec "$BASH_COMMAND"' DEBUG
fi{command_not_found_handler}"#,
    command_not_found: Some(
        r#"

command_not_found_handle() {
    echo "Rook: command '$1' not found. Asking Rook..."
    '{rook_bin}' term run "$@"
    return 0
}"#,
    ),
...
add-zsh-hook preexec rook_preexec{command_not_found_handler}"#,
    command_not_found: Some(
        r#"
...
function @rook; {rook_bin} term run $argv; end
function @r; {rook_bin} term run $argv; end

function rook_preexec --on-event fish_preexec
    string match -q -r '^rook term' -- $argv[1]; and return
    string match -q -r '^(@rook|@r)($|\s)' -- $argv[1]; and return
    {rook_bin} term log "$argv[1]" 2>/dev/null &
end"#,
...
function @rook {{ & '{rook_bin}' term run @args }}
function @r {{ & '{rook_bin}' term run @args }}

Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    $line = $null
    [Microsoft.PowerShell.PSConsoleReadLine]::GetBufferState([ref]$line, [ref]$null)
    if ($line -notmatch '^rook term' -and $line -notmatch '^(@rook|@r)($|\s)') {{
        Start-Job -ScriptBlock {{ & '{rook_bin}' term log $using:line }} | Out-Null
    }}
...
pub async fn handle_term_init(
    ...
    let session = match named_session {
        ...
        None => {
            let session = session_manager
                .create_session(
                    working_dir,
                    "Rook Term Session".to_string(),
                    SessionType::Terminal,
                    Config::global().get_rook_mode().unwrap_or_default(),
                )
                .await?;
    ...
    let rook_bin = std::env::current_exe()
        .map(|p| p.to_string_lossy().into_owned())
        .unwrap_or_else(|_| "rook".to_string());

    let command_not_found_handler = if with_command_not_found {
        config
            .command_not_found
            .map(|s| s.replace("{rook_bin}", &rook_bin))
            .unwrap_or_default()
    } else {
        String::new()
    };
...
pub async fn handle_term_log(command: String) -> Result<()> {
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!("AGENT_SESSION_ID not set. Run 'eval \"$(rook term init <shell>)\"' first.")
    })?;
...
pub async fn handle_term_run(prompt: Vec<String>) -> Result<()> {
    ...
    let mut session = build_session(config, None).await;
    session.headless(prompt_with_context).await?;

    Ok(())
}

/// Handle `rook term info` - print compact session info for prompt integration
pub async fn handle_term_info() -> Result<()> {
    ...
    let model_name = config
        ...
        .map(|name| {
            ...
            if let Some(stripped) = short.strip_prefix("rook-") {
                stripped.to_string()
            } else {
                short.to_string()
            }
    ...
    let context_limit = config
        ...
        .ok()
        .and_then(|model_name| {
            config.get_rook_provider().ok().and_then(|provider_name| {

use crate::recipes::github_recipe::ROOK_RECIPE_GITHUB_REPO_CONFIG_KEY;
use cliclack::spinner;
use console::style;
use rook::agents::extension::{ToolInfo, PLATFORM_EXTENSIONS};
use rook::agents::extension_manager::get_parameter_names;
use rook::agents::Agent;
use rook::agents::{extension::Envs, ExtensionConfig};
use rook::config::declarative_providers::{
    create_custom_provider, remove_custom_provider, CreateCustomProviderParams,
};
use rook::config::extensions::{
    get_all_extension_names, get_all_extensions, get_enabled_extensions, get_extension_by_name,
    name_to_key, remove_extension, set_extension, set_extension_enabled,
};
use rook::config::paths::Paths;
use rook::config::permission::PermissionLevel;
use rook::config::signup_tetrate::TetrateAuth;
use rook::config::{
...
use rook::posthog::{get_telemetry_choice, TELEMETRY_ENABLED_KEY};
use rook::providers::base::ConfigKey;
use rook::providers::chatgpt_codex::reasoning_levels_for_model;
use rook::providers::formats::anthropic::supports_adaptive_thinking;
use rook::providers::provider_test::test_provider_configuration;
use rook::providers::{create, providers, retry_operation, RetryConfig};
use rook::session::SessionType;
...
pub fn configure_telemetry_consent_dialog() -> anyhow::Result<bool> {
    ...
    println!();
    println!("{}", style("Help improve rook").bold());
    println!();
    println!(
        "{}",
        style("Would you like to help improve rook by sharing anonymous usage data?").dim()
    );
    println!(
        "{}",
        style("This helps us understand how rook is used and identify areas for improvement.")
            .dim()
    );
    println!();
    ...
    println!("{}", style("  • rook version and install method").dim());
    ...
    println!(
        "{}",
        style("or any personal data. You can change this anytime with 'rook configure'.").dim()
    );
    println!();

    let enabled = cliclack::confirm("Share anonymous usage data to help improve rook?")
        .initial_value(true)
        .interact()?;
    ...
    if enabled {
        let _ = cliclack::log::success("Thank you for helping improve rook!");
    } else {
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    println!();
    println!("{}", style("Welcome to rook! Let's get you set up.").dim());
    println!(
        "{}",
    ...
    println!();
    cliclack::intro(style(" rook-configure ").on_cyan().black())?;
...
async fn handle_existing_config() -> anyhow::Result<()> {
    ...
    let action = cliclack::select("What would you like to configure?")
        ...
        .item(
            "settings",
            "rook settings",
            "Set the rook mode, Tool Output, Tool Permissions, Experiment, rook recipe github repo and more",
        )
        .interact()?;
    ...
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            cliclack::outro(style(format!(
                "Failed to store {} securely: {}. Please ensure your system's secure storage is accessible. Alternatively you can run with ROOK_DISABLE_KEYRING=true or set the key in your environment variables",
                key_name, e
            )).on_red().white())?;
            Ok(false)
        }
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let current_provider: Option<String> = config.get_rook_provider().ok();
    ...
    let model: String = match models_res {
        ...
        Ok(_) => {
            let default_model =
                std::env::var("ROOK_MODEL").unwrap_or(provider_meta.default_model.clone());
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&rook::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with rook",
        )
        .item(
            "stdio",
...
pub async fn configure_settings_dialog() -> anyhow::Result<()> {
    ...
    let setting_type = setting_select
        ...
        .item(
            ...
            "rook will pull recipes from this repo if not found locally.",
        )
        .interact()?;
...
pub fn configure_rook_mode_dialog() -> anyhow::Result<()> {
    let config = Config::global();

    if std::env::var("ROOK_MODE").is_ok() {
        let _ = cliclack::log::info(
            "Notice: ROOK_MODE environment variable is set and will override the configuration here.",
        );
    }

    let mode = cliclack::select("Which rook mode would you like to configure?")
        .item(
            RookMode::Auto,
            "Auto Mode",
    ...
    config.set_rook_mode(mode)?;
    let msg = match mode {
        RookMode::Auto => "Set to Auto Mode - full file modification enabled",
        RookMode::Approve => "Set to Approve Mode - all tools and modifications require approval",
        RookMode::SmartApprove => "Set to Smart Approve Mode - modifications require approval",
        RookMode::Chat => "Set to Chat Mode - no tools or modifications enabled",
    };
    cliclack::outro(msg)?;
    Ok(())
}
...
pub fn configure_telemetry_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("ROOK_TELEMETRY_OFF").is_ok() {
        let _ = cliclack::log::info(
            "Notice: ROOK_TELEMETRY_OFF environment variable is set and will override the configuration here.",
        );
    }
    ...
    if enabled {
        cliclack::outro("Telemetry enabled - thank you for helping improve rook!")?;
    } else {
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("ROOK_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info(
            "Notice: ROOK_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.",
        );
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("ROOK_CLI_MIN_PRIORITY", 0.8)?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("ROOK_CLI_MIN_PRIORITY", 0.2)?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("ROOK_CLI_MIN_PRIORITY", 0.0)?;
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("ROOK_DISABLE_KEYRING").is_ok() {
        let _ = cliclack::log::info(
            "Notice: ROOK_DISABLE_KEYRING environment variable is set and will override the configuration here.",
        );
    }

    let currently_disabled = config.get_param::<String>("ROOK_DISABLE_KEYRING").is_ok();
    ...
    match storage_option {
        "keyring" => {
            ...
            config.set_param("ROOK_DISABLE_KEYRING", Value::String("".to_string()))?;
            cliclack::outro("Secret storage set to system keyring (secure)")?;
            let _ =
                cliclack::log::info("You may need to restart rook for this change to take effect");
        }
        "file" => {
            // Set the disable flag to use file storage
            config.set_param("ROOK_DISABLE_KEYRING", Value::String("true".to_string()))?;
            cliclack::outro(format!(
...
/// Configure experiment features that can be used with rook
...
fn configure_recipe_dialog() -> anyhow::Result<()> {
    let key_name = ROOK_RECIPE_GITHUB_REPO_CONFIG_KEY;
    let config = Config::global();
    let default_recipe_repo = std::env::var(key_name)
        .ok()
        .or_else(|| config.get_param(key_name).unwrap_or(None));
    let mut recipe_repo_input =
        cliclack::input("Enter your rook recipe GitHub repo (owner/repo): eg: my_org/rook-recipes")
            .required(false);
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    let current_max_turns: u32 = config.get_param("ROOK_MAX_TURNS").unwrap_or(1000);
    ...
    config.set_param("ROOK_MAX_TURNS", max_turns)?;

    cliclack::outro(format!(
        "Set maximum turns to {} - rook will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use rook::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    use rook::conversation::message::Message;
    use rook::providers::create;
    ...
    let configured_model: String = config.get_rook_model()?;
    let model_config = match rook::model::ModelConfig::new(&configured_model) {
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Platform {
                                ...
                                display_name: Some(rook::config::DEFAULT_DISPLAY_NAME.to_string()),
                                bundled: Some(true),
                                available_tools: Vec::new(),
                            },
                        });
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use rook.")?;
                }
                Err(e) => {
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro(
                        "Tetrate Agent Router Service setup complete! You can now use rook.",
                    )?;
                }
                Err(e) => {
                    eprintln!("⚠️  Configuration test failed: {}", e);
...
async fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = rook::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        rook::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
    ...
    if let Err(e) = rook::providers::cleanup_provider(selected_id).await {

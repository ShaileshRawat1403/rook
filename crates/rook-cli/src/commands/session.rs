...
use regex::Regex;
use rook::session::{generate_diagnostics, Session, SessionManager};
...
fn export_session_to_markdown(
    messages: Vec<rook::conversation::message::Message>,
    session_name: &String,
) -> String {
    ...
    for message in &messages {
        ...
        let is_only_tool_response = message.role == rmcp::model::Role::User
            && message.content.iter().all(|content| {
                matches!(
                    content,
                    rook::conversation::message::MessageContent::ToolResponse(_)
                )
            });
        ...
        if message.content.iter().any(|content| {
            matches!(
                content,
                rook::conversation::message::MessageContent::ToolRequest(_)
            )
        }) {

//! Generate manpages for the rook CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man rook`).
//!
//! When rook is packaged for Linux distributions (deb, rpm, etc.), the generated
...
//!
//! Output: target/man/rook.1, target/man/rook-session.1, etc.
...
fn generate_see_also(
    ...
    // Always reference the main rook command if we're not it
    if current_name != "rook" {
        references.push("rook".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "rook" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "rook" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("rook-{}", subcmd_name);

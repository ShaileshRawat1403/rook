...
#[cfg(feature = "otel")]
use rook::otel::otlp;
use rook::tracing::langfuse_layer;
...
fn default_env_filter() -> EnvFilter {
    EnvFilter::new("")
        // Keep rook and MCP logs visible without verbose debug payloads.
        .add_directive("mcp_client=info".parse().unwrap())
        .add_directive("rook=info".parse().unwrap())
        .add_directive("rook_cli=info".parse().unwrap())
...
fn setup_logging_internal(name: Option<&str>, force: bool) -> Result<()> {
    ...
    let mut setup = || {
        result = (|| {
            let log_dir = rook::logging::prepare_log_directory("cli", true)?;
            ...
            #[cfg(feature = "otel")]
            if !force {
                layers.extend(otlp::init_otlp_layers(rook::config::Config::global()));
            }
...
mod tests {
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = rook::logging::prepare_log_directory("cli", true).unwrap();
        assert!(log_dir.exists());
        assert!(log_dir.is_dir());

        // Verify directory structure
        let path_components: Vec<_> = log_dir.components().collect();
        assert!(path_components.iter().any(|c| c.as_os_str() == "rook"));
    ...
    fn test_default_filter_avoids_debug_by_default() {
        ...
        assert!(!filter.contains("rook=debug"));
        assert!(filter.contains("mcp_client=info"));
        assert!(filter.contains("rook=info"));
        assert!(filter.contains("rook_cli=info"));
    }
}

...
use anyhow::Result;
use rook::agents::{Agent, AgentConfig, RookPlatform};
use rook::config::permission::PermissionManager;
use rook::config::RookMode;
use rook::model::ModelConfig;
use rook::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("ROOK_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    rook::agents::moim::SKIP.with(|f| f.set(true));
    ...
    agent
        .update_provider(
            provider_arc as Arc<dyn rook::providers::base::Provider>,
            &session.id,
        )
        .await?;

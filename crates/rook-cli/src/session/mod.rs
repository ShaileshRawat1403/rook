...
use rook::conversation::Conversation;
use std::io::Write;
use std::str::FromStr;
...
use console::Color;
pub use output::{
    render_risk_badge, render_session_state, render_trust_summary, render_welcome_state,
};
use rook::agents::AgentEvent;
use rook::agents::SUBAGENT_TOOL_REQUEST_TYPE;
use rook::permission::permission_confirmation::PrincipalType;
use rook::permission::Permission;
use rook::permission::PermissionConfirmation;
use rook::providers::base::Provider;
use rook::utils::safe_truncate;

use anyhow::{Context, Result};
use completion::RookCompleter;
use input::InputResult;
use rmcp::model::PromptMessage;
use rmcp::model::ServerNotification;
use rmcp::model::{ErrorCode, ErrorData};
use rook::agents::extension::{Envs, ExtensionConfig, PLATFORM_EXTENSIONS};
use rook::agents::types::RetryConfig;
use rook::agents::{Agent, SessionConfig, COMPACT_TRIGGERS};
use rook::config::extensions::name_to_key;
...
impl HistoryManager {
    ...
    fn load(
        &self,
        editor: &mut rustyline::Editor<RookCompleter, rustyline::history::DefaultHistory>,
    ) {
...
pub struct CliSession {
    ...
    tui_tx: Option<tokio::sync::mpsc::Sender<crate::tui::state::TuiEvent>>,
}
...
impl CliSession {
    ...
    pub async fn handle_confirmation(
        ...
        self.agent
            .handle_confirmation(request_id, confirmation)
            .await;
    }

    pub async fn list_sessions(&self) -> Result<Vec<rook::session::session_manager::Session>> {
        self.agent.config.session_manager.list_sessions().await
    }
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: rook::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(rook::config::DEFAULT_EXTENSION_TIMEOUT),
            bundled: None,
    ...
    pub async fn add_streamable_http_extension(&mut self, extension_url: String) -> Result<()> {
        let config = Self::parse_streamable_http_extension(
            &extension_url,
            rook::config::DEFAULT_EXTENSION_TIMEOUT,
        );
        self.add_and_persist_extensions(vec![config]).await
    }
    ...
    fn create_editor(
        &self,
    ) -> Result<rustyline::Editor<RookCompleter, rustyline::history::DefaultHistory>> {
        let builder =
        ...
        let mut editor =
            rustyline::Editor::<RookCompleter, rustyline::history::DefaultHistory>::with_config(
                config,
            )?;
        let completer = RookCompleter::new(self.completion_cache.clone());
    ...
    async fn handle_input(
        ...
        match input {
            ...
            InputResult::RookMode(mode) => {
                history.save(editor);
                self.handle_rook_mode(&mode).await?;
            }
                    ...
                    None => {
                        output::render_error(
                            "No editor found. Set one with:\n  \
                                 rook configure set rook_prompt_editor \"vim\"\n  \
    ...
    async fn handle_rook_mode(&self, mode: &str) -> Result<()> {
        let config = Config::global();
        let mode = match RookMode::from_str(&mode.to_lowercase()) {
            Ok(mode) => mode,
            Err(_) => {
                output::render_error(&format!(
                    "Invalid mode '{mode}'. Mode must be one of: {}",
                    RookMode::VARIANTS.join(", ")
                ));
                return Ok(());
            }
        };
        self.agent.update_rook_mode(mode, &self.session_id).await?;
        config.set_rook_mode(mode)?;
        output::rook_mode_message(&format!("Rook mode set to '{mode}'"));
        Ok(())
    }
            ...
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    if curr_rook_mode != RookMode::Auto {
                        config.set_rook_mode(RookMode::Auto).unwrap();
                    }
    ...
    async fn process_agent_response(
        ...
        loop {
            tokio::select! {
                result = stream.next() => {
                    // Bridge to TUI if active
                    if let Some(tx) = &self.tui_tx {
                        if let Some(Ok(ref event)) = result {
                            let _ = tx.try_send(crate::tui::state::TuiEvent::Agent(event.clone()));
                        }
                    }
                    ...
                    match result {
                        Some(Ok(AgentEvent::Message(message))) => {
                            if let Some((id, security_prompt)) = find_tool_confirmation(&message) {
                                let permission = if interactive {
                                    ...
                                    if rook_mode == RookMode::Approve || rook_mode == RookMode::SmartApprove {
                                        cancel_token_clone.cancel();
                                        drop(stream);
                                        return Err(anyhow::anyhow!(
                                            "Tool approval required in non-interactive mode with RookMode::{rook_mode}. \
                                             This is an invalid configuration — Approve/SmartApprove modes require an \
                                             interactive terminal. Use RookMode::Auto for headless sessions."
                                        ));
                                    }
                                    tracing::warn!(
                        ...
                        Some(Ok(AgentEvent::RunStateChanged(_))) => {
                            // Ignored in main CLI for now
                        }
    ...
    pub async fn get_session(&self) -> Result<rook::session::Session> {
        self.agent
            .config
            .session_manager
            .get_session(&self.session_id, false)
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let show_cost = config
            .get_param::<bool>("ROOK_CLI_SHOW_COST")
            .unwrap_or(false);
...
fn prompt_tool_confirmation(security_prompt: &Option<String>) -> Result<Permission> {
    ...
    let prompt = if let Some(security_message) = security_prompt {
        ...
        "Rook would like to call the above tool, do you allow?".to_string()
    };
                ...
                let formatted = match notification_type {
                    ...
                    Some("response_generated") => {
                        ...
                        let min_priority = config
                            .get_param::<f32>("ROOK_CLI_MIN_PRIORITY")
                            .ok()
    ...
    for content in &message.content {
        if let MessageContent::ToolRequest(tool_request) = content {
            if let Ok(tool_call) = &tool_request.tool_call {
                tracing::info!(
                    monotonic_counter.rook.tool_calls = 1,
        ...
        if let MessageContent::ToolResponse(tool_response) = content {
            ...
            tracing::info!(
                monotonic_counter.rook.tool_completions = 1,
                tool_name = %tool_name,
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<rook::providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                rook::providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
async fn get_reasoner() -> Result<Arc<dyn Provider>, anyhow::Error> {
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("ROOK_PLANNER_PROVIDER") {
        provider
    } else {
        println!("WARNING: ROOK_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_rook_provider()
            .expect("No provider configured. Run 'rook configure' first")
    };

    // Try planner-specific model first, fall back to default model
    let model = if let Ok(model) = config.get_param::<String>("ROOK_PLANNER_MODEL") {
        model
    } else {
        println!("WARNING: ROOK_PLANNER_MODEL not found. Using default model...");
        config
            .get_rook_model()
            .expect("No model configured. Run 'rook configure' first")
    };

    let model_config =
        ModelConfig::new_with_context_env(model, &provider, Some("ROOK_PLANNER_CONTEXT_LIMIT"))?;
    let extensions = rook::config::extensions::get_enabled_extensions_with_config(config);
...
mod tests {
    use super::*;
    use rook::agents::extension::Envs;
    use rook::config::ExtensionConfig;

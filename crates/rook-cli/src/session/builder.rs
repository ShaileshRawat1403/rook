...
use console::style;
use rook::agents::{Agent, Container, ExtensionError};
use rook::config::resolve_extensions_for_new_session;
use rook::config::{get_all_extensions, Config, ExtensionConfig, RookMode};
use rook::providers::create;
use rook::recipe::Recipe;
use rook::session::session_manager::SessionType;
use rook::session::EnabledExtensionsState;
...
/// Configuration for building a new Rook session
///
...
async fn offer_extension_debugging_help(
    extension_name: &str,
    error_message: &str,
    provider: Arc<dyn rook::providers::base::Provider>,
    interactive: bool,
) -> Result<(), anyhow::Error> {
...
async fn load_extensions(
    ...
    provider_for_debug: Arc<dyn rook::providers::base::Provider>,
    interactive: bool,
    session_id: &str,
) -> Arc<Agent> {
...
fn resolve_provider_and_model(
    ...
    saved_model_config: Option<rook::model::ModelConfig>,
    ...
    let provider_name = session_config
        ...
        .clone()
        .or(saved_provider)
        .or_else(|| recipe_settings.and_then(|s| s.rook_provider.clone()))
        .or_else(|| config.get_rook_provider().ok())
        .unwrap_or_else(|| {
            output::render_error("No provider configured. Run 'rook configure' first.");
            process::exit(1);
        });
    ...
    let model_name = session_config
        ...
        .or_else(|| recipe_settings.and_then(|s| s.rook_model.clone()))
        .or_else(|| config.get_rook_model().ok())
        .unwrap_or_else(|| {
            output::render_error("No model configured. Run 'rook configure' first.");
            process::exit(1);
        });
            ...
            .is_some_and(|mc| mc.model_name == model_name)
    {
        ...
        rook::model::ModelConfig::new(&model_name)
            .unwrap_or_else(|e| {
...
async fn resolve_session_id(
    session_config: &SessionBuilderConfig,
    session_manager: &rook::session::session_manager::SessionManager,
    rook_mode: RookMode,
) -> String {
...
async fn configure_session_prompts(
    ...
    let system_prompt_file: Option<String> = config.get_param("ROOK_SYSTEM_PROMPT_FILE_PATH").ok();
...
pub async fn build_session(
    ...
    rook::posthog::set_session_context("cli", session_config.resume);
    ...
    let session_id =
        resolve_session_id(&session_config, &session_manager, agent.config.rook_mode).await;
    ...
    .await
    {
        ...
        Err(e) => {
            output::render_error(&format!(
                ...
                For more info, see: https://rook-docs.ai/docs/troubleshooting/#keychainkeyring-errors",
                e
            ));
            process::exit(1);
        }
    ...
    agent
        .update_rook_mode(agent.config.rook_mode, &session_id)
        .await
        .unwrap_or_else(|e| {
    ...
    fn test_session_builder_config_creation() {
        let config = SessionBuilderConfig {
            ...
            streamable_http_extensions: vec![StreamableHttpOptions {
                ...
                timeout: rook::config::DEFAULT_EXTENSION_TIMEOUT,
            }],

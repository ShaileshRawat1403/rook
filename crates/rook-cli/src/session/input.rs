use super::completion::RookCompleter;
...
pub fn get_newline_key() -> char {
    Config::global()
        .get_param::<String>("ROOK_CLI_NEWLINE_KEY")
        .ok()
...
///
/// When `rook_prompt_editor` is configured, defaults to `true` (backward compat).
/// Users can override by explicitly setting `rook_prompt_editor_always` to `false`.
...
pub fn get_input(
    editor: &mut Editor<RookCompleter, rustyline::history::DefaultHistory>,
    conversation_messages: Option<&Vec<String>>,
) -> Result<InputResult> {
    let config = Config::global();
    let prompt_editor = config.get_rook_prompt_editor().ok().flatten();
    let editor_always_override = config.get_rook_prompt_editor_always().ok().flatten();
    let editor_always = should_use_editor_always(prompt_editor.as_deref(), editor_always_override);

    if editor_always {
        if let Ok(Some(editor_cmd)) = config.get_rook_prompt_editor() {
...
fn handle_slash_command(input: &str) -> Option<InputResult> {
    ...
    match input {
        ...
        s if s.starts_with(CMD_MODE) => Some(InputResult::RookMode(
...
fn print_help() {
    ...
    let modes = RookMode::VARIANTS.join(", ");
    println!(
        "Available commands:
...
/mode <name> - Set the Rook mode to use ({modes})
/plan <message_text> -  Enters 'plan' mode with optional message. Create a plan based on the current messages and asks user if they want to act on it.
                        If user acts on the plan, Rook mode is set to 'auto' and returns to normal execution mode.
                        To warm up Rook before using '/plan', we recommend setting '/mode approve' and providing the right working context first.
                        The model is used based on $ROOK_PLANNER_PROVIDER and $ROOK_PLANNER_MODEL environment variables.
                        If no model is set, the default model is used.
/endplan - Exit plan mode and return to normal execution mode.
...
/edit [text] - Open your prompt editor to compose a message. Optionally pre-fill with text.
               Uses $ROOK_PROMPT_EDITOR, $VISUAL, or $EDITOR (in that order).
/? or /help - Display this help message
/clear - Clears the current chat history

Navigation:
Ctrl+C - Clear current line if text is entered, otherwise exit the session
Ctrl+{newline_key} - Add a newline (configurable via ROOK_CLI_NEWLINE_KEY)
...
fn print_editor_help() {
  ...
  Configure editor: rook configure set rook_prompt_editor \"vim\"
  Falls back to $VISUAL or $EDITOR if rook_prompt_editor is not set.
  When rook_prompt_editor is set, the editor is used for every prompt by default.
  To use inline prompts with on-demand /edit: rook configure set rook_prompt_editor_always false"
    );
}

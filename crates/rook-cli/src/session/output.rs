...
use rook::providers::canonical::maybe_get_canonical_model;
#[cfg(target_os = "windows")]
use rook::subprocess::SubprocessExt;
...
pub const ROOK_EMOJI: &str = "ROOK";
pub const ROOK_NAME: &str = "Rook";
    ...
    fn as_str(&self) -> String {
        match self {
            Theme::Light => Config::global()
                .get_param::<String>("ROOK_CLI_LIGHT_THEME")
                .unwrap_or(DEFAULT_CLI_LIGHT_THEME.to_string()),
            Theme::Dark => Config::global()
                .get_param::<String>("ROOK_CLI_DARK_THEME")
...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("ROOK_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("ROOK_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
                    .unwrap_or(Theme::Ansi)
            )
    );
    static SHOW_FULL_TOOL_OUTPUT: RefCell<bool> = RefCell::new(
        Config::global().get_param::<bool>("ROOK_SHOW_FULL_OUTPUT").unwrap_or(false)
    );
}
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("ROOK_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("ROOK_CLI_THEME", theme_str) {
...
pub fn run_status_hook(status: &str) {
    if let Ok(hook) = Config::global().get_param::<String>("ROOK_STATUS_HOOK") {
...
pub fn rook_mode_message(text: &str) {
    println!("\n{}", style(text).yellow(),);
}
...
pub fn render_welcome_state() {
    ...
    println!(
        ...
        style(" Welcome to Rook").bold().white()
    );
    println!("{}  {}", style("│ ").dim(), style(" ").dim());
    println!(
        "{}  {}  {}",
        style("│ ").dim(),
        style("You're ready to work. Here's what you can do:").dim(),
        style(" ").dim()
    );
    println!(
        "{}  {}  {}  {}  {}",
        ...
        style("rook").cyan(),
        style("─").dim(),
        style("Start an interactive session").dim()
    );
    println!(
        "{}  {}  {}  {}  {}",
        style("│ ").dim(),
    ...
    println!(
        ...
        style("─").dim(),
        style("Set up your provider").dim()
    );
    println!(
        "{}  {}  {}  {}  {}",
        style("│ ").dim(),
        style("─").dim(),
        style("rook doctor").cyan(),
        style("─").dim(),
        style("Check your setup").dim()
    );
    println!("{}  {}", style("│ ").dim(), style(" ").dim());
    println!(
        "{}  {}  {}",
        style("│ ").dim(),
        style("Not configured? Run 'rook configure' first.").yellow(),
        style(" ").dim()
    );
    println!(
        "{}",
...
pub fn render_session_state(state: &str) {
    let icon = style(ROOK_EMOJI).cyan();
    let name = style(ROOK_NAME).dim();

    match state {
        "planning" => {
            println!("\n{}  {} · {}", icon, name, style("planning").yellow());
        }
        "acting" => {
            println!("\n{}  {} · {}", icon, name, style("acting").cyan());
        }
        "waiting" => {
            println!(
                "\n{}  {} · {}",
                icon,
                name,
                style("waiting for approval").yellow()
            );
        }
        "completed" => {
            println!("\n{}  {} · {}", icon, name, style("completed").green());
        }
        _ => {}
    }
}

pub fn render_risk_badge(risk_level: &str) {
    let (icon, label, color) = match risk_level {
        "low" => ("🟢", "LOW RISK", Color::Green),
        "medium" => ("🟡", "MEDIUM RISK", Color::Yellow),
        "high" => ("🟠", "HIGH RISK", Color::Yellow),
        "critical" => ("🔴", "CRITICAL", Color::Red),
        _ => ("⚪", "UNKNOWN", Color::White),
    };

    println!("{}  {}", style(icon).fg(color), style(label).fg(color));
}

pub fn render_trust_summary(
    success: bool,
    duration_secs: u64,
    tokens: i32,
    ...
    if let Some(args) = &call.arguments {
        if let Some(Value::String(content)) = args.get("content") {
            ...
            println!(
                "    {} {}",
                style("items").dim(),
                style(item_count.to_string()).dim()
            );
            if let Some(first_item) = first_item {
                println!(
                    "    {} {}",
                    style("focus").dim(),
                    style(safe_truncate(&first_item, 80)).dim()
                );
            }
        }
    }
...
fn set_terminal_title() {
    ...
    print!("\x1b]0;Rook {}\x07", sanitized);
    let _ = std::io::stdout().flush();
}

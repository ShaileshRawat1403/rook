...
use rook::posthog::get_telemetry_choice;
use rook::recipe::Recipe;
use rook_mcp::mcp_server_runner::{serve, McpCommand};
use rook_mcp::{AutoVisualiserRouter, ComputerControllerServer, MemoryServer, TutorialServer};
...
use crate::session::{build_session, render_welcome_state, SessionBuilderConfig};
use rook::agents::Container;
...
#[derive(Parser)]
#[command(name = "rook", author, version, display_name = "", about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
...
pub struct SessionOptions {
    ...
    #[arg(
        ...
        long_help = "Run extensions (stdio and built-in) inside the specified container. The extension must exist in the container. For built-in extensions, rook must be installed inside the container."
    )]
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), rook::config::DEFAULT_EXTENSION_TIMEOUT);
...
pub struct ExtensionOptions {
    ...
    #[arg(
        ...
        long_help = "Add one or more builtin extensions that are bundled with rook by specifying their names, comma-separated",
        value_delimiter = ','
    )]
...
pub struct InputOptions {
    ...
    #[arg(
        ...
        long = "text",
        value_name = "TEXT",
        help = "Input text to provide to rook directly",
        long_help = "Input text containing commands for rook. Use this in lieu of the instructions argument.",
    ...
    #[arg(
        ...
        help = "Dynamic parameters (e.g., --params username=alice --params channel_name=rook-channel)",
...
pub struct ModelOptions {
    ...
    #[arg(
        ...
        long_help = "Override the ROOK_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
    )]
    ...
    #[arg(
        ...
        long_help = "Override the ROOK_MODEL environment variable for this run. The model must be supported by the specified provider."
    )]
...
enum RecipeCommand {
    ...
    #[command(about = "Open a recipe in Rook Desktop")]
    Open {
...
enum Command {
    ...
    #[command(about = "Check that your Rook setup is working")]
    Doctor {},

    /// Manage system prompts and behaviors
    #[command(about = "Run one of the mcp servers bundled with rook")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run rook as an ACP (Agent Client Protocol) agent
    #[command(about = "Run rook as an ACP agent server on stdio")]
    Acp {
    ...
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the rook CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure rook during update
        #[arg(short, long, help = "Enforce to re-configure rook during update")]
        reconfigure: bool,
    },

    /// Terminal-integrated session (one session per terminal)
    #[command(
        about = "Terminal-integrated Rook session",
        long_about = "Runs a Rook session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(rook term init zsh)\"  # Add to ~/.zshrc\n\n\
                      Usage:\n  \
                        rook term run \"list files in this directory\"\n  \
                        @rook \"create a python script\"  # using alias\n  \
                        @r \"quick question\"  # short alias"
    )]
    Term {
        #[command(subcommand)]
        command: TermCommand,
    ...
    Completion {
        ...
        #[arg(long, default_value = "rook", help = "Provide a custom binary name")]
        bin_name: String,
    },
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(rook term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                      With --default (anything typed that isn't a command goes to Rook):\n  \
                        echo 'eval \"$(rook term init zsh --default)\"' >> ~/.zshrc"
    )]
    Init {
        ...
        /// Make Rook the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make Rook the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to rook. Only supported for zsh and bash."
        )]
        default: bool,
    },
    ...
    #[command(
        ...
        long_about = "Run a prompt in the terminal-integrated session.\n\n\
                      Examples:\n  \
                        rook term run list files in this directory\n  \
                        @rook list files  # using alias\n  \
                        @r why did that fail  # short alias"
    )]
    Run {
        /// The prompt to send to rook (multiple words allowed without quotes)
...
async fn handle_serve_command(host: String, port: u16, builtins: Vec<String>) -> Result<()> {
    ...
    use rook_acp::server_factory::{AcpServer, AcpServerFactoryConfig};
    ...
    let router = rook_acp::transport::create_router(server);
...
async fn handle_interactive_session(
    ...
    tracing::info!(
        monotonic_counter.rook.session_starts = 1,
        session_type,
        interactive = true,
        "Session started"
    );
    ...
    let rook_mode = Config::global().get_rook_mode().unwrap_or_default();
    let mut session_id = get_or_create_session_id(identifier, resume, false, rook_mode).await?;
    ...
    let mut session: crate::CliSession = build_session(
...
async fn log_session_completion(
    ...
    tracing::info!(
        monotonic_counter.rook.session_completions = 1,
        session_type,
        exit_type,
    ...
    tracing::info!(
        monotonic_counter.rook.session_duration_ms = session_duration.as_millis() as u64,
        session_type,
        "Session duration"
    );
    ...
    if total_tokens > 0 {
        tracing::info!(
            monotonic_counter.rook.session_tokens = total_tokens,
            session_type,
            "Session tokens"
        );
    }
    ...
    match (
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    rook::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
            ...
            tracing::info!(
                monotonic_counter.rook.recipe_runs = 1,
...
async fn handle_local_models_command(command: LocalModelsCommand) -> Result<()> {
    use rook::providers::local_inference::hf_models;
    use rook::providers::local_inference::local_model_registry::{
    ...
    match command {
        LocalModelsCommand::Search { query, limit } => {
            ...
            for model in &results {
                ...
                println!(
                    "  Download: rook local-models download {}:<quantization>",
                    model.repo_id
                );
            }
        }
        LocalModelsCommand::Download { spec } => {
            ...
            let local_path = rook::config::paths::Paths::in_data_dir("models").join(&file.filename);
            ...
            // Download
            let manager = rook::download_manager::get_download_manager();
            manager
                .download_model(
            ...
            loop {
                if let Some(progress) = manager.get_progress(&format!("{}-model", model_id)) {
                    match progress.status {
                        rook::download_manager::DownloadStatus::Downloading => {
                            print!(
                                "\r  {:.1}% ({:.0}MB / {:.0}MB)",
                        ...
                        rook::download_manager::DownloadStatus::Completed => {
                            println!("\nDownloaded: {}", model_id);
                            break;
                        }
                        rook::download_manager::DownloadStatus::Failed => {
                            let err = progress.error.unwrap_or_default();
                            anyhow::bail!("Download failed: {}", err);
                        }
                        rook::download_manager::DownloadStatus::Cancelled => {
                            println!("\nDownload cancelled.");
                            break;
                        }
                    }
...
async fn handle_default_session() -> Result<()> {
    ...
    let rook_mode = Config::global().get_rook_mode().unwrap_or_default();
    let session_id = get_or_create_session_id(None, false, false, rook_mode).await?;
...
pub async fn cli() -> anyhow::Result<()> {
    ...
    tracing::info!(
        monotonic_counter.rook.cli_commands = 1,
    ...
    match cli.command {
        ...
        Some(Command::Acp { builtins }) => rook_acp::server::run(builtins).await,
        Some(Command::Serve {
            host,
            port,
            builtins,

...
use rook::acp::{PermissionDecision, ACP_CURRENT_MODEL};
use rook::agents::extension::{Envs, PLATFORM_EXTENSIONS};
use rook::agents::mcp_client::McpClientTrait;
use rook::agents::platform_extensions::developer::DeveloperClient;
use rook::agents::{Agent, AgentConfig, ExtensionConfig, RookPlatform, SessionConfig};
use rook::builtin_extension::register_builtin_extensions;
use rook::config::base::CONFIG_YAML_NAME;
use rook::config::extensions::get_enabled_extensions_with_config;
use rook::config::paths::Paths;
use rook::config::permission::PermissionManager;
use rook::config::{Config, RookMode};
use rook::conversation::message::{ActionRequiredData, Message, MessageContent};
use rook::mcp_utils::ToolResult;
use rook::permission::permission_confirmation::PrincipalType;
use rook::permission::{Permission, PermissionConfirmation};
use rook::providers::base::Provider;
use rook::session::session_manager::SessionType;
use rook::session::{EnabledExtensionsState, Session, SessionManager};
use rook_acp_macros::custom_methods;
use sacp::schema::{
...
struct RookAcpSession {
    ...
    tool_requests: HashMap<String, rook::conversation::message::ToolRequest>,
...
struct AgentSetupRequest {
    ...
    resolved_provider: Option<(String, rook::model::ModelConfig)>,
}

pub struct RookAcpAgent {
    sessions: Arc<Mutex<HashMap<String, RookAcpSession>>>,
    ...
    thread_manager: Arc<rook::session::ThreadManager>,
...
fn extract_locations_from_meta(
    tool_response: &rook::conversation::message::ToolResponse,
...
fn extract_tool_locations(
    tool_request: &rook::conversation::message::ToolRequest,
    tool_response: &rook::conversation::message::ToolResponse,
            ...
            if let Some(v) = obj.get(*key) {
                ...
                if !s.is_empty() {
                    ...
                    if first_line.len() > 60 {
                        return Some(format!("{}…", rook::utils::safe_truncate(first_line, 57)));
                    }
...
async fn list_provider_entries(current_provider: Option<&str>) -> Vec<ProviderListEntry> {
    let mut providers = rook::providers::providers()
        .await
        .into_iter()
...
async fn resolve_provider_and_model_from_config(
    config: &Config,
    rook_session: &Session,
) -> Result<(String, rook::model::ModelConfig), String> {
    let global_provider = config.get_rook_provider().ok();
    let provider_override = rook_session
        .provider_name
        .as_deref()
    ...
    let model_config = match &rook_session.model_config {
        Some(mc) => mc.clone(),
        None if explicitly_switched => {
            let entry = rook::providers::get_from_registry(&provider_name)
                .await
                .map_err(|e| e.to_string())?;
            let default_model = &entry.metadata().default_model;
            rook::model::ModelConfig::new(default_model)
                .map_err(|e| e.to_string())?
                .with_canonical_limits(&provider_name)
        }
        None => {
            let model_id = config.get_rook_model().map_err(|e| e.to_string())?;
            rook::model::ModelConfig::new(&model_id)
                .map_err(|e| e.to_string())?
                .with_canonical_limits(&provider_name)
        }
    };
...
async fn resolve_provider_and_model(
    ...
    let config =
        Config::new(config_dir.join(CONFIG_YAML_NAME), "rook").map_err(|e| e.to_string())?;
    resolve_provider_and_model_from_config(&config, rook_session).await
}

fn build_mode_state(current_mode: RookMode) -> Result<SessionModeState, sacp::Error> {
    let mut available = Vec::with_capacity(RookMode::VARIANTS.len());
    for &name in RookMode::VARIANTS {
        let rook_mode: RookMode = name.parse().map_err(|_| {
            sacp::Error::internal_error() // impossible but satisfy linters
                .data(format!("Failed to parse RookMode variant: {}", name))
        })?;
        let mut mode = SessionMode::new(SessionModeId::new(name), name);
        mode.description = rook_mode.get_message().map(Into::into);
        available.push(mode);
    }
...
async fn build_eager_config(
    resolved: &Result<(String, rook::model::ModelConfig), String>,
    ...
    let recommended = rook::providers::canonical::recommended_models_from_registry(provider_name);
    let available: Vec<ModelInfo> = recommended
        .iter()
        .map(|name| ModelInfo::new(ModelId::new(&**name), &**name))
        .collect();
    let ms = SessionModelState::new(ModelId::new(mc.model_name.as_str()), available);
    let provider_selection = session_provider_selection(rook_session);
...
impl RookAcpAgent {
    ...
    pub async fn new(
        ...
        let thread_manager = Arc::new(rook::session::ThreadManager::new(
    ...
    fn load_config(&self) -> Result<Config> {
        Config::new(self.config_dir.join(CONFIG_YAML_NAME), "rook").map_err(Into::into)
    }

    async fn create_provider(
        &self,
        provider_name: &str,
        model_config: rook::model::ModelConfig,
    ...
    fn spawn_agent_setup(
        ...
        let rook_mode = rook_session.rook_mode;
        let internal_session_id = rook_session.id.clone();
        ...
        tokio::spawn(async move {
            ...
            let result: Result<(), String> = async {
                ...
                let (provider_name, model_config) = match resolved_provider {
                    ...
                    None => resolve_provider_and_model_from_config(&config, &rook_session).await?,
                };
                let ext_state = EnabledExtensionsState::extensions_or_default(
                    Some(&rook_session.extension_data),
                    &config,
                );
                let provider = provider_factory(provider_name.to_string(), model_config, ext_state)
                    ...
                    .map_err(|e| e.to_string())?;
                agent
                    .update_provider(provider.clone(), &rook_session.id)
                    .await
                    .map_err(|e| e.to_string())?;

                agent
                    .update_rook_mode(rook_mode, &internal_session_id)
                    .await
                    .map_err(|e| e.to_string())?;
                ...
                RookAcpAgent::add_mcp_extensions(&agent, mcp_servers, &internal_session_id)
                    .await
                    .map_err(|e| e.to_string())?;
                debug!(
                    target: "perf",
        ...
        if let Ok(tool_call) = &tool_request.tool_call {
            ...
            let args_json = tool_call
                ...
                .map(|a| {
                    ...
                    if s.len() > 300 {
                        format!("{}…", rook::utils::safe_truncate(&s, 300))
                    } else {
                        s
                    }
                })
...
impl RookAcpAgent {
    async fn on_initialize(
        ...
        Ok(InitializeResponse::new(args.protocol_version)
            ...
            .auth_methods(vec![AuthMethod::Agent(
                AuthMethodAgent::new("rook-provider", "Configure Provider")
                    .description("Run `rook configure` to set up your AI provider and API key"),
            )]))
    }
    ...
    async fn on_new_session(
        ...
        let thread_metadata = rook::session::ThreadMetadata {
            provider_id: requested_provider.clone(),
            mode: Some(self.rook_mode.to_string()),
            ..Default::default()
        };
        ...
        let mode_state = build_mode_state(self.rook_mode)?;
        ...
        let resolved = resolve_provider_and_model(&self.config_dir, &rook_session).await;
        let initial_usage_update = resolved
            .as_ref()
            .ok()
            .map(|(_, mc)| build_usage_update(&rook_session, mc.context_limit()));
        let (model_state, config_options) =
            build_eager_config(&resolved, &mode_state, &rook_session).await;
    ...
    async fn create_internal_session(
        ...
        let mut builder = self.session_manager.update(&rook_session.id);
        builder = builder.thread_id(Some(thread_id.to_string()));
        if let Some(provider) = provider_name {
            builder = builder.provider_name(provider);
        }
        if let Some(model) = model_name {
            if let Ok(mc) = rook::model::ModelConfig::new(model) {
        ...
        self.session_manager
            .get_session(&rook_session.id, false)
            .await
            .map_err(|e| {
    ...
    async fn on_load_session(
        ...
        let loaded_mode = rook_session.rook_mode;
        ...
        // matching request. No RookAcpSession required.
        let mut replay_tool_requests =
            HashMap::<String, rook::conversation::message::ToolRequest>::new();
            ...
            for content_item in &message.content {
                match content_item {
                    ...
                    MessageContent::ToolRequest(tool_request) => {
                        ...
                        // don't require a full RookAcpSession.
        ...
        let initial_usage_update = resolved
            ...
            .or_else(|| {
                rook_session
                    .model_config
                    .as_ref()
                    .map(|mc| build_usage_update(&rook_session, mc.context_limit()))
            });
        let (model_state, config_options) =
            build_eager_config(&resolved, &mode_state, &rook_session).await;
    ...
    async fn on_prompt(
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                Ok(rook::agents::AgentEvent::Message(message)) => {
                    self.thread_manager
    ...
    async fn on_set_model(
        ...
        let model_config = rook::model::ModelConfig::new(model_id)
            .map_err(|e| {
        ...
        let t_step = std::time::Instant::now();
        let mode = agent.rook_mode().await;
        agent
            .update_rook_mode(mode, &internal_id)
            .await
            .map_err(|e| {
                sacp::Error::internal_error().data(format!("Failed to propagate mode: {}", e))
            })?;
        debug!(target: "perf", sid = %sid, ms = t_step.elapsed().as_millis() as u64, "perf: set_model update_rook_mode");
    ...
    async fn update_thread_metadata(
        &self,
        thread_id: &str,
        f: impl FnOnce(&mut rook::session::ThreadMetadata),
    ) -> Result<(), sacp::Error> {
        self.thread_manager
    ...
    async fn build_config_update(
        ...
        let rook_mode = agent.rook_mode().await;
        let model_state = build_model_state(&*provider).await?;
        let mode_state = build_mode_state(rook_mode)?;
    ...
    async fn on_set_mode(
        ...
        let internal_id = self.internal_session_id(thread_id).await?;
        let mode = mode_id.parse::<RookMode>().map_err(|_| {
    ...
    async fn update_provider(
        ...
        let resolved_provider_name = if use_default_provider {
            config.get_rook_provider().map_err(|e| {
        ...
        let default_model = if let Some(model_name) = model_name {
            ...
            config.get_rook_model().map_err(|e| {
                sacp::Error::internal_error().data(format!(
        ...
        let model_config = rook::model::ModelConfig::new(model)
            .map_err(|e| {
                sacp::Error::invalid_params().data(format!("Invalid model config: {}", e))
            })?
        ...
        debug!(target: "perf", sid = %sid, ms = t_step.elapsed().as_millis() as u64, "perf: update_provider update_rook_mode");
...
impl RookAcpAgent {
    ...
    async fn on_get_extensions(&self) -> Result<GetExtensionsResponse, sacp::Error> {
        let extensions = rook::config::extensions::get_all_extensions();
        let warnings = rook::config::extensions::get_warnings();
    ...
    async fn on_read_config(
        ...
        let response = match config.get_param::<serde_json::Value>(&req.key) {
            ...
            Err(rook::config::ConfigError::NotFound(_)) => ReadConfigResponse {
...
pub struct RookAcpHandler {
    pub agent: Arc<RookAcpAgent>,
}

impl HandleDispatchFrom<Client> for RookAcpHandler {
...
{
    Box::pin(async move {
        let handler = RookAcpHandler { agent };
...
pub async fn run(builtins: Vec<String>) -> Result<()> {
    register_builtin_extensions(rook_mcp::BUILTIN_EXTENSIONS.clone());
...
mod tests {
    ...
    use rook::conversation::message::{ToolRequest, ToolResponse};
    use rook::providers::errors::ProviderError;
    use sacp::schema::{
...
```",
    ...
    impl Provider for MockModelProvider {
        ...
        async fn stream(
            &self,
            _model_config: &rook::model::ModelConfig,
            _session_id: &str,
            _system: &str,
            _messages: &[rook::conversation::message::Message],
            _tools: &[rmcp::model::Tool],
        ) -> Result<rook::providers::base::MessageStream, ProviderError> {
            unimplemented!()
        }

        fn get_model_config(&self) -> rook::model::ModelConfig {
            rook::model::ModelConfig::new_or_fail("unused")
        }
    ...
    fn make_session_with_usage(
        ...
        Session {
            ...
            extension_data: rook::session::ExtensionData::default(),
            total_tokens,
            input_tokens,
            output_tokens,
    ...
    fn test_build_mode_state(current_mode: RookMode) -> Result<SessionModeState, sacp::Error> {
        build_mode_state(current_mode)
    }

    #[test_case(
        build_mode_state(RookMode::Auto).unwrap(),
        "openai",
        vec![
    ...
    #[test_case(
        build_mode_state(RookMode::Approve).unwrap(),
        "openai",

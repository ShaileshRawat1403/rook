...
use rook_acp::server::AcpProviderFactory;
use rook_test_support::{McpFixture, FAKE_CODE, TEST_IMAGE_B64, TEST_MODEL};
use sacp::schema::{
...
pub async fn run_load_mode<C: Connection>() {
    ...
    assert_eq!(
        ...
        SessionModeId::new(<&str>::from(RookMode::default()))
    );
    let session_id = session.session_id().0.to_string();
    conn.set_mode(&session_id, <&str>::from(RookMode::Approve))
        .await
        .unwrap();
    ...
    assert_eq!(
        modes.unwrap().current_mode_id,
        SessionModeId::new(<&str>::from(RookMode::Approve))
    );
...
async fn run_mode_set_impl<C: Connection>(via: SetModeVia) {
    ...
    let approve = <&str>::from(RookMode::Approve);
    match via {
...
pub async fn run_prompt_image<C: Connection>() {
    ...
    let output = session
        ...
        .await
        .unwrap();
    assert_eq!(output.text, "Hello Rook!\nThis is a test image.");
    assert_notifications(
        &session.notifications(),
        &[
            Notification::ToolCall,
...
pub async fn run_prompt_image_attachment<C: Connection>() {
    ...
    let output = session
        ...
        .await
        .unwrap();
    assert!(output.text.contains("Hello Rook!"));
    assert_notifications(&session.notifications(), &[Notification::AgentMessage]);
    expected_session_id.assert_matches(&session.session_id().0);
}

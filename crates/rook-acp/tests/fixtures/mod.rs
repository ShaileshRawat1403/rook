...
use fs_err as fs;
pub use rook::acp::{map_permission_response, PermissionDecision};
use rook::builtin_extension::register_builtin_extensions;
use rook::config::paths::Paths;
use rook::config::{PermissionManager, RookMode};
use rook::providers::api_client::{ApiClient, AuthMethod as ApiAuthMethod};
use rook::providers::base::Provider;
use rook::providers::openai::OpenAiProvider;
use rook::session_context::SESSION_ID_HEADER;
use rook_acp::server::{serve, AcpProviderFactory, RookAcpAgent};
use rook_test_support::{ExpectedSessionId, TEST_MODEL};
use sacp::schema::{
...
#[allow(dead_code)]
pub async fn serve_agent_in_process(agent: Arc<RookAcpAgent>) -> (DuplexTransport, JoinHandle<()>) {
...
pub async fn spawn_acp_server_in_process(
    ...
    let config_path = data_root.join(rook::config::base::CONFIG_YAML_NAME);
    if !config_path.exists() {
        fs::write(
            &config_path,
            format!("ROOK_MODEL: {current_model}\nROOK_PROVIDER: openai\n"),
        )
        .unwrap();
    }

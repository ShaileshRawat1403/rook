...
use rook::acp::{AcpProvider, AcpProviderConfig};
...
use rook_test_support::{ExpectedSessionId, IgnoreSessionId, TEST_MODEL};
use sacp::schema::{
...
impl Connection for AcpProviderConnection {
    ...
    async fn new(config: TestConnectionConfig, openai: OpenAiFixture) -> Self {
        ...
        let rook_mode = config.rook_mode;
        ...
        let provider_config = AcpProviderConfig {
            ...
            mode_mapping: RookMode::VARIANTS
                .iter()
                .map(|v| {
                    let mode = RookMode::from_str(v).unwrap();
                    (mode, mode.to_string())
                })
                .collect(),
    ...
    async fn new_session(&mut self) -> anyhow::Result<SessionData<AcpProviderSession>> {
        ...
        let rook_id = format!("test-session-{}", self.session_counter);
        ...
        let session = AcpProviderSession {
            ...
            session_id: sacp::schema::SessionId::new(rook_id),

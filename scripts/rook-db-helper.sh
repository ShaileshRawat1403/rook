...
get_rook_db_path() {
    if [[ -n "${ROOK_PATH_ROOT:-}" ]]; then
        echo "${ROOK_PATH_ROOT}/data/sessions/sessions.db"
    else
...
show_status() {
    echo -e "${BLUE}=== Rook Database Status ===${NC}"
    ...
    if [[ ! -f "${DB_PATH}" ]]; then
        ...
        echo ""
        echo "This is normal if you haven't run Rook yet."
        echo "Once you run Rook, a database will be created automatically."
        return
    fi
...
show_help() {
    ...
    echo -e "${BLUE}Rook Database Migration Helper${NC}"
    echo ""

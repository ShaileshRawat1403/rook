# Rook Benchmark Scripts

This directory contains scripts for running and analyzing Rook benchmarks.

## run-benchmarks.sh

This script runs Rook benchmarks across multiple provider:model pairs and analyzes the results.

### Prerequisites

- Rook CLI must be built or installed
...
3. For each provider:model pair:
   - Sets the `ROOK_PROVIDER` and `ROOK_MODEL` environment variables

{
  description = "rook - An AI agent CLI";
  ...
  outputs = { self, nixpkgs, flake-utils, rust-overlay }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        cargoToml = builtins.fromTOML (builtins.readFile ./crates/rook-cli/Cargo.toml);
      ...
      in
      {
        packages.default = pkgs.rustPlatform.buildRustPackage {
          ...
          cargoBuildFlags = [ "--package" "rook-cli" ];
          ...
          checkPhase = ''
            ...
            # Run tests for rook-cli package only
            cargo test --package rook-cli --release
          '';
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            echo "rook development environment"
            echo "Rust version: $(rustc --version)"
            echo ""
            echo "Commands:"
            echo "  nix build           - Build rook CLI"
            echo "  nix run             - Run rook CLI"
            echo "  cargo build -p rook-cli - Build with cargo"
            echo "  cargo run -p rook-cli   - Run with cargo"
          '';
        };
      }
    );

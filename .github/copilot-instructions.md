...
- Core crates: `rook` (agent logic), `rook-cli` (CLI, hosts `rook serve` ACP server), `rook-acp` (ACP schema), `rook-mcp` (MCP servers)
- UI: Tauri app under `ui/rook`, consuming `ui/sdk` generated ACP types
...
- `just check-acp-schema` - ACP schema and generated TS types are in sync

**Tauri UI checks (`.github/workflows/rook-ui-ci.yml`):**
- `pnpm install --frozen-lockfile` - Fresh dependency install (in `ui/rook/`)
- `pnpm check` + `pnpm typecheck` - Biome + tsc
- `pnpm test` - Vitest tests
- `cargo clippy` / `cargo fmt --check` inside `ui/rook/src-tauri`

...
ROOK_HOST = "127.0.0.1"
ROOK_PORT = "3001"
ROOK_URL = f"http://{ROOK_HOST}:{ROOK_PORT}"
...
async def setup_agent() -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{ROOK_URL}/agent/update_provider",
...
def submit_tool_result(tool_id: str, result: List[Dict[str, Any]]) -> None:
    """Submit the tool execution result back to Rook.
...
async def chat_loop() -> None:
    """Main chat loop that handles the conversation with Rook."""

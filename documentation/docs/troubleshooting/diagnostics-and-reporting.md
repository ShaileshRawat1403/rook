...
rook provides several built-in features to help you get support, report issues, and request new functionality. This guide covers the diagnostics system, bug reporting, and feature request tools.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="rook CLI">
    ...
    rook session diagnostics --session-id <session_id>

    # Interactive selection (prompts you to choose a session)
    rook session diagnostics

    # Save to a custom location
    rook session diagnostics --session-id <session_id> --output /path/to/diagnostics.zip
    ```

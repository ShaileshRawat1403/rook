...
rook is an extensible open source AI agent that enhances your software development by automating coding tasks. 
  ...
  <TabItem value="windows" label="Windows">
    ...
    <Tabs groupId="interface">
      ...
      <TabItem value="cli" label="rook CLI">
        ...
        If you see a PATH warning after installation, you'll need to add rook to your PATH before running `rook configure`. See the [Windows CLI installation instructions](/docs/getting-started/installation) for detailed steps.
...
rook works with [supported LLM providers](/docs/getting-started/providers) that give rook the AI intelligence it needs to understand your requests. On first use, you'll be prompted to configure your preferred provider.
...
<Tabs groupId="interface">
  ...
  rook will open a browser for you to authenticate with Tetrate, or create a new account if you don't have one already. When you return to the rook desktop app, you're ready to begin your first session.
...
## Start Session
Sessions are single, continuous conversations between you and rook. Let's start one.
...
From the prompt, you can interact with rook by typing your instructions exactly as you would speak to a developer.

Let's ask rook to make a tic-tac-toe game!

```
create an interactive browser-based tic-tac-toe game in javascript where a player competes against a bot
```

rook will create a plan and then get right to work on implementing it. Once done, your directory should contain a JavaScript file as well as an HTML page for playing.


## Enable an Extension

While you're able to manually navigate to your working directory and open the HTML file in a browser, wouldn't it be better if rook did that for you? Let's give rook the ability to open a web browser by enabling the [`Computer Controller` extension](/docs/mcp/computer-controller-mcp).
...
<Tabs groupId="interface">
    ...
    <TabItem value="ui" label="rook Desktop" default>
        ...
        5. Now that rook has browser capabilities, let's ask it to launch your game in a browser:
    </TabItem>
    <TabItem value="cli" label="rook CLI">
        ...
        ```
        4. Now that rook has browser capabilities, let's resume your last session:
        ```sh
         rook session -r
        ```
        5. Ask rook to launch your game in a browser:
    </TabItem>
</Tabs>
...
## Next Steps
Congrats, you've successfully used rook to develop a web app! 🎉

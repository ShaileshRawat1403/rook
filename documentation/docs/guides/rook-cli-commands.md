...
rook provides a command-line interface (CLI) with several commands for managing sessions, configurations and extensions. This guide covers all available CLI commands and interactive session features.

## Flag Naming Conventions

rook CLI follows consistent patterns for flag naming to make commands intuitive and predictable:
...
#### configure
Configure rook settings - providers, extensions, etc.
...
:::tip Type to Filter
When selecting from menus in `rook configure`, start typing to filter options in real-time. This works for lists of providers, extensions, and tools.
:::

---

#### info [options]
Shows rook information, including the version, configuration file location, session storage, and logs.
...
#### version
Check the current rook version you have installed.
...
#### update [options]
Update the rook CLI to a newer version.

**Options:**
- **`--canary, -c`**: Update to the canary (development) version instead of the stable version
- **`--reconfigure, -r`**: Forces rook to reset configuration settings during the update process
...
#### completion
Generate shell-specific scripts to enable tab completion of rook commands, subcommands, and options. The script is printed to stdout, so you need to redirect it to the appropriate location for your shell and then reload or source your shell configuration.
...
rook completion bash
rook completion zsh
rook completion fish
```
...
:::tip Testing
After installing and reloading your shell, test completion by typing `rook ` and pressing Tab to see available commands, or `rook session --` and Tab to see available options.
:::
...
rook session -n my-project

# Resume a previous session
rook session --resume -n my-project
rook session --resume --session-id 20251108_2
rook session --resume --path ./session.json    # exported session
rook session --resume --path ./session.jsonl   # legacy session storage

# Fork a specific session by name
rook session --resume --fork --name my-project

# Fork the most recent session and show message history
rook session --resume --fork --history

# Start with extensions
rook session --with-extension "npx -y @modelcontextprotocol/server-memory"
rook session --with-builtin developer
rook session --with-streamable-http-extension "http://localhost:8080/mcp"
...
rook session -n my-session --debug --max-turns 25
```
...
rook session list --format json

# Sort sessions by date in ascending order
rook session list --ascending

# Filter sessions by working directory
rook session list -w ~/projects/myapp

# List only the 10 most recent sessions
rook session list --limit 10
```
...
rook session remove --session-id 20251108_3

# Remove a specific session by name
rook session remove -n my-project

# Remove all sessions starting with "project-"
rook session remove -r "project-.*"

# Remove all sessions containing "migration"
rook session remove -r ".*migration.*"
```

:::caution
Session removal is permanent and cannot be undone. rook will show which sessions will be removed and ask for confirmation before deleting.
:::
...
rook session export -n my-session --format json -o session-backup.json

# Export specific session as readable markdown
rook session export -n my-session -o session.md

# Export to stdout in different formats
rook session export --session-id 20251108_4 --format json
rook session export -n my-session --format yaml

# Export session by path (legacy)
rook session export --path ./my-session.jsonl -o exported.md
```
...
rook session diagnostics --session-id 20251108_5

# Generate diagnostics for a session by name
rook session diagnostics -n my-project-session

# Save diagnostics to a custom location
rook session diagnostics --session-id 20251108_5 -o /path/to/my-diagnostics.zip

# Interactive selection (prompts you to choose a session)
rook session diagnostics
```
...
- **`-t, --text <TEXT>`**: Input text to provide to rook directly
...
rook run --instructions plan.md

# Load a recipe with a prompt that rook executes and then exits  
...
rook run --recipe recipe.yaml --debug

# Show recipe details
rook run --recipe recipe.yaml --explain

# Run a recipe with parameters
rook run --recipe recipe.yaml --params environment=production --params region=us-west-2

# Run instructions from a file without session storage
rook run --no-session -i instructions.txt

# Run with a specified provider and model
rook run --provider anthropic --model claude-4-sonnet -t "initial prompt"

# Run with limited turns before prompting user
rook run --recipe recipe.yaml --max-turns 10
```
...
#### recipe
Used to validate recipe files, manage recipe sharing, list available recipes, and open recipes in rook desktop.
...
- **`open <RECIPE_NAME>`**: Open a recipe file directly in rook desktop
  - **`-p, --param <KEY=VALUE>`**: Pre-fill recipe parameter (can be specified multiple times)
...
rook recipe deeplink my-recipe.yaml

# Generate a deeplink and provide parameter values
rook recipe deeplink my-recipe.yaml -p environment=production -p region=us-west-2
...
rook recipe list --format json

# Open a recipe in rook desktop
rook recipe open my-recipe.yaml

# Open a recipe by name
rook recipe open my-recipe

# Open a recipe and provide parameter value
rook recipe open my-recipe --param name=myproject

# Validate a recipe file
rook recipe validate my-recipe.yaml
...
**Usage:**
```bash
rook schedule <COMMAND>

# Add a new scheduled recipe which runs every day at 9 AM
rook schedule add --schedule-id daily-report --cron "0 0 9 * * *" --recipe-source ./recipes/daily-report.yaml

# List all scheduled jobs
rook schedule list

# List the 10 most recent rook sessions created by a scheduled job
rook schedule sessions --schedule-id daily-report -l 10

# Run a recipe immediately
rook schedule run-now --schedule-id daily-report

# Remove a scheduled job
rook schedule remove --schedule-id daily-report
```
...
#### acp
Run rook as an Agent Client Protocol (ACP) agent server over stdio. This enables rook to work with ACP-compatible clients like Zed.
...
:::info
This command is automatically invoked by ACP-compatible clients and is not typically run directly by users. The client manages the lifecycle of the `rook acp` process. See [Using rook in ACP Clients](/docs/guides/acp-clients) for details.
:::
...
#### @rook / @g
Ask rook questions directly from your shell prompt, with command history included in the context. These aliases are created when you set up [terminal integration](/docs/guides/terminal-integration.md).

**Examples:**
```bash
# Ask questions with command history context
@rook create a python script to process these files
@rook create a PR description summarizing these changes
...
Once you're in an interactive session (via `rook session` or `rook run --interactive`), you can use these slash commands. All commands support tab completion. Press `/ + <Tab>` to cycle through available commands.

**Available Commands:**
- **`/?` or `/help`** - Display the help menu
- **`/builtin <names>`** - Add builtin extensions by name (comma-separated)
- **`/clear`** - Clear the current chat history
- **`/endplan`** - Exit plan mode and return to 'normal' rook mode
- **`/exit` or `/quit`** - Exit the session
- **`/extension <command>`** - Add a stdio extension (format: ENV1=val1 command args...)
- **`/mode <name>`** - Set the rook mode to use ('auto', 'approve', 'chat', 'smart_approve')
...
/clear
```
You can also create [custom slash commands for running recipes](/docs/guides/context-engineering/slash-commands) in rook Desktop or the CLI. 

---

### Themes

The `/t` command controls the syntax highlighting theme for markdown content in rook CLI responses. This affects the styles used for headers, code blocks, bold/italic text, and other markdown elements in the response output.
...
The rook CLI theme is independent from the rook Desktop theme.
:::
...
For composing longer prompts or working with complex code snippets, you can configure rook to use your preferred text editor instead of CLI input. This replaces the standard CLI input and keyboard shortcuts for the entire session.
...
GUI editors require a `--wait` or equivalent flag to ensure rook waits for you to finish editing before continuing. Without this flag, the editor opens but rook immediately proceeds as if you're done. Terminal editors like vim and nano don't need this flag.

---

### Command History Search

The `Ctrl+R` shortcut provides interactive search through your stored CLI [command history](/docs/guides/logs#command-history). This feature makes it easy to find and reuse recent commands without retyping them. When you type a search term, rook searches backwards through your history for matches.

**How it works:**
1. Press `Ctrl+R` in your rook CLI session

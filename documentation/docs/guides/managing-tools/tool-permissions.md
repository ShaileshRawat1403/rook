...
Tool permissions provide fine-grained control over how rook uses different tools within extensions. This guide will help you understand and configure these permissions effectively.

## Understanding Tools and Extensions

Before diving into permissions, let's clarify the key components:

- **Extensions** are packages that add functionality to rook (like Developer, Google Drive, etc.)
- **Tools** are specific functions within each extension that rook can use
...
rook performs best with fewer than 25 total tools enabled across all extensions. Consider enabling only the extensions you need for your current task.
:::
...
3. **Task Focus**
   ...
   - Help rook make better tool choices

...
A good plan keeps everyone on track and helps measure progress. That's why the rook CLI includes the `/plan` prompt completion command to help break down your projects into clear, manageable steps.
 
:::tip Plans in the rook Desktop
...
The rook CLI's plan mode is interactive, asking clarifying questions to understand your project before creating a plan. If you can provide thoughtful and informative answers to those questions, rook can generate a really useful and actionable plan.
...
You can also customize how rook creates plans by editing the `plan.md` [prompt template](/docs/guides/prompt-templates).
:::
...
If these aren't set, rook will use your default provider and model settings. You might want to set different planning models if you find certain models are better at breaking down tasks into clear steps. However, your default model configuration is usually sufficient.
...
rook can produce good plans for relatively simple projects such as the home construction example:
...
After gathering information through clarifying questions, rook creates a detailed plan for hiring an architect. This sub-plan integrates with the larger home construction project, with steps that reflect and support the overall construction context.

## A development project example
In this example, a developer has written a CLI in Python that interacts with the Contentful CMS to let a user search for strings and replace them with new strings. As a website, the search/replace feature would be more usable and also allow for a larger set of features. The developer is using the rook CLI to plan the conversion project.

If rook believes the project can be completed in many different ways and using a wide variety of components, it will ask you a clarifying question for each of these decision points. For example, if you start a plan like this:
...
```
rook parses your project description, consults with the LLM mode you've configured, and then if it needs more information, starts a round of clarifying questions.

## Clarifying questions
Converting a Python CLI into a website seems simple enough but rook will have questions about things like styling, authentication, features, technology stack, and more. You might see questions like this:
...
:::tip
When rook requests a project artifact like source code during plan mode, you'll need to paste the content directly into the chat. Simply copying the file contents and prefixing it with a brief description like 'Here's the requested code:' is sufficient. Note that providing just a file path won't work in plan mode.
:::

When answering multiple questions, number your responses to match each question. For example, instead of answering with a simple 'no' or 'don't remember', provide context like '2. Do not store my preferences.' This helps rook track which questions have been answered and prevents repeated questions.
...
```
While rook creates a standardized plan format, it customizes the content based on your answers. rook can generate the code needed to implement the steps of the plan it produces. You should review the plan and any code that it generates before ending plan mode (`/endplan`) and asking rook to implement the plan. 
...
## Basic usage
You need to have an active rook session before you can put the CLI into plan mode. If you are going to dedicate a session to creating a plan, you should give your new session a name as in the following example:

```bash
~ rook session -n web-project-plan
starting session | provider: databricks model: databricks-meta-llama
    session id: 20251110_5
    working directory: /Users/alincoln

rook is running! Enter your instructions, or try asking what rook can do.
```
...
```
 ...
 Plan mode in the CLI is a special interaction mode where rook helps break down tasks into manageable steps.  If you want to close the plan mode and return to the active session, type `/endplan`.

...
description: "Share a rook session setup (including tools, goals, and instructions) as a reusable recipe that others can launch with a single click"
---
...
Sometimes you finish a task in rook and realize, "Hey, this setup could be useful again." Maybe you have curated a great combination of tools, defined a clear goal, and want to preserve that flow. Or maybe you're trying to help someone else replicate what you just did without walking them through it step by step. 

You can turn your current rook session into a reusable recipe that includes the tools, goals, and setup you're using right now and package it into a new Agent that others (or future you) can launch with a single click.
...
You can customize how rook generates recipes by editing the `recipe.md` [prompt template](/docs/guides/prompt-templates).
:::
...
<Tabs groupId="interface">
  ...
  1. Open the recipe using a direct link or manual URL entry, or from your Recipe library:
     ...
     **Slash Command:**
         ...
         1. Enter a [custom slash command](/docs/guides/context-engineering/slash-commands) in any rook chat session
  ...
  4. The recipe automatically submits and rook begins execution. If the recipe includes a [prompt](#core-components), it's sent as the first message. If not, you can click an activity bubble or send a prompt to get started.
  ...
  Using a recipe with the rook CLI might involve the following tasks:
  ...
  Recipes can be stored locally on your device or in a GitHub repository. Configure your recipe repository using either the `rook configure` command or [config file](/docs/guides/config-files#global-settings).
  ...
  :::
   ...
   <Tabs>
     <TabItem value="configure" label="Using rook configure" default>
   ...
   <Tabs groupId="interface">
     <TabItem value="local" label="Local Recipe" default>
       ...
       **With Parameters** - Supply parameter values when running recipes. See the [`run` command documentation](/docs/guides/rook-cli-commands#run-options) for detailed examples and options.
       ...
       Basic example:
       ```sh
       rook run --recipe recipe.yaml --params language=Python
       ```

       **Slash Command** - Enter a [custom slash command](/docs/guides/context-engineering/slash-commands) in any rook chat session
      ...
      **Preview Recipe** - Use the [`explain`](/docs/guides/rook-cli-commands#run-options) command to view details before running:
       ...
       **Interactive Mode** - With parameter prompts:
       ```sh
       rook run --recipe code-review --interactive
       ```
       The interactive mode will prompt for required values:
       ```sh
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="rook CLI">
   ...
   ```sh
   rook recipe validate recipe.yaml
   ```
...
## Share Recipe
Share your recipe with rook users using a recipe link or recipe file.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="rook CLI">
    ...
    ```sh
    rook recipe deeplink <FILE>
    ```

    You can also provide parameter values to pre-fill the `Recipe Parameters` dialog:
    ```sh
    rook recipe deeplink <FILE> --param key1=value1 --param key2=value2
    ```
  </TabItem>
</Tabs>

When someone clicks the link, it will open rook Desktop with your recipe configuration. They can also use your recipe link to [import a recipe](/docs/guides/recipes/storing-recipes#importing-recipes) for future use.
...
<Tabs groupId="interface">
  ...
  In rook Desktop, you can export a recipe file or copy its content to share with others.
...
<Tabs groupId="interface">
  <TabItem value="ui" label="rook Desktop" default>
Automate rook recipes by running them on a schedule. When creating a schedule, you'll configure:
...
At the bottom of the `Schedule Details` page you can view the list of sessions created by the scheduled recipe and open or restore each session.
  ...
  <TabItem value="cli" label="rook CLI">
  Automate rook recipes by scheduling them to run with a [cron expression](https://en.wikipedia.org/wiki/Cron#Cron_expression).
...
2. rook calls a `final_output` tool with JSON matching your schema
3. Output is validated against the JSON schema
4. If validation fails, rook receives error details and must correct the output
...
rook run --recipe analysis.yaml --params project_path=./src > output.log
...
:::info
Structured output is supported in recipes run in both the rook CLI and rook Desktop. However, creating and editing the `json_schema` configuration must be done manually in the recipe file.
:::
...
To protect your privacy and system integrity, rook excludes:

- Global and local memory  
- API keys and personal credentials  
- System-level rook settings  


This means others may need to supply their own credentials or memory context if the recipe depends on those elements.

## Learn More
Check out the [Recipes](/docs/guides/recipes) guide for more docs, tools, and resources to help you master rook recipes.

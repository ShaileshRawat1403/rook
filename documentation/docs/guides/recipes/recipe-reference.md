...
Recipes are reusable rook configurations that package up instructions and settings so the setup can be easily shared and launched by others.
...
:::info
`.yml` files aren't supported by rook CLI.
:::
...
The `activities` field defines an optional message and clickable activity bubbles (buttons) that appears when a recipe is opened in rook Desktop.
...
| `bundled` | Boolean | (Optional) Whether the extension is bundled with rook |
...
- **`builtin`**: Built-in extension that is part of the bundled rook MCP server
...
If a recipe uses an extension that requires a secret, rook can prompt users to provide the secret when running the recipe:

1. When a recipe is loaded, rook scans all extensions (including those in subrecipes) for `env_keys` fields
2. If any required environment variables are missing from the secure keyring, rook prompts the user to enter them
...
The `required` and `optional` parameters work best for recipes opened in rook Desktop. If a value isn't provided for a `user_prompt` parameter, the parameter won't be substituted and may appear as literal `{{ parameter_name }}` text in the recipe output.
...
- `file`: The parameter value should be a file path. rook reads the file contents and substitutes the actual content (not the path) into the template
...
When a recipe with parameters is opened in rook Desktop, users are presented with a **Recipe Parameters** dialog where they can:
...
The `response` field enables recipes to enforce a final structured JSON output. When you specify a `json_schema`, rook will:
...
This feature is designed for **non-interactive automation** to ensure consistent, parseable output. Recipes can produce structured output when run from either the rook CLI or rook Desktop. See [use cases and ideas for automation workflows](/docs/guides/recipes/session-recipes#structured-output-for-automation).
...
:::note
Settings specified in a recipe will override your default rook configuration when that recipe is executed. If no settings are specified, rook will use your configured defaults.
:::
...
Recipes saved from rook Desktop include additional metadata fields. These fields are used by the Desktop app for organization and management but are ignored by CLI operations. 
...
When these occur, rook will provide helpful error messages indicating what needs to be fixed.

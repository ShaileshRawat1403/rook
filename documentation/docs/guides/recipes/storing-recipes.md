...
This guide covers storing, organizing, and finding rook recipes when you need to access them again later. 

:::info Desktop UI vs CLI
- **rook Desktop** has a visual Recipe Library for browsing and managing saved recipes
- **rook CLI** stores recipes as files that you find using file paths or environment variables
:::
...
<Tabs groupId="interface">
  <TabItem value="desktop" label="rook Desktop" default>
...
<Tabs groupId="interface">
  ...
  rook Desktop accepts `.yaml`, `.yml`, and `.json` files, but **the CLI only supports `.yaml` and `.json`**. For full compatibility across both interfaces, avoid `.yml` extensions.
  ...
  <TabItem value="cli" label="rook CLI">
    Recipe import is only available in rook Desktop.
  </TabItem>
</Tabs>
...
Use the `rook recipe list` command to find all available recipes from multiple sources:
...
:::tip
The `rook recipe list` command is the recommended way to find recipes as it automatically searches all configured sources and provides consistent formatting.
:::

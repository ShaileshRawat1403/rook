...
When the main recipe is run, rook generates a tool for each subrecipe that:
...
**Usage:**
```bash
rook run --recipe code-review-pipeline.yaml --params repository_path=/path/to/repo
```
...
**Usage:**
```bash
rook run --recipe smart-analyzer.yaml --params repository_path=/path/to/project
```
...
**Usage:**
```bash
rook run --recipe travel-planner.yaml
```

---
title: macOS Sandbox for rook Desktop
sidebar_label: Sandbox for rook Desktop
description: Optional sandboxing for rook Desktop to control file access, filter network traffic, and enforce security policies on macOS
---

rook Desktop includes an optional macOS sandbox that you can enable when you need stricter control and visibility over what rook can access on your system. Use it to:

- **Restrict file system access** — Block writes to SSH keys, shell configs, and rook configuration files
...
rook runs with full tool access, but the sandbox uses two layers of protection:

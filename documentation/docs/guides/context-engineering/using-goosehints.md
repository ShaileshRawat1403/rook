...
`.goosehints` is a text file used to provide additional context about your project and improve the communication with goose. The use of `.goosehints` ensures that rook understands your requirements better and can execute tasks more effectively.
...
<Tabs groupId="interface">
    ...
    7. Click `Save`
    8. Restart your session so rook can read the updated `.goosehints`
...
The `.goosehints` file supports natural language. Write clear, specific instructions using direct language that rook can easily understand and follow. Include relevant context about your project and workflow preferences, and prioritize your most important guidelines first.

rook loads hints at the start of your session. As it accesses files in nested directories, it also loads the hint files for those directories. rook adds hints to the system prompt for every request. Because `.goosehints` content uses tokens, keeping it concise can reduce cost and improve performance.
...
By default, rook looks for both `AGENTS.md` and `.goosehints` at each level. If you're using [custom context files](#custom-context-files), rook applies the same nested loading behavior to those filenames too.
...
rook looks for `AGENTS.md` then `.goosehints` files by default, but you can configure a different filename or multiple context files using the `CONTEXT_FILE_NAMES` environment variable. This is useful for:
...
Here's how it works:
1. rook looks for each configured filename in both global (`~/.config/goose/`) and local project locations

...
When working with the rook CLI, you can pass files and instructions to the `rook run` command to execute tasks and workflows. This could be a simple one-liner command or a complex set of instructions stored in a file.
...
If you don't want rook to exit at the end of the task, you can pass the `-s` or `--interactive` flag to start an interactive session after processing your initial commands:

```bash
rook run -i instructions.txt -s
```

This is useful when you want to continue working with rook after your initial commands are processed.
...
rook run -n my-project -t "initial instructions"

# Resume a previous session
rook run -n my-project -r
```
...
rook run --no-session -t "your command here"
```
### Set Provider and Model
You can run rook sessions with a specific provider and model, which overrides the provider and model settings in your [environment variables](/docs/guides/environment-variables.md).
...
```bash
rook run --with-builtin "developer,computercontroller" -t "your instructions"
```

- Using custom extensions

```bash
rook run --with-extension "ENV1=value1 custom-extension-args" -t "your instructions"
```

- Using streamable HTTP extensions

```bash
rook run --with-streamable-http-extension "https://example.com/streamable" -t "your instructions"
```
...
rook run --debug -t "your instructions"

# Debug a recipe execution
rook run --debug --recipe recipe.yaml
```

### JSON Output Format

For automation, scripting, and CI/CD integration, you can get structured output from `rook run` using the `--output-format` flag:
...
rook run --output-format json -t "your instructions"

# Stream JSON events in real-time
rook run --output-format stream-json -t "your instructions"

# Run a recipe with JSON output
rook run --output-format json --recipe recipe.yaml

# Combine with other options
rook run --output-format json --no-session -t "automated task"
```
...
```bash
rook run -t "Create a CHANGELOG.md entry comparing current git branch with main"
```
...
```bash
rook run --with-builtin "developer,git" -n dev-session -s
```

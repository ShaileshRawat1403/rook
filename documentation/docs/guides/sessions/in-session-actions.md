...
rook provides features you can use to manage conversations and share information during sessions.
...
- rook misunderstood your intent and went in the wrong direction
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="rook CLI">
        Message editing options are not available in the rook CLI.
    </TabItem>
</Tabs>
...
<Tabs groupId="interface">
    <TabItem value="ui" label="rook Desktop" default>
        ...
        rook creates a new session with conversation history up to and including your edited message. The new session is named "(edited)" and the original session remains unchanged.
    ...
    <TabItem value="cli" label="rook CLI">
    Message editing is not available in the rook CLI, but you can [duplicate entire sessions](/docs/guides/sessions/session-management#duplicate-sessions) using the `--fork` flag.
    </TabItem>
</Tabs>
...
Queue messages while rook is processing a task to manage your workflow. This is useful when:

- You want to prepare next steps while rook is working
...
:::tip
rook may perform better when complex tasks are split into subtasks, a technique called [*prompt chaining*](https://www.promptingguide.ai/techniques/prompt_chaining). This structured approach can both improve accuracy and give you more control over the process.
:::

<Tabs groupId="interface">
    <TabItem value="ui" label="rook Desktop" default>
      Add a message to the queue:
      1. While rook is processing a response, type your next message
      2. Press `Enter` to add it to the queue (or interrupts if using [interruption keywords](#interrupt-task))
      
      Queued messages appear as numbered cards showing the queue order. The first message in the queue is automatically sent when rook finishes each response.
      
      :::info Related Features
      - In general, pressing `Enter` while rook is processing a task queues the message, but clicking `Send` sends the task immediately and [interrupts the task](#interrupt-task)
      - When you type common interrupt keywords like "stop", "wait", or "hold on" in a queued message, rook pauses until you enter or send the next message and then continues processing the queue
      :::

      #### Queue Management Controls
    
      Queued messages run automatically in order as rook finishes each task, but you can manage the queue:
      ...
      **Without queuing:**
      ...
      This approach might lead to overwhelming responses where important details get missed or tasks are handled superficially. Even sending a single prompt with clear sequential steps doesn't allow rook to focus on each task individually or build context progressively.

      **With queuing:**

      1. You send: "Refactor the authentication code to support OAuth 2.0"
      2. While rook is working, you queue the following messages:
    ...
    <TabItem value="cli" label="rook CLI">
        Message queuing is not available in the rook CLI.
    </TabItem>
</Tabs>

## Interrupt Task

Interrupt rook while it's processing a task to take control of the conversation. This is useful when:

- rook is heading in the wrong direction
...
<Tabs groupId="interface">
    <TabItem value="ui" label="rook Desktop" default>
        ...
        rook stops processing the current task and asks for more information.
        ...
        rook stops processing the current task and pivots to the new request context.
        
        :::info Related features
        - Clicking `Send` while rook is processing a task interrupts the task but pressing `Enter` [queues the message](#queue-messages)
        - Typing a stop or pause keyword in a queued message also stops rook from processing the current task
    ...
    <TabItem value="cli" label="rook CLI">
        ...
        rook responds contextually to your new request.
        
    </TabItem>
</Tabs>

## Voice Dictation
Speak to rook directly instead of typing your prompts.
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="rook CLI">
        Voice dictation is not available in the rook CLI. 
    </TabItem>
</Tabs>

## Spellcheck

Spellcheck is enabled by default in the rook Desktop chat input field.
...
<Tabs groupId="interface">
    ...
    To disable or re-enable spellcheck:
        ...
        5. Restart rook for the change to take effect
        
    </TabItem>
    <TabItem value="cli" label="rook CLI">
        Spellcheck is not available in the rook CLI.
    </TabItem>
</Tabs>

## Share Files in Session

Provide rook with context from your codebase, documents, and other files to get more relevant and accurate assistance.

<Tabs groupId="interface">
    <TabItem value="ui" label="rook Desktop" default>
        Share files with rook in several ways:

...
In your first session, rook prompts you to [set up an LLM (Large Language Model) provider](/docs/getting-started/installation#set-llm-provider).
:::
...
<Tabs groupId="interface">
    <TabItem value="ui" label="rook Desktop" default>
        ...
        rook Desktop supports working with multiple active chat sessions in the same window. Your new sessions are added to the `Chat` section of the sidebar, so you can quickly switch between your 10 most recent sessions. Just click a session to [continue working](#resume-session) in that session.
        ...
        On macOS, you can also use the rook dock icon to quickly start sessions:
            - **Drag and drop** a folder onto the rook icon to open a new session in that directory
            - **Right-click** the rook icon and select `New Window` to open a new session in your most recent directory
        :::
        
        #### Keyboard Shortcuts
        
        You can also use keyboard shortcuts to start a new session or manage rook windows.
        ...
        | Focus rook Window | `Cmd+Option+G` | `Ctrl+Alt+G` |
        | Keep rook Window Always on Top | `Cmd+Shift+T` | `Ctrl+Shift+T` |
        ...
        The session opens to your most recently opened directory in a new rook window.
...
<Tabs groupId="interface">
    <TabItem value="ui" label="rook Desktop" default>
        A session's display name is auto-generated based on the context of your initial prompt. Session names help you identify rook sessions so you can switch between active sessions or [resume a session](#resume-session).
    ...
    <TabItem value="cli" label="rook CLI">
        rook sessions are given an auto-generated name based on the context of your initial prompt.
        ...
        ```sh
        rook session --name react-migration
        ```

        If you want to confirm the session name, run:

        ```sh
        rook session list -l 1
        ```
...
<Tabs groupId="interface">
  <TabItem value="ui" label="rook Desktop" default>
    ...
    You can use keyboard shortcuts and search bar buttons to search sessions in rook Desktop.
    ...
    :::tip Ask rook directly
    You can also use the built-in [Chat Recall extension](/docs/mcp/chatrecall-mcp) to ask rook to search your conversation history:
  ...
  <TabItem value="cli" label="rook CLI">
    ...
    To search conversation content across all your sessions, start a rook session and ask directly:

    - "Find my earlier conversation about React hooks from last week"
    - "Show me sessions where I worked on database migrations"

    rook will search your session history and show relevant conversations with context from matching sessions.
...
<Tabs groupId="interface">
    ...
    rook Desktop allows you to switch between multiple chat sessions in the same window. You can start a task in one session, switch to another to do some work, and return to the first session when the task is completed. 
    ...
    1. Click the <PanelLeft className="inline" size={16} /> button in the top-left to open the sidebar
    2. Click `View All` at the bottom of the `Chat` section
    3. Find the session you'd like to resume. rook provides [search features](#search-sessions) to help you find the session.
    ...
    <TabItem value="cli" label="rook CLI">
        ...
        ```
        rook session -r --name <name>
        ```
        For example, to resume the session named `react-migration`, you would run:

        ```
        rook session -r --name react-migration
        ```
    </TabItem>
</Tabs>

Sessions created in rook Desktop can be resumed in the CLI and vice versa. All sessions are stored in the [same database](/docs/guides/logs#session-records).
...
<Tabs groupId="interface">
    <TabItem value="ui" label="rook Desktop" default>
        ...
        Deleting a session from rook Desktop will also delete it from the CLI. This action cannot be undone.
        :::
...
<Tabs groupId="interface">
    <TabItem value="ui" label="rook Desktop" default>
        Import complete sessions from JSON files to restore, share, or migrate sessions between rook instances. Importing creates a new session with a new ID rather than overwriting existing sessions.

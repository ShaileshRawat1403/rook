...
When working with [Large Language Models (LLMs)](/docs/getting-started/providers), there are limits to how much conversation history they can process at once. rook provides smart context management features to help handle context and conversation limits so you can maintain productive sessions. Here are some key concepts:
...
- **Context Management**: How rook handles conversations approaching these limits
- **Turn**: One complete prompt-response interaction between rook and the LLM

## How rook Manages Context
rook uses a two-tiered approach to context management:
...
This layered approach lets rook handle token and context limits gracefully.
...
When you reach the auto-compaction threshold:
  1. rook will automatically start compacting the conversation to make room.
  2. Once complete, you'll see a confirmation message that the conversation was compacted and summarized.
  3. Continue the session. Your previous conversation remains visible, but only the compacted conversion is included in the active context for rook.

:::tip Customize Compaction
You can customize how rook summarizes conversations during compaction by editing the `compaction.md` [prompt template](/docs/guides/prompt-templates).
:::
...
<Tabs groupId="interface">
  ...
  4. Continue the session. Your previous conversation remains visible, but only the compacted conversion is included in the active context for rook.
...
<TabItem value="cli" label="rook CLI" default>
...
When auto-compaction is disabled, or if a conversation still exceeds the context limit, rook offers different ways to handle it:
...
rook Desktop exclusively uses summarization by compacting the conversation to manage context, preserving key information while reducing size.
...
## Maximum Turns
The `Max Turns` limit is the maximum number of consecutive turns that rook can take without user input (default: 1000). When the limit is reached, rook stops and prompts: "I've reached the maximum number of actions I can do without user input. Would you like me to continue?" If the user answers in the affirmative, rook continues until the limit is reached and then prompts again.
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="rook CLI">
      ...
      │  10
        // highlight-end
      │
      └  Set maximum turns to 10 - rook will ask for input after 10 consecutive actions
      ```

      :::tip
      In addition to the persistent `Max Turns` setting, you can provide a runtime override for a specific session or task via the `rook session --max-turns` and `rook run --max-turns` [CLI commands](/docs/guides/rook-cli-commands).
      :::
...
- **100+ turns**: More suitable for complex, multi-step automation where you trust rook to work independently, like "migrate this entire project from React 16 to React 18" or "implement comprehensive test coverage for this service"

Remember that even simple-seeming tasks often require multiple turns. For example, asking rook to "fix the failing tests" might involve analyzing test output (1 turn), identifying the root cause (1 turn), making code changes (1 turn), and verifying the fix (1 turn).

## Token Usage
After sending your first message, rook Desktop and rook CLI display token usage.
...
Context limits are automatically detected based on your model name, but rook provides settings to override the default limits:
...
- **LiteLLM Proxy Models**: When using LiteLLM with custom model names that don't match rook's patterns
...
<Tabs groupId="interface">
  <TabItem value="ui" label="rook Desktop" default>
     ...
     Model context limit overrides are not yet available in the rook Desktop app.
...
rook monitors your API provider balance and warns you when credits are running low or exhausted. When this happens, you'll see an **Insufficient Credits** notification.
...
:::tip
rook detects low balance conditions automatically, so you won't lose your conversation context—just add credits and pick up where you left off.
:::

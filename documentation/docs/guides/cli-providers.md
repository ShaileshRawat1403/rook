...
- already have a Claude Code, Codex, Cursor, or Google Gemini CLI subscription and want to use it through rook instead of paying per token
- need session persistence to save, resume, and export conversation history
- want to use rook recipes and scheduled tasks to create repeatable workflows
...
- **Recipe compatibility**: Use CLI providers in automated rook recipes
...
- Automatic filtering of rook extensions from system prompts (since Claude Code has its own tool ecosystem)
...
- Automatic filtering of rook extensions from system prompts
...
Once configured, you can start a rook session using these providers just like any others:
...
This provides a consistent permission experience across all rook providers while leveraging Claude Code's built-in safety checks.
...
- **Claude Code**: Converts rook messages to text content blocks with role prefixes (Human:/Assistant:), similar to Codex and Gemini CLI
- **Codex**: Converts messages to simple text prompts with role prefixes (Human:/Assistant:), similar to Gemini CLI
- **Cursor Agent**: Converts rook messages to Cursor's JSON message format, handling tool calls and responses appropriately

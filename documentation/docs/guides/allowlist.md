---
sidebar_position: 90
title: rook Extension Allowlist
sidebar_label: Extension Allowlist
---

rook is an extensible framework that, by default, allows you to install any MCP server. However, you may want stricter controls on which MCP servers can be installed as extensions (e.g. in a corporate setting). 

This guide explains how you can create an **allowlist** of safe extensions that work with rook Desktop and CLI. An allowlist lets administrators control which MCP servers can be installed as rook extensions. When enabled, rook will only install extensions that are on the list, and will block installation of any others.

## How It Works

1. The allowlist is a YAML file that contains a list of allowed extension commands.
2. rook fetches the allowlist from a URL specified by the `ROOK_ALLOWLIST` environment variable.
...
Create an environment variable called `ROOK_ALLOWLIST` and set the value to the URL of your YAML file:

```bash
export ROOK_ALLOWLIST=https://example.com/rook-allowlist.yaml
```
...
1. Check if the `ROOK_ALLOWLIST` environment variable is set correctly.

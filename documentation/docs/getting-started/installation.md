...
<Tabs>
  <TabItem value="mac" label="macOS" default>
    Choose to install the Desktop and/or CLI version of rook:

    <Tabs groupId="interface">
      <TabItem value="ui" label="rook Desktop" default>
        Install rook Desktop directly from the browser or with [Homebrew](https://brew.sh/).
        ...
        Homebrew downloads the [same app](https://github.com/Homebrew/homebrew-cask/blob/master/Casks/b/block-rook.rb) but can take care of updates too.
        ```bash
          brew install --cask block-rook
        ```
        ---
        <div style={{ marginTop: '1rem' }}>
          :::info Permissions
          If you're on an Apple Mac M3 and the rook Desktop app shows no window on launch, check and update the following:

          Ensure the `~/.config` directory has read and write access.

          rook needs this access to create the log directory and file. Once permissions are granted, the app should load correctly. For steps on how to do this, refer to the  [Known Issues Guide](/docs/troubleshooting/known-issues#macos-permission-issues)
          :::
        </div>
      </TabItem>
      <TabItem value="cli" label="rook CLI">
        Install rook directly from the browser or with [Homebrew](https://brew.sh/).
        ...
        :::tip Updating rook
        It's best to keep rook updated. To update rook, run:
        ```sh
        rook update
        ```
        :::
        ...
        <h3>Option 2: Install via Homebrew</h3>
        Homebrew downloads the [a precompiled CLI tool](https://github.com/Homebrew/homebrew-core/blob/master/Formula/b/block-rook-cli.rb) and can take care of updates.
        ```bash
        brew install block-rook-cli
        ```
      </TabItem>
    </Tabs>
  </TabItem>
  ...
  <TabItem value="linux" label="Linux">
    ...
    <Tabs groupId="interface">
      <TabItem value="ui" label="rook Desktop" default>
        Install rook Desktop directly from the browser.
  ...
  <TabItem value="windows" label="Windows">
    ...
    <Tabs groupId="interface">
      ...
      <TabItem value="cli" label="rook CLI">
        To install rook natively on Windows, you need one of the following environments:
        ...
        **PowerShell Installation:**
        ...
        If you see a warning that rook is not in your PATH, you need to add rook to your PATH:
        ...
        <summary>Install via Windows Subsystem for Linux (WSL)</summary>
          ...
          We recommend running the rook CLI natively on Windows, but you can use WSL if you prefer a Linux-like environment.
...
## Set LLM Provider
rook works with [supported LLM providers][providers] that give rook the AI intelligence it needs to understand your requests. On first use, you'll be prompted to configure your preferred provider.

<Tabs groupId="interface">
  <TabItem value="ui" label="rook Desktop" default>
    On the welcome screen the first time you open rook, choose how to configure a provider:
  ...
  <TabItem value="cli" label="rook CLI">
    ...
    [rook opens the browser and prints details]
  ...
  Then run `rook configure` again. rook will detect the environment variable and display:
...
<Tabs groupId="interface">
    <TabItem value="ui" label="rook Desktop" default>
        Starting a session in the rook Desktop is straightforward. After choosing your provider, you'll see the session interface ready for use.

        Type your questions, tasks, or instructions directly into the input field, and rook will get to work immediately.
    </TabItem>
    <TabItem value="cli" label="rook CLI">
...
## Pin a rook version in CI/CD
In CI/CD (and other automated, non-interactive environments), pin a specific version with `ROOK_VERSION` to make installs reproducible and avoid 404s when downloading the rook CLI binary assets if the `stable` release tag doesn’t include them.
...
If you're packaging rook for a Linux distribution or creating a custom build, you can generate Unix manpages from the CLI command definitions:
...
You can also configure Extensions to extend rook's functionality, including adding new ones or toggling them on and off. For detailed instructions, visit the [Using Extensions Guide][using-extensions].

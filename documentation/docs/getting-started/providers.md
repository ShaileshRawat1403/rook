...
rook is compatible with a wide range of LLM providers, allowing you to choose and integrate your preferred model.
...
|-----------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Amazon Bedrock](https://aws.amazon.com/bedrock/)                           | Offers a variety of foundation models, including Claude, Jurassic-2, and others. **AWS environment variables must be set in advance, not configured through `rook configure`**                                           | Credential auth: `AWS_PROFILE`, or `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`<br /><br />Bearer token auth: `AWS_BEARER_TOKEN_BEDROCK` and `AWS_REGION`, `AWS_DEFAULT_REGION`, or `AWS_PROFILE` |
...
| [OpenAI](https://platform.openai.com/api-keys)                              | Provides gpt-4o, o1, and other advanced language models. Also supports OpenAI-compatible endpoints (e.g., self-hosted LLaMA, vLLM, KServe). **o1-mini and o1-preview are not supported because rook uses tool calling.** | `OPENAI_API_KEY`, `OPENAI_HOST` (optional), `OPENAI_ORGANIZATION` (optional), `OPENAI_PROJECT` (optional), `OPENAI_CUSTOM_HEADERS` (optional)                                       |
...
| [Ramalama](https://ramalama.ai/)                                            | Local model using native [OCI](https://opencontainers.org/) container runtimes, [CNCF](https://www.cncf.io/) tools, and supporting models as OCI artifacts. Ramalama API is a compatible alternative to Ollama and can be used with the rook Ollama provider. Supports Qwen, Llama, DeepSeek, and other open-source models. **Because this provider runs locally, you must first [download and run a model](#local-llms).**  | `OLLAMA_HOST`                                                                                                                                                                       |
...
rook supports [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) agents as providers. ACP providers pass rook extensions through to the agent as MCP servers.

| Provider                                                                    | Description                                                                                                                                                                                                               | Requirements                                                                                                                                                                          |
|-----------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Claude ACP](https://github.com/zed-industries/claude-agent-acp) (`claude-acp`) | Uses Claude Code via ACP. Passes rook extensions to the agent as MCP servers. | `npm install -g @zed-industries/claude-agent-acp`, active Claude Code subscription |
| [Codex ACP](https://github.com/zed-industries/codex-acp) (`codex-acp`) | Uses OpenAI Codex via ACP. Passes rook extensions to the agent as MCP servers. | `npm install -g @zed-industries/codex-acp`, active ChatGPT Plus/Pro subscription |
...
To configure your chosen provider, see available options, or select a model, visit the `Models` tab in rook Desktop or run `rook configure` in the CLI.
...
<Tabs groupId="interface">
  ...
  **First-time users:**
  ...
  <Tabs groupId="setup">
    ...
    3. rook will automatically detect your provider and configure the connection.
    ...
    2. rook will open a browser window for you to sign in with the credentials of your active ChatGPT Plus or Pro subscription.
    3. Authorize rook to access your ChatGPT subscription.
    4. When you return to rook Desktop, you're ready to begin your first session.
    </TabItem>
    ...
    3. When you return to rook Desktop, you're ready to begin your first session.
    </TabItem>

    <TabItem value="openrouter" label="OpenRouter">
    1. Choose `Automatic setup with OpenRouter`. 
    2. rook will open a browser window for you to authenticate with OpenRouter, or create a new account if you don't have one already.
    3. When you return to the rook Desktop, you're ready to begin your first session.
    </TabItem>
...
<Tabs groupId="interface">
  <TabItem value="ui" label="rook Desktop" default>
    ...
    :::info Custom Headers
    Currently, custom headers can't be defined in rook Desktop. As a workaround, edit the provider configuration file after creation.
    :::
  ...
  <TabItem value="config" label="Config File">
    ...
    rook session start --provider custom_corp_api
    ```
...
Your changes are available in your next rook session.
...
<Tabs groupId="interface">
  ...
  <TabItem value="config" label="Config File">
    ...
    :::tip
    If the provider's API key is stored in the keychain, use rook CLI to remove the custom provider. This also removes the stored API key.
    :::
...
rook is a free and open source AI agent that you can start using right away, but not all supported [LLM Providers][providers] provide a free tier. 

Below, we outline a couple of free options and how to get started with them.

:::warning Limitations
These free options are a great way to get started with rook and explore its capabilities. However, you may need to upgrade your LLM for better performance.
:::
...
<Tabs groupId="local-llms">
    ...
    <Tabs groupId="ollama-models">
      <TabItem value="ramalala" label="Ramalala">
        ...
        2. In a terminal, run any Ollama [model supporting tool-calling](https://ollama.com/search?c=tools) or [GGUF format HuggingFace Model](https://huggingface.co/search/full-text?q=%22tools+support%22+%2B+%22gguf%22&type=model):
          ...
          The `--runtime-args="--jinja"` flag is required for Ramalama to work with the rook Ollama provider.
  ...
  <TabItem value="lmstudio" label="LM Studio">
    ...
    4. Configure rook to use LM Studio:
    ...
    :::tip Model Name
    Make sure the model name you enter in rook matches the model identifier shown in LM Studio's server panel.
    :::
  </TabItem>
  <TabItem value="docker" label="Docker Model Runner" default>
    ...
    4. Configure rook to use Docker Model Runner, using the OpenAI API compatible endpoint: 
...
1. Run [`rook configure`](#configure-provider-and-model) and select **GitHub Copilot**
...
rook supports two authentication methods for Azure OpenAI:
...
- Configure with `rook configure` and select Azure OpenAI, leaving the API key field empty

This method simplifies authentication and enhances security for enterprise environments.

## Multi-Model Configuration

Beyond single-model setups, rook supports [multi-model configurations](/docs/guides/multi-model/) that can use different models and providers for specialized tasks:
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="rook CLI">
    **Interactive configuration:**
    ...
    When you run `rook configure` and select a Gemini 3 model, you'll be prompted to choose a thinking level:
...
Some models expose their internal reasoning or "chain of thought" as part of their response. rook automatically captures this reasoning output and makes it available to you. The following models and providers support reasoning output:

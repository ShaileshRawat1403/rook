...
Extensions are add-ons that provide a way to extend the functionality of rook by connecting with applications and tools you already use in your workflow. These extensions can be used to add new features, access data and resources, or integrate with other systems.

Extensions are based on the [Model Context Protocol (MCP)](https://github.com/modelcontextprotocol), so you can connect
rook to a wide ecosystem of capabilities.

rook automatically checks external extensions for known malware before activation. If a malicious package is detected, the [extension will be blocked](/docs/troubleshooting/known-issues#malicious-package-detected) with a clear error message.

:::tip Tutorials
Check out the [step-by-step tutorials](/docs/category/mcp-servers) for adding and using various rook extensions
:::

## Built-in Extensions
rook includes several built-in extensions you can start using out of the box:
...
rook provides a [central directory][extensions-directory] of extensions that you can install and use. 

You can also add any other [MCP Server](#mcp-servers) as a rook extension, even if it's not listed in our directory.
...
You can install any MCP server as a rook extension. 

Many MCP servers work better when they understand your active workspace. rook supports [MCP Roots](/docs/guides/mcp-roots), which lets roots-aware extensions see your current session working directory automatically.
...
<Tabs groupId="interface">
  ...
  4. On the `Add custom extension` modal, enter the necessary details
     ...
     - The `Timeout` field lets you set how long rook should wait for a tool call from this extension to complete
  5. Click `Add` button
...
The Smart Extension Recommendation system in rook automatically identifies and suggests relevant extensions based on your tasks and needs. This section explains how to use this feature effectively and understand its capabilities and limitations.

When you request a task, rook checks its enabled extensions and their tools to determine if it can fulfill the request. If not, it suggests or enables additional extensions as needed. You can also request specific extensions by name.
...
rook automatically detects when an extension is needed based on your task requirements. Here's an example of how rook identifies and enables a needed extension during a conversation:
...
■  rook would like to enable the following extension, do you approve?
// highlight-start
...
rook responds to explicit requests for extensions, allowing users to manually enable specific tools they need. Here's an example of how rook handles a direct request to enable an extension:
...
rook relies on extension properties to determine how to handle an extension. You can edit these properties if you want to change the extension's display settings and behavior, such as the name, timeout, or environment variables.
...
You can start a tailored rook session with specific extensions directly from the CLI. 
...
```bash
rook session --with-builtin "{extension_id}"
```

For example, to enable the Developer and Computer Controller extensions and start a session, you'd run:

```bash
rook session --with-builtin "developer,computercontroller"
```

Or alternatively:

```bash
rook session --with-builtin developer --with-builtin computercontroller
```
...
```bash
rook session --with-extension "{extension command}" --with-extension "{another extension command}"
```
...
```bash
rook session --with-extension "uvx mcp-server-fetch"
```
...
```bash
rook session --with-extension "VAR=value command arg1 arg2"
```

For example, to start a session with the [GitHub extension](https://github.com/github/github-mcp-server), you'd run:

```bash
rook session --with-extension "GITHUB_PERSONAL_ACCESS_TOKEN=<YOUR_TOKEN> npx -y @modelcontextprotocol/server-github"
```
...
```bash
rook session --with-streamable-http-extension "{extension URL}" --with-streamable-http-extension "{another extension URL}"
```

For example, to start a session with a Streamable HTTP extension, you'd run:

```bash
rook session --with-streamable-http-extension "https://example.com/streamable"
```
...
rook extensions are implemented with MCP, a standard protocol that allows AI models and agents to securely connect with local or remote resources. Learn how to build your own [extension as an MCP server](https://modelcontextprotocol.io/quickstart/server).

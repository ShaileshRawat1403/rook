...
rook recipes can execute multiple [subrecipe](/docs/guides/recipes/subrecipes) instances concurrently using isolated worker processes. This feature enables efficient batch operations, parallel processing of different tasks, and faster completion of complex workflows.
...
Parallel subrecipe execution uses an isolated worker system that automatically manages concurrent task execution. rook creates individual tasks for each subrecipe instance and distributes them across up to 10 concurrent workers.
...
When running different subrecipes, rook determines the execution mode based on:
...
When running the same subrecipe with different parameters, rook determines the execution mode based on:
...
If your prompt implies multiple executions of the same subrecipe, rook will automatically create parallel instances:

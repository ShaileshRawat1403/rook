...
description: Use rook with the Playwright CLI to generate automated tests using natural language
---
...
With the [Playwright CLI](https://github.com/microsoft/playwright-cli), rook can navigate websites, click buttons, fill forms, and turn those interactions into Playwright tests, all from natural language. Unlike the Playwright MCP, which sends the full page structure to the LLM on every request, Playwright CLI stores the accessibility tree locally. That means faster responses, lower costs, and no issues with large pages.

## Why as a Skill?

LLMs may not be trained on Playwright's CLI, so when asking an agent to use it, it may hallucinate commands and arguments resulting in errors and wasted tokens. The [Playwright CLI Skill](https://github.com/microsoft/playwright-cli/blob/main/skills/playwright-cli/SKILL.md) teaches rook how to use the CLI and when to invoke specific commands.
...
rook can even run the test for you to make sure it works as expected. If Playwright is already set up, just ask it to run the test. If not, rook can install Playwright for you and then run the test.
...
rook will use the CLI to open the recorded video, so you can see exactly what happened during the session.
...
When you have rook running several browser tasks at once, it can be hard to keep track of what's happening. The visual dashboard gives you a bird's eye view of all your active browser sessions, letting you watch progress in real time or jump in and take control when needed.

```
Show playwright dashboard
```

From here you can see live previews of every browser rook is controlling. Click into any session to watch it full-size, or take over the mouse and keyboard yourself if rook needs a hand. Press `Escape` when you're done and rook picks up right where you left off.

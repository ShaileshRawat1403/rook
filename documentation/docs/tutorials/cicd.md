...
description: Set up rook in your CI/CD pipeline to automate tasks
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

rook isn’t just useful on your local machine, it can also streamline tasks in CI/CD environments. By integrating rook into your pipeline, you can automate tasks such as:
...
This guide walks you through setting up rook in your CI/CD pipeline, with a focus on using GitHub Actions for code reviews.


## Using rook with GitHub Actions
You can run rook directly within GitHub Actions. Follow these steps to set up your workflow.
...
jobs:
   rook-comment:
      ...
      steps:
         ...
         - name: Run rook and filter output
           run: |
              rook run --instructions instructions.txt | \
...
- Grants the necessary permissions for rook to interact with the repository
...
#### Pinning rook versions in CI/CD

In CI/CD, we recommend pinning a specific rook version with `ROOK_VERSION` for reproducible runs. This also avoids 404 errors when downloading the rook CLI binary assets if the `stable` release tag doesn’t include them.
...
:::info Replacements
Replace `REPLACE_WITH_VERSION`, `REPLACE_WITH_PROVIDER`, and `REPLACE_WITH_MODEL` with the rook version you want to pin and your LLM provider/model names. Add any other necessary configuration required.
:::
...
### 5. Run rook and Clean Output

Now, run rook with the formatted instructions and clean the output by removing ANSI color codes and unnecessary log messages.
...
Finally, post the rook output as a comment on the pull request:
...
With this workflow, rook will run on pull requests, analyze the changes, and post a summary as a comment on the PR.

This is just one example of what's possible. Feel free to modify your GitHub Action to meet your needs.

---

## Running Multiple rook Instances in Parallel

rook supports running multiple concurrent sessions with isolated state, making it safe to run parallel jobs in your CI/CD pipeline. Each rook instance maintains its own conversation history, agent context, and extension configurations without interference.
...
When running rook in a CI/CD environment, keep these security practices in mind:
...
3. **Input Validation**
      - Ensure any inputs passed to rook are sanitized and validated to prevent unexpected behavior.

...
description: Create interactive UI applications that render inside rook Desktop
---
...
MCP Apps support in rook is experimental and based on a draft specification. The implementation is minimal and may change, and does not yet support advanced capabilities or persistent app windows.
:::
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "show_demo_app") {
    return {
      ...
      // This metadata tells rook to render the MCP App
      _meta: {
        ui: {
...
<body class="light">
  <div class="container">
    ...
    <div class="info-section">
      ...
      It communicates with rook via JSON-RPC messages through the sandbox bridge.
      <br><br>
...
- Switching rook between light/dark mode
...
└──────────────────┬───────────────────┘
                   │ postMessage
┌──────────────────▼───────────────────┐
│          rook Desktop               │  Renders UI, routes messages
└──────────────────┬───────────────────┘
                   │ MCP Protocol
┌──────────────────▼───────────────────┐
...
Your server returns a `ui://` resource URI, rook fetches the HTML and renders it in an iframe. The app communicates back via `postMessage`—requesting theme info, sending messages to chat, or resizing itself.

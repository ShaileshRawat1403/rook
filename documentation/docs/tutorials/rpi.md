...
```sh
mkdir -p ~/.config/rook/recipes/subrecipes

curl -sL https://raw.githubusercontent.com/aaif-rook/rook/main/documentation/src/pages/recipes/data/recipes/rpi-research.yaml -o ~/.config/rook/recipes/rpi-research.yaml
curl -sL https://raw.githubusercontent.com/aaif-rook/rook/main/documentation/src/pages/recipes/data/recipes/rpi-plan.yaml -o ~/.config/rook/recipes/rpi-plan.yaml
curl -sL https://raw.githubusercontent.com/aaif-rook/rook/main/documentation/src/pages/recipes/data/recipes/rpi-implement.yaml -o ~/.config/rook/recipes/rpi-implement.yaml
curl -sL https://raw.githubusercontent.com/aaif-rook/rook/main/documentation/src/pages/recipes/data/recipes/rpi-iterate.yaml -o ~/.config/rook/recipes/rpi-iterate.yaml

curl -sL https://raw.githubusercontent.com/aaif-rook/rook/main/documentation/src/pages/recipes/data/recipes/subrecipes/rpi-codebase-locator.yaml -o ~/.config/rook/recipes/subrecipes/rpi-codebase-locator.yaml
curl -sL https://raw.githubusercontent.com/aaif-rook/rook/main/documentation/src/pages/recipes/data/recipes/subrecipes/rpi-codebase-analyzer.yaml -o ~/.config/rook/recipes/subrecipes/rpi-codebase-analyzer.yaml
curl -sL https://raw.githubusercontent.com/aaif-rook/rook/main/documentation/src/pages/recipes/data/recipes/subrecipes/rpi-pattern-finder.yaml -o ~/.config/rook/recipes/subrecipes/rpi-pattern-finder.yaml
```
</details>
...
In rook, we use a structured RPI workflow using recipes to systematically tackle complex codebase changes. The workflow consists of slash commands that guide rook through disciplined phases of work:
...
```
/research_codebase "look through the cloned rook repo and research how the LLM Tool Discovery is implemented"
```


This command invokes the **[RPI Research Codebase](https://raw.githubusercontent.com/aaif-rook/rook/refs/heads/main/documentation/src/pages/recipes/data/recipes/rpi-research.yaml)** recipe, whose job is very strict:
...
With this recipe, rook automatically spawns three parallel subagents:
...
After rook began researching, I noticed that it was researching "tool discovery" in general. But I only wanted to remove a specific feature called Tool Selection Strategy. So I stopped rook and reran research with a more accurate topic.

That wasn't a failure. In fact, this is exactly why research exists. Had I told rook to "remove the LLM Tool Discovery feature", it may have removed our other tool discovery methods as well. Fortunately, catching these types of mistakes early is cheap and easy to recover from.
:::
...
The **[RPI Create Plan](https://raw.githubusercontent.com/aaif-rook/rook/refs/heads/main/documentation/src/pages/recipes/data/recipes/rpi-plan.yaml)** recipe starts by reading the research document rook created.
...
2. **Presented design options**
    ...
    Where there were multiple reasonable approaches, rook laid them out and asked me to choose.
...
Again, you as the human need to step in here to review the plan and make sure it's solid. If there's anything amiss, instead of starting over you can run the **[RPI Iterate Plan](https://raw.githubusercontent.com/aaif-rook/rook/refs/heads/main/documentation/src/pages/recipes/data/recipes/rpi-iterate.yaml)** plan (`/iterate_plan`) with details on what's wrong. rook will then read the existing plan, research only what needs rethinking, propose targeted updates, and edit the plan accordingly.
...
The **[RPI Implement Plan](https://raw.githubusercontent.com/aaif-rook/rook/refs/heads/main/documentation/src/pages/recipes/data/recipes/rpi-implement.yaml)** recipe is intentionally boring. In fact, I fell asleep while rook was running it. Implementation should feel mechanical. If it feels creative, something upstream is missing. But knowing that you have a rock solid plan, I advise you to go do something else with your time while rook works (unless there are manual steps in the plan).
...
That last bit was really helpful because my context window filled up partway through and rook was able to compact and pick up right where it left off because of the status updates in the plan.

## Final Result

For 10 phases of work that spanned 32 files, the Research phase took 9 minutes, the Plan phase took 4 minutes, and the Implement phase took 39 minutes. So in total, this took just shy of an hour... 52 minutes to be exact. This included rook working and testing as well as me answering questions.

Definitely not a fast process. BUT! When I put up [this PR](https://github.com/aaif-rook/rook/pull/6250), the build passed and the separate Code Review Agent didn't have a single comment. That's just how well done the work was.

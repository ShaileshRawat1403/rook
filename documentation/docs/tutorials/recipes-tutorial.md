---
description: Learn how to create and use rook recipes with this comprehensive tutorial covering prompts, parameters, and MCP servers
---

# Recipes

rook recipes are files that contain all the details to allow rook to do one specific task. Since they are contained in just one file, they are easy to share through all the normal ways we share files, including version management systems like git. Let's get started with the simplest recipe possible.
...
```sh
rook run --recipe trip.yaml
```

## Extensions

rook recipes have a section where you can specify which [extensions](/docs/guides/recipes/recipe-reference#extensions) rook can use during execution. rook will only use the ones you specify. 

Let's say we want to make sure we have good weather during our Europe trip. We can just add a weather extension (this example uses the [weather-mcp-server](https://github.com/TuanKiri/weather-mcp-server) by TuanKiri under the MIT License) to our recipe, modify the prompt a bit and now rook will check the weather before adding a city to our trip.

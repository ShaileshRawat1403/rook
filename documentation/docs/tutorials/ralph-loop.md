---
title: Ralph Loop
description: Run rook in a loop with fresh context per iteration and cross-model review
---

Ralph Loop, based on [Geoffrey Huntley's Ralph Wiggum technique](https://ghuntley.com/ralph/), is an iterative development pattern that keeps rook working on a task until it's genuinely complete.
...
- [Install the rook CLI](/docs/getting-started/installation) because the Ralph Loop runs via the terminal.
...
The terminal will show rook moving through the worker and reviewer phases. Each iteration starts with a fresh session to keep the context clean. Here's what a successful run looks like:
...
▶ WORK PHASE
... (rook creates initial implementation) ...

▶ REVIEW PHASE
... (rook reviews the work) ...

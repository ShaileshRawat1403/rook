...
You'll use rook to orchestrate a full software team of subagents:
...
You'll be building an AI-powered briefing app by spinning up a team of subagents to help you. Each agent has a clear role. Your job is to figure out how to prompt rook to delegate the work. 
...
<details>
  ...
  ```md
  You're the Planner agent for a hands-on AI app building session using rook and subagents. We are building the MVP *right now*.
...
Now that development is done, spin up two final subagents: a **QA Engineer** and a **Tech Writer**. They'll work together to assess the app's quality and document how to use it. Your job is to prompt rook in a way that gets both agents to collaborate without overlapping or duplicating work.
...
<details>
  ...
  - 🧪 A **QA Agent** who will:
    ...
    - Create a simple mock that returns fake data instead of calling the real rook CLI
    - Assert that the response includes: `title`, `date`, and 2–3 `takeaways`
    - Include tests for:
      - Valid topic input
      - Missing or invalid input
      - rook CLI timeout or error
...
**Congratulations! You've built a full-stack AI app using rook subagents!** 🎉

:::warning
Don't expect your app to be production ready. This workshop shows how vibe coding with rook can accelerate prototyping, but the human still owns the judgment and polish.
:::
...
### Process Timeout
**Cause**: rook taking too long or hanging
**Solutions**: 
1. **Check your rook command flags**:
   ```javascript
   ['run', '-t', prompt, '--quiet', '--no-session', '--max-turns', '1']
   ```
2. **Test rook manually**:
   ```bash
   rook run -t "Return JSON: {\"test\": \"value\"}" --quiet --no-session --max-turns 1
   ```
...
### JSON Parsing Errors
**Cause**: rook returns formatted output with color codes
**Solutions**:
...
2. **Add debug logging** to see what you're getting:
   ```javascript
   console.log('Raw rook response:', aiResponse);

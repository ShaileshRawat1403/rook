...
This guide covers how to use the `rook bench` command to run benchmarks and analyze results.
...
```bash
rook bench selectors
```

2. Create a basic configuration file:

```bash
rook bench init-config -n bench-config.json
...
```bash
rook bench run -c bench-config.json
```
...
```bash
RUST_LOG=debug rook bench bench-config.json
```

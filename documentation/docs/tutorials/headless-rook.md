---
title: Using rook in Headless Mode for Automation
description: rook Headless Mode
---

# Automate Development Tasks with rook Headless Mode
...
rook run --with-builtin developer -t "Check system logs for errors in the last 24 hours, identify performance bottlenecks, and generate a maintenance report"

# Container optimization
rook run --no-session -t "Analyze the Dockerfile, optimize for smaller image size, and update the build process documentation"

# Cloud resource audit
rook run -t "Review our AWS infrastructure configuration, identify cost optimization opportunities, and create a migration plan for underutilized resources"
```
...
- name: AI-Powered Code Review
  run: |
    rook run --with-builtin developer \
...
rook run --debug -t "Examine the failing test suite, identify the root cause of failures, implement fixes, and ensure all tests pass"

# Automated documentation updates
rook run -t "Review code changes and update the README.md and API documentation to reflect new features and modifications"
```
...
for repo in repo1 repo2 repo3; do
  cd $repo
  rook run --no-session -t "Update all dependencies to latest stable versions, run tests, and create a PR if changes are needed"
  cd ..
done

# Database migration automation
rook run -t "Analyze the current database schema, generate migration scripts for the new requirements, and create rollback procedures"
```
...
0 2 * * * /usr/local/bin/rook run --no-session -t "Run comprehensive security audit, check for vulnerabilities, and email report to security team"

# Weekly dependency updates
0 9 * * 1 /usr/local/bin/rook run -t "Check for outdated dependencies, create update PRs for non-breaking changes, and schedule review for major updates"
```
...
rook run -t "Analyze the test failures in the latest CI run, identify the root cause, and create a fix with appropriate unit tests"

# Better: Even more detailed
rook run -t "Review the failed tests in tests/integration/, identify why the authentication middleware is failing, implement a fix that maintains backward compatibility, and add regression tests"
```
...
export ROOK_MAX_TURNS=50
export ROOK_MODE=auto
export ROOK_DISABLE_SESSION_NAMING=true
```

The `CONTEXT_STRATEGY` and `MAX_TURNS` settings help manage conversation limits, while `ROOK_MODE` set to `auto` allows for non-interactive execution. `ROOK_DISABLE_SESSION_NAMING` avoids the extra background model call used to generate a session name and keeps the default "CLI Session" name.
...
```bash
#!/bin/bash
if ! rook run --no-session -t "Run security audit and fix critical issues"; then
    echo "rook automation failed - manual intervention required"
    exit 1
fi
```
...
rook run --recipe automation-recipe.yaml
...
rook run --recipe main-workflow.yaml \
...
**What this means**: rook cannot ask for clarification, approval, or additional input during execution. If it's unsure of what to do, the prompt result will usually show you a question like "How should I proceed?".

**Impact**: If instructions are ambiguous or if unexpected situations arise, rook will make its best judgment based on available context, which might not always align with your intentions.
...
rook run -t "Fix the issues"

# Better: Specific and actionable
rook run -t "Fix the TypeScript compilation errors in src/components/, ensure all imports are correct, and update any deprecated API calls to use the latest syntax"
```
...
**What this means**: rook cannot prompt for permission to use potentially risky tools or operations.
...
```bash
export ROOK_MODE=auto  # Automatically approve safe operations
# or configure specific tool permissions in your config
```

### 4. Context Decision Automation

**What this means**: When conversation context limits are reached, rook automatically applies the configured strategy without user input.
...
```bash
export ROOK_CONTEXT_STRATEGY=summarize  # Usually the best choice for automation
export ROOK_MAX_TURNS=100  # Prevent runaway execution
```
...
export ROOK_MODEL=gpt-4o

# Output control
export ROOK_CLI_MIN_PRIORITY=0.2  # Reduce verbose output
```
...
export ROOK_ALLOWLIST=https://company.com/allowed-extensions.json
```

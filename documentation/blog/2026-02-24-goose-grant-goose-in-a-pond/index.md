...
authors:
  - angie
image: /img/blog/rook-grant-rook-in-a-pond.png
---

![blog banner](/img/blog/rook-grant-rook-in-a-pond.png)

We launched the [rook grant program](/grants/) awarding $100K grants for developers building the future of agentic AI. We're looking for ambitious, open source projects that push rook into new territory, and today, We're thrilled to introduce one of our grant recipients: **Goose In A Pond**, a project that's taking rook off the desktop and into your home.

<!--truncate-->

## What Is Goose In A Pond?

Goose In A Pond is a fully local, privacy-first smart home assistant built on top of rook. Think of it as what your smart speaker *should* be: an AI assistant that actually runs on your hardware, understands your voice offline, controls your devices, and never sends your data to the cloud.

The project is being built by [Jarida](https://jarida-io.web.app/), a team of five developers based in Nairobi, Kenya, led by [Jerry Ochieng](https://linkedin.com/in/jerry-ochieng). They're taking rook's open source agent framework and deploying it on edge hardware, specifically the NVIDIA Jetson Orin Nano, to create a modular, agentic home hub that you fully own and control.
...
One of the coolest parts of this project is the fully offline voice pipeline. Using rook's experimental [Perception extension](https://github.com/michaelneale/rook-perception) as a foundation, the system listens for a wake word ("rook," naturally 🪿), then switches to a higher quality transcription mode to capture what you say. Wake word detection, speech recognition, and text-to-speech all run locally using open source models like Whisper, Vosk, and Coqui TTS.
...
They're essentially building rook into a universal remote for your entire home - open or closed ecosystem, it doesn't matter.

### A Self-Improving Assistant

Goose In A Pond isn't just a static tool. Through rook's [Memory extension](/docs/mcp/memory-mcp) and a feedback loop system, it learns your preferences, adapts its behavior, and refines its own prompts over time. The team is also exploring self-refinement techniques where the system analyzes its own session logs to optimize its automation behavior. It's the kind of agent that gets better the more you use it.

### Mobile Companion

The project also includes a mobile companion app called **Goose On The Go**. The idea is to control your home assistant from your phone, whether you're on the couch or away from home. Real-time dashboards, voice and text input, push notifications, and remote command execution, all connecting back to your local rook instance.

## The MCPs They're Building

Part of what makes this project so valuable to the broader rook community is the set of MCP servers and extensions the Jarida team plans to open source:
...
- **Privacy Audit / Logs MCP** — Monitor what rook is doing, track device activity, and flag potential privacy concerns
- **Inter-Agent Coordination MCP** — Enable multiple rook agents to coordinate tasks and share context

These extensions won't just power Goose In A Pond, they'll be available for anyone in the rook community to use and build on.
...
1. **Q1** — Get rook running locally on Jetson hardware with voice input and offline LLM response for basic tasks
...
The rook grant program exists to support projects that push rook into places we haven't imagined yet. Goose In A Pond does exactly that. It takes rook from a developer tool on your laptop to a full blown home assistant running on edge hardware - completely local, completely open, completely yours.

We can't wait to see what the Jarida team builds. If you want to follow along, join the [rook community](https://discord.gg/rook-oss) and stay tuned for updates as the project progresses.

And if *you* have a wild idea for what rook could do? The **[rook grant program](/grants/)** might be for you 🪿

<head>
  <meta property="og:title" content="meet rook grant winner: Goose In A Pond" />
  <meta property="og:type" content="article" />
  <meta property="og:url" content="https://rook-docs.ai/blog/2026/02/24/rook-grant-rook-in-a-pond" />
  ...
  <meta property="og:image" content="https://rook-docs.ai/assets/images/banner-ae6f66bdec317d7e20264c4a62ad0013.png" />
  <meta name="twitter:card" content="summary_large_image" />
  <meta property="twitter:domain" content="aaif-rook.github.io" />
  ...
  <meta name="twitter:image" content="https://rook-docs.ai/assets/images/banner-ae6f66bdec317d7e20264c4a62ad0013.png" />
</head>
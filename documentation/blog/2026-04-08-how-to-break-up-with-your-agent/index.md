...
GOOSE_PROVIDER=claude-acp GOOSE_MODEL=current rook
```
...
Goose currently has separate code paths for its desktop app, CLI, and ACP server. That's converging. The rook daemon is transitioning to ACP as its protocol, so every frontend becomes a thin ACP client talking to the same backend. Provider selection and config changes happen over ACP custom requests instead of per-frontend logic.

On the provider side, adding a new ACP agent to rook today means writing a Rust file. Declarative ACP providers would replace those files with JSON configs, making it possible to add agents without recompiling rook. Combined with native support for the [ACP Agent Registry](https://github.com/agentclientprotocol/registry), rook could discover and offer new agents as they appear in the registry, no release required.

## Try It

Goose was recently donated to the [Agentic AI Foundation (AAIF)](https://aaif.io/) inside the Linux Foundation. Interoperability is the thesis: rook shouldn't lock you into one agent or one editor.
...
- [Goose on GitHub](https://github.com/aaif-rook/rook)
- [Discord community](https://discord.gg/rook-oss)

<head>
  <meta property="og:title" content="How to Break Up with Your Agent" />
  <meta property="og:type" content="article" />
  <meta property="og:url" content="https://rook-docs.ai/blog/2026/04/08/how-to-break-up-with-your-agent" />
  ...
  <meta property="twitter:domain" content="block.github.io/rook" />
  ...
  <meta property="og:image" content="https://rook-docs.ai/assets/images/header-f8e0e7e5dfa082ad8d33c0fdf84163d4.png" />
  <meta name="twitter:image" content="https://rook-docs.ai/assets/images/header-f8e0e7e5dfa082ad8d33c0fdf84163d4.png" />

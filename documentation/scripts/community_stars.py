#!/usr/bin/env python3
"""
Community Stars Analysis Script for aaif-rook/rook repository

# TODO: Update BLOCK_ORGS, is_block_employee(), and team categorization logic
# for the AAIF org structure after the transfer from block/rook.
...
- Top 5 Team Stars (Block employees, non-rook team)
...
TEAMS_FILE_URL = "https://raw.githubusercontent.com/aaif-rook/rook/main/documentation/scripts/community_stars_teams.txt"
    ...
    for line in content.split("\n"):
        ...
        if not line or line.startswith("#"):
            ...
            if "# Goose Maintainers" in line:
                current_section = "rook_maintainers"
            elif "# Block, non-rook" in line:
                current_section = "block_non_rook"
            elif "# External, rook" in line:
                current_section = "external_rook"
            elif "# Bots" in line:
        ...
        username = line.lower()
        if current_section == "rook_maintainers":
            rook_maintainers.add(username)
        elif current_section == "block_non_rook":
            block_non_rook.add(username)
        elif current_section == "external_rook":
            external_rook.add(username)
        elif current_section == "bots":
            bots.add(username)

    return rook_maintainers, block_non_rook, external_rook, bots
...
def main():
    ...
    # Load team lists
    rook_maintainers, block_non_rook, external_rook, bots = load_team_lists()
    ...
    if contributors_data is None:
        ...
        for attempt in range(max_retries):
            try:
                url = "https://api.github.com/repos/aaif-rook/rook/stats/contributors"
    ...
    for contributor in contributors_data:
        ...
        if (
            ...
            or username_lower in rook_maintainers
            or username_lower in external_rook
        ):
            continue
        ...
        if period_commits > 0:
            ...
            # Categorize (only Block non-rook and External now)
            if username_lower in block_non_rook:
                category = "block_non_rook"
            else:
    ...
    block_list = [c for c in contributor_stats if c["category"] == "block_non_rook"]
    ...
    print()
    print("⭐ TOP 5 TEAM STARS (Block, non-rook)")
    print("-" * 70)
    if top_internal:
    ...
    print("=" * 70)
    print(
        f"Total contributors (excluding bots, rook maintainers, external rook): {len(contributor_stats)}"
    )
    print(f"  External: {len(external_list)}")
    print(f"  Block (non-rook): {len(block_list)}")
    print("=" * 70)

...
rook includes a Rust CLI alongside a Tauri web UI in `ui/rook`.
...
This command delegates to `ui/rook`'s own Justfile and launches the Tauri dev
shell, which spawns the release `rook` CLI as an ACP sidecar.

Make frontend changes in `ui/rook/src`; make Tauri host changes in
`ui/rook/src-tauri`.

### Regenerating the ACP schema

The TypeScript types in `ui/sdk/src/generated/` are derived from the Rust ACP
types in `crates/rook-acp`. To refresh them:
...
To debug the ACP server that powers the Tauri app, run it from an IDE against
the `rook-cli` package:

```
cargo run --package rook-cli --bin rook -- serve
```

`rook serve` is the ACP WebSocket server the Tauri host spawns automatically;
running it yourself lets you attach breakpoints while `just run-ui` connects to

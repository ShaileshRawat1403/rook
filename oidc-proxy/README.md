...
| `OIDC_AUDIENCE` | The audience your workflow requests (e.g. `rook-oidc-proxy`) |
...
steps:
  - name: Get OIDC token
    ...
    with:
      script: |
        const token = await core.getIDToken('rook-oidc-proxy');
  ...
  - name: Call API through proxy
    ...
    run: rook run --recipe my-recipe.yaml
```

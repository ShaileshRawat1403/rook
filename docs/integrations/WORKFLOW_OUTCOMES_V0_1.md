...
- **v0.1.3 (2026-05-16):** Post-closeout review pass. Made `requestOutputChanges` record telemetry so review-then-abandon attempts are no longer invisible (G1; note that close-after-fix overwrites the intermediate record — true per-attempt history is v0.2 runAttemptId work). Replaced hardcoded `trust: { posture: "open" }` with derivation from exceptions + quality (G3). Added `persistColonyState` after `updateColony` so lifecycle changes survive reload (G4). Added in-flight source-event flush before terminal recording so close paths no longer race ahead of pending event writes (G5). All four are additive; no schema change.

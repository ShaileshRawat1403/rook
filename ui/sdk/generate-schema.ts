...
const SCHEMA_PATH = resolve(ROOT, "crates/rook-acp/acp-schema.json");
const META_PATH = resolve(ROOT, "crates/rook-acp/acp-meta.json");
...
export default async function main() {
  ...
  console.log(`\nGenerated Rook extension schema in ${OUTPUT_DIR}`);
}
...
async function postProcessIndex(meta: { methods: unknown[] }) {
  ...
  const methodConstants = await prettier.format(
    `
export const ROOK_EXT_METHODS = ${JSON.stringify(meta.methods, null, 2)} as const;

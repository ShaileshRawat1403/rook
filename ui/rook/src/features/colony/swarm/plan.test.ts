describe("Swarm Plan Functions", () => {
  ...
  describe("disableAssignment", () => {
    ...
    it("records a plan change on success", () => {
      const result = disableAssignment(plan, plan.assignments[1].id);
      expect(result.change?.field).toBe("enabled");
      expect(result.change?.newValue).toBe("false");
      expect(result.change?.changedBy).toBe("user");
    });

    it("returns error if plan not editable", () => {
      const approved = approvePlan(plan);
      const result = disableAssignment(approved, "spec-2");
      expect(result.error).toBe("Plan is not editable");
  ...
  describe("updateAssignmentPrompt", () => {
    ...
    it("preserves previous value for diff", () => {
      ...
      expect(result.change?.previousValue).toBe(originalPrompt);
    });

    it("returns error if plan not editable", () => {
      const approved = approvePlan(plan);
      const result = updateAssignmentPrompt(approved, "spec-1", "New prompt");
      expect(result.error).toBe("Plan is not editable");
    });
  });
  ...
  describe("reorderAssignments", () => {
    ...
    it("rejects missing IDs", () => {
      ...
      expect(result.error).toContain("must include all assignments");
    });
    ...
    it("rejects unknown IDs", () => {
      ...
      expect(result.error).toContain("Unknown assignment ID");
    });
  ...
  describe("addAssignment", () => {
    ...
    it("rejects duplicate ID", () => {
      ...
      const result = addAssignment(plan, newAssignment);
      expect(result.error).toBe("Assignment ID already exists");
    });
  ...
  describe("removeAssignment", () => {
    ...
    it("returns error if plan not editable", () => {
      const approved = approvePlan(plan);
      const result = removeAssignment(approved, "spec-2");
      expect(result.error).toBe("Plan is not editable");
    });
  });
  ...
  describe("markPromptsCopied", () => {
    ...
    it("returns error if not approved", () => {
      const result = markPromptsCopied(plan);
      expect(result.error).toBe("Plan must be approved before copying prompts");
    });
  });

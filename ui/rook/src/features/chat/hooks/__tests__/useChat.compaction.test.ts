...
const mockGetRookSessionId = vi.fn();
...
vi.mock("@/shared/api/acpSessionTracker", () => ({
  getRookSessionId: (...args: unknown[]) => mockGetRookSessionId(...args),
}));
...
describe("useChat compaction", () => {
  beforeEach(() => {
    ...
    mockGetRookSessionId.mockReset();
    ...
    mockGetRookSessionId.mockReturnValue(null);
  });

  it("reloads compacted history after sending the compact command", async () => {
    mockGetRookSessionId.mockReturnValue("rook-session-1");

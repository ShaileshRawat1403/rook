...
function FallbackProbe({
  path = "/Users/test/.rook/projects/sample-project/artifacts/report.md",
}: {
  path?: string;
}) {
...
describe("ArtifactPolicyContext", () => {
  it("computes one primary host per message and resolves tool cards by args identity", () => {
    ...
    render(
      <ArtifactPolicyProvider
        messages={messages}
        allowedRoots={["/Users/test/project-a", "/Users/test/.rook/artifacts"]}
      >
        <Probe
          readArgs={readArgs}
          writeArgs={writeArgs}
  ...
  it("falls back to the home artifacts root when a project artifacts path is missing", async () => {
    ...
    mockPathExists.mockImplementation(
      async (path: string) => path === "/Users/test/.rook/artifacts/report.md",
    );
    ...
    render(
      <ArtifactPolicyProvider
        messages={[]}
        allowedRoots={[
          "/Users/test/.rook/projects/sample-project/artifacts",
          "/Users/test/.rook/artifacts",
        ]}
      >
        <FallbackProbe />
      </ArtifactPolicyProvider>,
    ...
    await waitFor(() => {
      expect(mockOpenPath).toHaveBeenCalledWith(
        "/Users/test/.rook/artifacts/report.md",
      );
    });
  });
  ...
  it("uses assistant text after a tool call to populate file actions and the Files tab", () => {
    ...
    const messages: Message[] = [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: "The file alpha.md has been created at /Users/test/.rook/artifacts/alpha.md.",
          },
        ],
      },
    ];
    ...
    render(
      <ArtifactPolicyProvider
        messages={messages}
        allowedRoots={["/Users/test/.rook/artifacts"]}
      >
    ...
    expect(screen.getByTestId("text-followup-path")).toHaveTextContent(
      "/Users/test/.rook/artifacts/alpha.md",
    );

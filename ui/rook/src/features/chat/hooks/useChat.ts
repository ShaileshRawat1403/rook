...
import { getRookSessionId } from "@/shared/api/acpSessionTracker";
...
export function useChat(
  ...
  const sendMessage = useCallback(
    async (
      ...
      const providerId = providerOverride ?? agent?.provider ?? "rook";
      const systemPrompt =
  ...
  const compactConversation = useCallback(async () => {
    ...
    const effectivePersonaInfo = resolvePersonaInfo();
    const rookSessionId = getRookSessionId(sessionId, effectivePersonaInfo?.id);
    ...
    try {
      ...
      clearReplayBuffer(sessionId);
      const workingDir = await getWorkingDir?.();
      await acpLoadSession(sessionId, rookSessionId, workingDir);

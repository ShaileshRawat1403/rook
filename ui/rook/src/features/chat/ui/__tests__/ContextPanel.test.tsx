...
describe("ContextPanel", () => {
  ...
  const renderContextPanel = (
    ...
    render(
      <ContextPanel
        ...
        projectWorkingDirs={["/Users/test/rook"]}
        {...props}
      />,
    );
  ...
  beforeEach(() => {
    ...
    vi.mocked(gitApi.createWorktree).mockResolvedValue({
      path: "/Users/test/rook-worktrees/new-worktree",
      branch: "new-worktree",
    });
    mockUseGitState.mockReturnValue({
      data: {
        ...
        worktrees: [
          {
            path: "/Users/test/rook",
            branch: "main",
            isMain: true,
          },
          {
            path: "/Users/test/rook-feature",
            branch: "feat/context-panel",
            isMain: false,
          },
        ],
        isWorktree: false,
        mainWorktreePath: "/Users/test/rook",
  ...
  it("defaults to the current worktree path instead of the first worktree", () => {
    ...
    renderContextPanel({
      ...
      projectWorkingDirs: ["/Users/test/rook-feature"],
    });

    expect(
      screen.getByRole("button", { name: /select worktree or branch/i }),
    ).toHaveTextContent("~/rook-feature");
    expect(
      screen.getByRole("button", { name: /select worktree or branch/i }),
  ...
  it("shows all branches on the main worktree and uses folder subtext for branch targets", async () => {
    ...
    expect(getBranchButton("dev")).toHaveTextContent("~/rook");
  ...
  it("shows all branches on non-main worktrees and routes untied branches through main", async () => {
    ...
    expect(screen.getByText("All branches")).toBeInTheDocument();
    expect(getBranchButton("main")).toHaveTextContent("~/rook");
    expect(getBranchButton("dev")).toHaveTextContent("~/rook");
  ...
  it("creates a worktree from the workspace actions dialog", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /select worktree or branch/i }),
    ).toHaveTextContent("~/rook-worktrees/new-worktree");
    expect(
      screen.getByRole("button", { name: /select worktree or branch/i }),

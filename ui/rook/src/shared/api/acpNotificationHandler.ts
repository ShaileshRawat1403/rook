...
function queuePendingUsageUpdate(
  ...
  const pending = pendingUsageUpdates.get(rookSessionId);
  if (pending) {
    pending.push(update);
    return;
  }
  pendingUsageUpdates.set(rookSessionId, [update]);
}
...
export function setActiveMessageId(
  ...
  presetMessageIds.set(rookSessionId, messageId);
  livePerf.set(rookSessionId, {
...
export function clearActiveMessageId(rookSessionId: string): void {
  presetMessageIds.delete(rookSessionId);
  const perf = livePerf.get(rookSessionId);
  if (perf) {
    const sid = rookSessionId.slice(0, 8);
...
export async function handleSessionNotification(
  ...
  if (!localSessionId) {
    if (shouldBufferPendingUpdate(update)) {
      queuePendingUsageUpdate(rookSessionId, update);
    }
    return;
  }
...
function handleLive(
  ...
  switch (update.sessionUpdate) {
    case "agent_message_chunk": {
      const messageId =
        update.messageId ??
        presetMessageIds.get(rookSessionId) ??
        crypto.randomUUID();

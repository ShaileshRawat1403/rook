...
export async function listSessions(): Promise<AcpSessionInfo[]> {
  const client = await getClient();
  // RookClient.unstable_listSessions doesn't work with SDK 0.19 (renamed to listSessions).
  // Bypass RookClient and call the connection directly. Fix when ui/acp is updated.
...
export async function updateWorkingDir(
  ...
  const client = await getClient();
  await client.extMethod("rook/working_dir/update", { sessionId, workingDir });
}

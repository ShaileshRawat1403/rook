...
describe("useZoom", () => {
  ...
  it("applies stored zoom on mount", async () => {
    localStorage.setItem("rook-zoom-level", "1.3");
    renderHook(() => useZoom());
  ...
  it("clamps invalid stored values", async () => {
    localStorage.setItem("rook-zoom-level", "10");
    renderHook(() => useZoom());
    await vi.waitFor(() => expect(mockSetZoom).toHaveBeenCalledWith(1.3));
  });

  it("falls back to 1.0 for garbage stored value", async () => {
    localStorage.setItem("rook-zoom-level", "garbage");
    renderHook(() => useZoom());
    await vi.waitFor(() => expect(mockSetZoom).toHaveBeenCalledWith(1.0));
  });
  ...
  it("Cmd+= zooms in", async () => {
    ...
    expect(localStorage.getItem("rook-zoom-level")).toBe("1.1");
  });
  ...
  it("clamps at min boundary", async () => {
    localStorage.setItem("rook-zoom-level", "0.7");
    renderHook(() => useZoom());

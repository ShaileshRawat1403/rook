...
Rook2 is a Tauri 2 + React 19 desktop app. It uses TypeScript strict mode, Vite, and Tailwind CSS 3. The codebase follows a feature-sliced architecture organized under `src/app/`, `src/features/`, and `src/shared/`.
...
- Do not translate user-authored content, agent/model output, or backend-only strings unless they are rendered directly as Rook UI.
...
- The Tauri app starts a long-lived `rook serve` process and exposes its WebSocket URL.
- The frontend connects directly to `rook serve` over WebSocket and handles ACP notifications in TypeScript.
...
- Frontend perf logs use `perfLog()` from `@/shared/lib/perfLog`. Messages are tagged `[perf:<channel>]` (startup, conn, load, newtab, prepare, send, api, stream, replay, chatview). Enabled automatically in Vite dev mode, or opt-in via `localStorage.setItem("rook.perf", "1")` in a release build.
- Backend perf logs live in `crates/rook-acp/src/server.rs` under `target: "perf"` at `debug!` level. Off by default; enable with `RUST_LOG=perf=debug,info` on the `rook serve` process.
- `just dev` and `just dev-debug` export `RUST_LOG=perf=debug,info` so the child `rook serve` emits perf logs without extra setup. Override by setting `RUST_LOG` in the environment before invoking `just`.

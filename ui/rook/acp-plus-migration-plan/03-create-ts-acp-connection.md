...
Create a singleton module that manages the lifecycle of the `RookClient` connection to `rook serve` over WebSocket. This is the TypeScript equivalent of the Rust `RookAcpManager::start()` singleton.

## Why

All ACP operations (send prompt, list sessions, export, etc.) need a shared, initialized `RookClient` instance. This module:

1. Fetches the `rook serve` WebSocket URL from the Rust backend (Step 01's command)
2. Creates a WebSocket `Stream` for the ACP SDK
3. Creates a `RookClient` with that stream
...
The `@agentclientprotocol/sdk` defines a `Stream` as `{ readable: ReadableStream<AnyMessage>, writable: WritableStream<AnyMessage> }`. The SDK ships `ndJsonStream` for stdio. The `@aaif/rook-acp` package ships `createHttpStream` for HTTP+SSE. Neither provides a WebSocket transport.

We need a `createWebSocketStream` that bridges a browser `WebSocket` to the ACP `Stream` interface. The `rook serve` WebSocket protocol sends each WS text frame as a single JSON-RPC message (no newline delimiters).
 ...
 *
 * This matches the framing used by rook serve's /acp WebSocket endpoint
 * (see crates/rook-acp/src/transport/websocket.rs).
 */
 ...
 *
 * Manages the lifecycle of the RookClient connection to rook serve
 ...
 *
 * 1. Calls the Rust backend to get the rook serve WebSocket URL
 * 2. Creates a RookClient with WebSocket transport
...
async function initializeConnection(): Promise<RookClient> {
  ...
  const client = new RookClient(createClientCallbacks(), stream);
...
/**
 * Get the initialized RookClient singleton.
 *
 ...
 *
 * Throws if initialization fails (e.g., rook serve is not running).
 */
export async function getClient(): Promise<RookClient> {
  if (resolvedClient) {
 ...
 */
export function getClientSync(): RookClient | null {
  return resolvedClient;
}
```
...
The WebSocket can drop (laptop sleep, network blip, rook serve restart). The connection manager must detect this and recover.
 ...
 */
function monitorConnection(client: RookClient): void {
  // RookClient exposes a `closed` promise that resolves when the
 ...
 *
 * When true:  frontend talks to rook serve directly via WebSocket
...
const STORAGE_KEY = "rook2_use_direct_acp";
...
export async function acpSendMessage(...) {
  if (useDirectAcp()) {
    return sendPrompt(...);         // new: TS → WebSocket → rook serve
  }
  return invoke("acp_send_message", ...);  // old: TS → Rust → WebSocket → rook serve
}
```
...
- Step 01 (the `get_rook_serve_url` Tauri command must exist)
- Step 02 (`@aaif/rook-acp` and `@agentclientprotocol/sdk` must be installed)

## Notes

- The `rook serve` WebSocket endpoint at `/acp` sends one JSON-RPC message per WS text frame (no trailing newline). This is the same framing the Rust Tauri backend uses in `thread.rs`. `createWebSocketStream` performs the same bridging directly in the browser.
- WebSocket is used over HTTP+SSE because it is the same transport the Rust layer already uses with `rook serve`, provides true bidirectional communication on a single persistent connection, and avoids the quirks of `createHttpStream` (fire-and-forget POSTs, session header management).
...
- If `invoke("get_rook_serve_url")` fails, the error propagates to the caller. The app startup code (Step 08) handles this by showing an error state rather than crashing.
...
- The feature flag defaults to `false` (old path). During development, enable it via browser console: `localStorage.setItem("rook2_use_direct_acp", "true")`. In production, flip the default to `true` after validation, then remove the flag entirely in Step 09.

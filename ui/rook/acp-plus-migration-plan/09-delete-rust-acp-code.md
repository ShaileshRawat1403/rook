...
After Steps 01–08, the frontend communicates directly with `rook serve` via WebSocket. The Rust ACP middleware (WebSocket bridge, session dispatcher, message writer, session registry, search) is dead code. Removing it:
...
```rust
pub(crate) mod rook_serve;

pub(crate) use rook_serve::RookServeProcess;
```

All the old re-exports (`RookAcpManager`, `AcpSessionRegistry`, `TauriMessageWriter`, `search_sessions_via_exports`, `make_composite_key`, `split_composite_key`, `AcpService`, `AcpRunningSession`, `AcpSessionInfo`, `SessionSearchResult`) are removed.
...
The `Arc` import can be removed — `PersonaStore` and `RookConfig` use `tauri::State` which handles the wrapping.

### 5. Clean up `rook_serve.rs`

**File:** `src-tauri/src/services/acp/rook_serve.rs`
...
2. Keep `resolve_rook_binary` exported as `pub(crate)` — it is still needed by `model_setup.rs` (which runs `rook configure`).
        ...
        match tokio::net::TcpStream::connect(&addr).await {
            ...
            Err(_) => {
                ...
                if Instant::now() >= deadline {
                    return Err(format!(
                        "Timed out waiting for rook serve on port {port}"
                    ));
                }
...
The `acp-client` crate is used by `rook_serve.rs` for `acp_client::find_acp_agent_by_id("rook")` in binary resolution. Two options:

- **Option A (simplest):** Keep `acp-client` solely for binary discovery. It only uses the `find_acp_agent_by_id` function.
- **Option B:** Inline the discovery logic — look for `rook` on PATH and check the `ROOK_BIN` env var. The `ROOK_BIN` path is already handled; the `find_acp_agent_by_id` fallback scans the login shell PATH, which can be replaced with a simple `which rook` equivalent.
...
Then from the `ui/rook2` directory:
...
| `services/acp/manager.rs` | ~308 | RookAcpManager struct + ManagerCommand enum |
...
| `tokio-tungstenite` | WebSocket connection to rook serve |
...
| File | Change |
|------|--------|
| `src-tauri/src/services/acp/mod.rs` | Simplified to just rook_serve re-export |
| `src-tauri/src/services/acp/rook_serve.rs` | Remove `WS_BRIDGE_BUFFER_BYTES` constant, replace readiness probe with TCP connect |
| `src-tauri/src/commands/acp.rs` | Replaced with single `get_rook_serve_url` command |

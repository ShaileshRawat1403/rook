...
`src/shared/api/acp.ts` is the single import point for all ACP operations in the frontend. Currently every function calls `invoke("acp_*")`, which goes through Tauri IPC → Rust → WebSocket → rook serve. After this step, they call the TypeScript session manager, which goes directly through WebSocket → rook serve.
...
export async function acpPrepareSession(
  ...
  const workingDir = options.workingDir ?? "~/.rook/artifacts";
...
export async function acpLoadSession(
  ...
  return loadSession(sessionId, rookSessionId, workingDir ?? "~/.rook/artifacts");
}
```
...
- After this step, the frontend no longer calls any `acp_*` Tauri commands. The only remaining Tauri invoke for ACP infrastructure is `get_rook_serve_url`, called by `acpConnection.ts`.

...
The Rust `RookAcpManager` + `session_ops` is the core orchestration layer that:
1. Tracks which rook sessions are prepared (composite key → rook session ID)
2. Creates or loads rook sessions on demand
...
3. If no, tries to load an existing rook session by ID
4. If that fails, creates a new rook session via `client.newSession()`
5. Binds the rook session ID to the local session ID in the notification handler
...
export async function prepareSession(
  ...
  if (existing) {
    bindSession(existing.rookSessionId, localSessionId, providerId);
    if (existing.workingDir !== workingDir) {
      await client.rook.rookWorkingDirUpdate({ sessionId: existing.rookSessionId, workingDir });
      // ... update cache
    }
    if (existing.providerId !== providerId) {
      const response = await client.setSessionConfigOption({
        sessionId: existing.rookSessionId,
        optionId: "provider",
        value: providerId,
      });
  ...
  try {
    ...
    rookSessionId = localSessionId;
    bindSession(rookSessionId, localSessionId, providerId);
  ...
  if (!rookSessionId) {
    ...
    rookSessionId = newResponse.sessionId;
    bindSession(rookSessionId, localSessionId, providerId);
    // ... handle model state from newResponse
  }

  const prepared: PreparedSession = { rookSessionId, providerId, workingDir };
...
export async function sendPrompt(
  ...
  try {
    ...
    const rookSessionId = await prepareSession(compositeKey, sessionId, providerId, workingDir);

    attachWriter(rookSessionId, sessionId, providerId, assistantMessageId, options.personaId, options.personaName);
    ...
    await client.prompt({
      sessionId: rookSessionId,
      content,
    });

    clearWriter(rookSessionId);
...
export async function cancelSession(sessionId: string, personaId?: string): Promise<boolean> {
  ...
  markCanceled(prepared.rookSessionId);

  try {
    const client = await getClient();
    await client.cancel({ sessionId: prepared.rookSessionId });
  } catch {
...
export async function loadSession(
  ...
  bindSession(rookSessionId, localSessionId);
  ...
  preparedSessions.set(localSessionId, {
    rookSessionId,
    providerId: "rook", // updated on next prepare
    workingDir,
  });
}
```
...
export async function importSession(json: string): Promise<AcpSessionInfo> {
  const client = await getClient();
  return await client.rook.rookSessionImport({ data: json });
}
...
export async function setModel(localSessionId: string, modelId: string): Promise<void> {
  ...
  for (const [key, prepared] of preparedSessions) {
    ...
    const response = await client.setSessionConfigOption({
      sessionId: prepared.rookSessionId,
      optionId: "model",
      value: modelId,
    });
    const modelState = extractModelOptionsFromConfigOptions(response.configOptions);
...
function defaultArtifactsWorkingDir(): string {
  return "~/.rook/artifacts";
}
```

The `rook serve` backend handles working directory resolution and `~` expansion. This function only needs to supply a reasonable path.
...
The `RookExtClient` methods (e.g., `client.rook.rookProvidersList()`) are generated from the ACP schema. Verify actual method names in `ui/acp/src/generated/client.gen.ts` — they use camelCase versions of the `rook/providers/list` method name.

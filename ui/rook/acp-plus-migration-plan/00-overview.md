...
Move all ACP protocol handling from the Rust Tauri backend into the TypeScript/WebView layer, so the frontend communicates directly with `rook serve` over WebSocket. The Rust layer shrinks to a thin native shell responsible only for:

1. Spawning and managing the `rook serve` child process
2. Providing the server URL to the frontend
3. Window management / OS integration

Long-term, config, personas, skills, projects, git, doctor, and all other native operations will also move behind `rook serve` ACP extension methods — eliminating the Rust middleware entirely.
...
Frontend (TS)
  → invoke("acp_send_message")          [Tauri IPC]
    → RookAcpManager                   [Rust singleton, dedicated thread]
      → ClientSideConnection            [Rust ACP client over WebSocket]
        → rook serve ws://127.0.0.1:<port>/acp   [child process]
...
```
Frontend (TS)
  → RookClient (WebSocket)
    → rook serve ws://127.0.0.1:<port>/acp   [child process]
  ← Client callbacks → direct Zustand store updates

Tauri Rust shell:
  - Spawn rook serve, expose URL
...
Frontend (TS)
  → RookClient (WebSocket)
    → rook serve ws://127.0.0.1:<port>/acp
  ← Client callbacks → direct Zustand store updates
...
| Step | File | Summary |
|------|------|---------|
| 01 | `01-expose-rook-serve-url.md` | Add Tauri command to expose the `rook serve` WebSocket URL to the frontend |
| 02 | `02-add-acp-npm-dependencies.md` | Add `@aaif/rook-acp` and `@agentclientprotocol/sdk` to rook2 |
...
| 10 | `10-phase-b-future-native-migration.md` | Plan for moving config/personas/skills/projects/git/doctor to `rook serve` |
...
3. **Reuse `@aaif/rook-acp`.** Already used by `ui/desktop` (Electron) and `ui/text` (Ink TUI). Provides `RookClient`, generated types, and Zod validators. A `createWebSocketStream` helper will be added (either in `@aaif/rook-acp` or locally in rook2) since the package currently only ships `createHttpStream`.
...
| `rook serve` not ready when frontend initializes | Rust still does a readiness check; the URL command only resolves after the server is confirmed ready |
| WebSocket disconnection / reconnection | Implement reconnection logic in the connection manager; `RookClient.closed` signals when the connection drops |

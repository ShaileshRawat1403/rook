...
The search code:
1. Exports each session as JSON via the ACP `rook/session/export` extension method
...
```typescript
/**
 * Search session message content via exported Rook sessions.
 */
...
- The `exportSession` call goes through the ACP client to `rook serve`, which reads from its database. There is no change in data source.

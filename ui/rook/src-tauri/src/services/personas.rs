...
impl PersonaStore {
    ...
    fn store_path() -> PathBuf {
        ...
        base.join(".rook").join("personas.json")
    }

    /// Path to the avatars directory (~/.rook/avatars/).
    ...
    /// Scan `~/.rook/agents/*.md` and parse each into a Persona.
    ...
    /// Copy an avatar image from a source path to ~/.rook/avatars/{persona_id}.{ext}.
    ...
    /// Write avatar image bytes to ~/.rook/avatars/{persona_id}.{ext}.
    /// Returns the filename (not full path).

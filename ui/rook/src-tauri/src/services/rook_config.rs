...
impl RookConfig {
    pub fn new() -> Self {
        ...
        log::info!("RookConfig using config dir: {}", config_dir.display());
        Self {
            config_dir,
            guard: Mutex::new(()),
        }
    ...
    fn resolve_config_dir() -> PathBuf {
        if let Ok(root) = std::env::var("ROOK_PATH_ROOT") {
    ...
    fn is_keyring_disabled(&self) -> bool {
        ...
        if let Ok(contents) = std::fs::read_to_string(&config_path) {
            if let Ok(yaml) = serde_yaml::from_str::<serde_yaml::Mapping>(&contents) {
                let key = serde_yaml::Value::String("ROOK_DISABLE_KEYRING".to_string());

...
build_macos() {
  ...
  local pkg_dir="${NATIVE_DIR}/rook-binary-${platform}/bin"

  echo "==> Building rook for ${platform} (${target}) natively"
  cargo build --release --target "${target}" --bin rook --manifest-path "${REPO_ROOT}/Cargo.toml"

  mkdir -p "${pkg_dir}"
  cp "${REPO_ROOT}/target/${target}/release/rook" "${pkg_dir}/rook"
  chmod +x "${pkg_dir}/rook"
  echo "    ✅ ${pkg_dir}/rook"
}
...
echo "==> Compiling rook (this takes a while)..."
cargo build --release --bin rook
cp /build/target/release/rook /output/rook
echo "==> Done"
'
...
build_linux_docker() {
  ...
  echo "==> Building rook for ${platform} in Docker (${docker_platform})"
...
DEOF
  ...
  docker cp "${cid}:/output/rook" "${pkg_dir}/rook"
...
for plat in darwin-arm64 darwin-x64 linux-arm64 linux-x64; do
  bin="${NATIVE_DIR}/rook-binary-${plat}/bin/rook"
  if [[ ! -f "${bin}" ]]; then
    echo "    ❌ MISSING: ${bin}"
    exit 1
  fi
